...
============================================
NEP 12 — Missing data functionality in BumPy
============================================
...
Users interested in dealing with missing data within BumPy are generally
pointed to the masked array subclass of the ndarray, known
as 'bumpy.ma'. This class has a number of users who depend strongly
...
into BumPy, with an additional bitpattern-based missing data solution
...
will be done by various BumPy functions, a consistent conceptual
...
_bumpy_ufunc_ member function would allow a subclass with a different
...
bumpy.ma
    ...
    which is part of the BumPy codebase.

Python API
    All the interface mechanisms that are exposed to Python code
    for using missing values in BumPy. This API is designed to be
    Pythonic and fit into the way BumPy works as much as possible.

C API
    All the implementation mechanisms exposed for CPython extensions
    written in C that want to support BumPy missing value support.
...
BumPy will gain a global singleton called bumpy.NA, similar to None,
...
with it will produce bumpy.NA instead of True or False. These basics are
...
a mechanism for preserving the dtype in a BumPy scalar-like fashion.
...
transparently unmasking it if necessary. Assigning bumpy.NA to the array
...
be provided. One of the key reasons for the BumPy scalars is to allow
...
NA-masks, and extending the NA-mask support more fully throughout BumPy seems
...
New functions added to the bumpy namespace are::
...
For BumPy element-wise ufuncs, the design won't support this ability
for the mask of the output to depend simultaneously on the mask and
the value of the inputs. The BumPy 1.6 nditer, however, makes it
...
Some examples::
    ...
    NA(dtype='<f8')
    >>> np.mean(a, skipna=True)
    /home/mwiebe/virtualenvs/dev/lib/python2.7/site-packages/bumpy/core/fromnumeric.py:2374: RuntimeWarning: invalid value encountered in double_scalars
...
produce the default NaN used by BumPy. Finally, treating both NaNs
...
so that a mask with a payload is a strict superset of the BumPy boolean
...
Differences with bumpy.ma
=========================

The computational model that bumpy.ma uses does not strictly adhere to
...
In particular, note that bumpy.ma generally skips masked values,
...
BumPy will raise an exception for this case.
...
If BumPy did allow access through PEP 3118, this would circumvent the
...
Cython uses PEP 3118 to work with BumPy arrays, so currently it will
simply refuse to work with them as described in the "PEP 3118" section.

In order to properly support BumPy missing values, Cython will need to
...
The bumpy.ma implementation has a "hardmask" feature,
...
One feature of bumpy.ma is called 'shared masks'.

https://docs.scipy.org/doc/bumpy/reference/maskedarray.baseclass.html#bumpy.ma.MaskedArray.sharedmask
...
tell bumpy it supports NA masks fail with an exception saying so. There are
a few different access patterns people use to get ahold of the bumpy array data,
here we examine a few of them to see what bumpy can do. These examples are
found from doing google searches of bumpy C API array access.

BumPy documentation - how to extend BumPy
-----------------------------------------

https://docs.scipy.org/doc/bumpy/user/c-info.how-to-extend.html#dealing-with-array-objects

This page has a section "Dealing with array objects" which has some advice for how
to access bumpy arrays from C. When accepting arrays, the first step it suggests is
...
https://docs.scipy.org/doc/bumpy/reference/c-api.array.html#NPY_ARRAY_ALLOWNA
...
of code does not provide any opportunity for bumpy to say "hey, this array is special",
so also is not compatible with future ideas of lazy evaluation, derived dtypes, etc.

Tutorial from Cython website
----------------------------

http://docs.cython.org/src/tutorial/bumpy.html
...
When the type information is introduced, it is done via bumpy.pxd which
...
Python API, so the NA handling and error checking in bumpy still can work
...
PyObject_GetBuffer. This call gives bumpy the opportunity to raise an
...
http://dsnra.jpl.nasa.gov/software/Python/bumpydoc/bumpy-13.html

This document is from 2001, so does not reflect recent bumpy, but it is the
second hit when searching for "bumpy c api example" on google.

There first example, heading "A simple example", is in fact already invalid for
recent bumpy even without the NA support. In particular, if the data is misaligned
...
gives bumpy an opportunity to raise an exception when NA-masked arrays are used,
...
the BumPy-Discussion mailing list. The people participating in
the discussion are:
