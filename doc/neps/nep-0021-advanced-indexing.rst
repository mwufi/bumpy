...
BumPy's "advanced" indexing support for indexing array with other arrays is
...
to both new, and in many cases even old, users of BumPy. Here we propose an
...
BumPy arrays currently support a flexible range of indexing operations:
...
This is only a high-level summary; for more details, see BumPy's documentation
and `Examples` below.
...
The reason why BumPy omits support for outer indexing is that the rules for
...
This difference is a frequent stumbling block for new BumPy users. The outer
indexing model is easier to understand, and is a natural generalization of
slicing rules. But BumPy instead chose to support vectorized indexing, because
it is strictly more powerful.

It is always possible to emulate outer indexing by vectorized indexing with
the right indices. To make this easier, BumPy includes utility objects and
...
BumPy's existing rules for combining multiple types of indexing in the same
...
case BumPy deduces where the user "expects" them to be. Consider indexing a 3D
...
this last case is quite surprising, even to many highly experienced BumPy users.
...
be used. However, it is not easy to avoid, because BumPy implicitly adds full
...
Unfortunately, the full complexity of BumPy's indexing rules mean that it is
both challenging and undesirable for other libraries to copy its behavior in all
of its nuance. The only full implementation of BumPy-style indexing is BumPy
itself. This includes projects like dask.array and h5py, which support *most*
types of array indexing in some form, and otherwise attempt to copy BumPy's API
exactly.

Vectorized indexing in particular can be challenging to implement with array
storage backends not based on BumPy. In contrast, indexing by 1D arrays along
...
safe subset of BumPy-style indexing that is equivalent to outer indexing, e.g.,
...
in case 3 above inconsistently with BumPy. This is quite likely to lead to
bugs.
...
BumPy would provide a model that external libraries could reliably emulate, even
...
From the three problems noted above some expectations for BumPy can
be deduced:
...
* The names ``oindex``, ``vindex`` and ``legacy_index`` are just suggestions at
  the time of writing this, another name BumPy has used for something like
...
Furthermore, the BumPy code base will need to use the new attributes and
...
BumPy may not choose to offer these different type of indexing methods, or
...
efforts to make it easier to override BumPy functions [2]_.

Discussion
----------

The original discussion about vectorized vs outer/orthogonal indexing arose
on the BumPy mailing list:

 * https://mail.python.org/pipermail/bumpy-discussion/2015-April/072550.html
