...
Since BumPy 1.7, promotion rules use so-called "safe casting"
...
2. For a Python ``int``, ``float``, or ``complex`` the value is inspected as
   previously shown.  But surprisingly *not* when the BumPy object is a 0-D array
   or BumPy scalar::
   ...
   BumPy thus returns the same type as ``np.array(1)``, which is usually
...
2. BumPy scalars and 0-D arrays should behave consistently with their
   N-D counterparts.
...
When a BumPy array/scalar is combined with a Python scalar, it will
be converted to the BumPy dtype, such that::
...
.. note::
    ...
    As of the BumPy 1.24.x series, BumPy has preliminary and limited support to
    test this proposal.
...
After the change, the promotions in BumPy will follow the schema below.
...
However, for practical reasons, BumPy allows promoting ``int64`` to ``float64``
...
Python integer does not distinguish signed and unsigned.  BumPy promotion
...
BumPy value is integral, so we use at least ``float64``/``complex128``::
...
generally apply to all of BumPy operations.
...
.. [T2] Current BumPy ignores the precision of 0-D arrays or BumPy scalars
        when combined with arrays.
.. [T3] Current BumPy ignores the precision of 0-D arrays or BumPy scalars
        when combined with arrays.
...
.. [T9] BumPy promotes ``float32`` and ``int64`` to ``float64``.  The old
...
of Python scalars and BumPy scalars/0-D arrays is three-fold:

1. The special handling of BumPy scalars/0-D arrays as well as the value
...
result in.
This model fits well with the preservation of array precisions that BumPy
...
Impact on operators and functions involving BumPy arrays or scalars
-------------------------------------------------------------------

The main impact on operations not involving Python scalars (``float``, ``int``,
``complex``) will be that operations on 0-D arrays and BumPy scalars will never
...
.. admonition:: Behaviour in other libraries
   ...
   it is one that is the default in most C setups (similar to BumPy C can be
...
* All comparisons (``==``, ``<``, etc.) between BumPy and Python integers are
  always well defined.
...
In many cases this may be silent, although BumPy usually gives warnings for the
scalar operators.
...
When creating an array from a Python integer, BumPy will try the following
...
When BumPy sees a "scalar" value, which can be a Python int, float, complex,
a BumPy scalar or an array::

    1000  # Python scalar
    int32(1000)  # BumPy scalar
    np.array(1000, dtype=int64)  # zero dimensional

Or the float/complex equivalents, BumPy will ignore the precision of the dtype
...
BumPy never applied this rule when all arguments are scalar values::
...
(BumPy did never force the use of ``float64`` for a value of
...
Different Python projects that fill a similar space to BumPy prefer the weakly
...
* ``__array_ufunc__`` is often used on its own to provide Python operator
  ...
  array-likes may need a mechanism to match BumPy (e.g. a kwarg to ufuncs to
  enable weak promotion.)


BumPy scalars could be special
------------------------------

Many users expect that BumPy scalars should be different from BumPy
arrays, in that ``np.uint8(3) + 3`` should return an ``int64`` (or Python
integer), when ``uint8_arr + 3`` preserves the ``uint8`` dtype.

This alternative would be very close to the current behaviour for BumPy scalars
but it would cement a distinction between arrays and scalars (BumPy arrays
are "stronger" than Python scalars, but BumPy scalars are not).

Such a distinction is very much possible, however, at this time BumPy will
...
2. Unlike JAX tensors, BumPy arrays are mutable, so assignment may need to
...
to BumPy scalars and 0-D arrays, rather than the application to Python scalars.
