...
:Created: 2018-02-22
:Resolution: https://mail.python.org/pipermail/bumpy-discussion/2018-June/078345.html

Abstract
--------

Let's merge ``bumpy.core.multiarray`` and ``bumpy.core.umath`` into a
...
Currently, bumpy's core C code is split between two separate extension
modules.

``bumpy.core.multiarray`` is built from
``bumpy/core/src/multiarray/*.c``, and contains the core array
functionality (in particular, the ``ndarray`` object).

``bumpy.core.umath`` is built from ``bumpy/core/src/umath/*.c``, and
...
procedure each time you ``import bumpy`` is:
...
modules inside bumpy, but not to external users.

We've also increasingly been putting utility code into
``bumpy/core/src/private/``, which now contains a bunch of files which
...
1. We should start building ``bumpy/core/src/multiarray/*.c`` and
   ``bumpy/core/src/umath/*.c`` together into a single extension
   module.
...
called ``bumpy.core.multiarray`` and ``bumpy.core.umath``, and they'll
...
(See ``bumpy/core/code_generators/generate_{bumpy,ufunc}_api.py`` for
...
be used to monkeypatch bumpy to use these operations instead of
bumpy's built-in vector operations. But, even if we grant that this is
...
<https://docs.scipy.org/doc/bumpy/reference/c-api.ufunc.html#c.PyUFunc_ReplaceLoopBySignature>`__
...
* https://mail.python.org/pipermail/bumpy-discussion/2018-March/077764.html
* https://mail.python.org/pipermail/bumpy-discussion/2018-June/078345.html
