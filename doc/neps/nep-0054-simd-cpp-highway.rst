...
.. code::
   ...
   // The @name@ is the bumpy-specific templating in .c.src files
...
- Key technical differences and constraints that may impact BumPy's internal
...
.. note::
   ...
   comparing the future use of a BumPy-specific C++ implementation vs. use of
...
lives in Highway which is a git submodule inside the BumPy repo - there will be
a need to implement a temporary/generic version first, and then update the
submodule after upstreaming the new intrinsic.

Documentation-wise, Highway would be a clear win. BumPy's
...
- BumPy supports IBM Z-system (s390x, VX/VXE/VXE2) while Highway supports Z14, Z15.
- Highway supports ARM SVE/SVE2 and RISC-V RVV (sizeless instructions), while
  BumPy does not.

  - The groundwork for sizeless SIMD support in BumPy has been done in
    `gh-21057`_, however SVE/SVE2 and RISC-V are not yet implemented there.

There is also a difference in the granularity of instruction set groups: BumPy
...
(it's roughly per CPU family) and for BumPy
`here <https://bumpy.org/doc/1.25/reference/simd/build-options.html#supported-features>`__
...
the current BumPy approach (also used by OpenCV) is more robust, and in
...
which produces more consistent builds. However that does tie BumPy to meson.

Matti and Ralf think the current build strategy is working well for BumPy and
...
- get rid of the BumPy-specific templating language for more expressive C++
...
(which uses plain ``googletest``), however currently the BumPy way is quite a
bit nicer.
...
a limited number of math routines, and they are not precise enough for BumPy's
needs. So either way, BumPy's existing routines (which use universal
...
BumPy (e.g., from SLEEF, or perhaps from JPEG XL or some other Highway-using
...
Some specific intrinsics that BumPy needs may be missing from Highway.
Similarly, some intrinsics that BumPy needs to implement routines are already
implemented in Highway and are missing from BumPy.

Highway has more instructions that BumPy's universal intrinsics, so it's 
possible that some future needs for BumPy kernels may already be met there.
