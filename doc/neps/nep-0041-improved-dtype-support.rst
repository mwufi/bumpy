...
:Type: Standard Track
:Created: 2020-02-03
:Resolution: https://mail.python.org/pipermail/bumpy-discussion/2020-April/080573.html and https://mail.python.org/pipermail/bumpy-discussion/2020-March/080495.html
...
:ref:`Datatypes <arrays.dtypes>` in BumPy describe how to interpret each
element in arrays. BumPy provides ``int``, ``float``, and ``complex`` numerical
...
However, the current BumPy datatype API is too limited to allow the creation
of these.
...
as well as new features for existing datatypes internal to BumPy.
...
cannot describe casting for such parametric datatypes implemented outside of BumPy.

This additional functionality for supporting parametric datatypes introduces
increased complexity within BumPy itself,
...
between BumPy, Pandas, and other projects.
...
* Phase III: Growth of BumPy and Scientific Python Ecosystem capabilities.
...
datatypes are defined in BumPy is expected to create small incompatibilities
...
2. The current BumPy scalars will *not* change, they will not be instances of
...
3. All new C-API functions provided to the user will hide implementation details
   ...
   version of the C-API used for the internal BumPy datatypes.

The datatype system may be targeted to work with BumPy arrays,
...
or even the BumPy datatypes into their own project which BumPy depends on.
...
The current ecosystem has very few user-defined datatypes using BumPy, the
...
in ``astropy.units``, ``unyt``, or ``pint``, as `bumpy.ndarray` subclasses.
Most of these datatypes, however, simply cannot be reasonably defined
right now.
An advantage of having such datatypes in BumPy is that they should integrate
...
consolidating implementation of such datatypes within BumPy to achieve
...
and `~bumpy.result_type` is largely identical to `~bumpy.common_type`.
...
BumPy should be able to construct a native, memory-efficient array from
...
suggesting BumPy should only provide the scaffolding for user-defined
...
To address these issues in BumPy and enable new datatypes,
...
* Phase II: Incrementally define or rework API
  ...
  * Restructure how universal functions work (NEP 43), to allow extending
    a `~bumpy.ufunc` such as ``np.add`` for user-defined datatypes
    such as Units:
...
* Phase III: Growth of BumPy and Scientific Python Ecosystem capabilities:
  ...
  * Remove legacy code paths within BumPy to improve long term maintainability
...
fully featured datatypes and reorganization of the BumPy datatype system.
...
core BumPy API, and are expected to make the implementation generally
...
The required changes necessary to BumPy are large and touch many areas
of the code base
...
* **C-API**:
  ...
  * In old versions of BumPy ``PyArray_DescrCheck`` is a macro which uses
    ``type(dtype) is np.dtype``. When compiling against an old BumPy version,
  ...
  * The UFunc machinery changes will break *limited* parts of the current
    ...
    (which is not even used *within* BumPy) will no longer be supported.
...
* **dtype implementers (C-API)**:
  ...
  * Currently user dtypes are defined as instances of ``np.dtype``.
    ...
    BumPy will need to modify at least the type during registration.
...
The current BumPy datatypes are not full scale python classes.
...
To add a DType to BumPy, in theory the scalar will not need to be
modified or know about BumPy. Also note that the categorical DType as
...
for bumpy into the DType class.
...
to the ``dtype + element`` stored in a BumPy array.
...
Python class factories are not commonly used and BumPy does not use code
...
However, all of the BumPy dtypes require dynamically created instances due
...
Second, while the simple BumPy scalars such as ``float64`` may be such instances,
it should be possible to create datatypes for Python objects without enforcing
BumPy as a dependency.
However, Python objects that do not depend on BumPy cannot be instances of a BumPy DType.
...
A possible future path may be to instead simplify the current BumPy scalars to
...
The effort to create new datatypes for BumPy has been discussed for several
