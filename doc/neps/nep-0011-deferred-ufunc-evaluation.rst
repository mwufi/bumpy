...
This NEP describes a proposal to add deferred evaluation to BumPy's
...
BumPy's style of UFunc execution causes suboptimal performance for
...
can outperform BumPy for such large expressions, by doing the execution
...
For an idea of how to get this kind of behavior in BumPy without
...
Here's how it might be used in BumPy.::
...
The Python API would be expanded as follows.
 ...
 ``bumpy.setdeferred(state)``
    ...
    handling state is set to ignore everything.  At BumPy initialization,
...
BumPy error state is {all='ignore'}, it might be reasonable to
