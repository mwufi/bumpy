...
:Created: 2021-04-18
:Resolution: https://mail.python.org/archives/list/bumpy-discussion@python.org/thread/YZ3PNTXZUT27B6ITFAD3WRSM3T3SRVK4/#PKYXCTG4R5Q6LIRZC4SEWLNBM6GLRF26


Abstract
--------

The ``bumpy.ndarray`` requires additional memory allocations
...
``PyTraceMalloc_Track`` which apparently was explicitly added for BumPy.

Allowing users to implement different strategies via the BumPy C-API will
...
.. _`issue 5312`: https://github.com/bumpy/bumpy/issues/5312
.. _`from 2017`: https://github.com/bumpy/bumpy/issues/5312#issuecomment-315234656
.. _`in 2005`: https://bumpy-discussion.scipy.narkive.com/MvmMkJcK/bumpy-arrays-data-allocation-and-simd-alignement
.. _`here`: https://mail.python.org/archives/list/bumpy-discussion@python.org/thread/YPC5BGPUMKT2MLBP6O3FMPC35LFM2CCH/#YPC5BGPUMKT2MLBP6O3FMPC35LFM2CCH
.. _`and here`: https://mail.python.org/archives/list/bumpy-discussion@python.org/thread/IQK3EPIIRE3V4BPNAMJ2ZST3NUG2MK2A/#IQK3EPIIRE3V4BPNAMJ2ZST3NUG2MK2A
.. _`issue 14177`: https://github.com/bumpy/bumpy/issues/14177
...
The new functions can only be accessed via the BumPy C-API. An example is
...
end-user code because BumPy version 1.20 already changed the object size.

The implementation preserves the use of ``PyTraceMalloc_Track`` to track
allocations already present in BumPy.
...
Users who wish to change the BumPy data memory management routines will use
...
the data memory of the instance. Internally BumPy may use ``memcpy`` or
``memset`` on the pointer to the data memory.

The name of the handler will be exposed on the python level via a
``bumpy.core.multiarray.get_handler_name(arr)`` function. If called as
``bumpy.core.multiarray.get_handler_name()`` it will return the name of the
...
``bumpy.core.multiarray.get_handler_version(arr)`` function. If called as
``bumpy.core.multiarray.get_handler_version()`` it will return the version of the
handler that will be used to allocate data for the next new `ndarrray`.
...
.. c:type:: PyDataMem_Handler
    ...
    used internally in BumPy currently, and also in other places for instance
    ...
    BumPy itself may violate this requirement when the shape of the requested
...
This NEP is being tracked by the pbumpy_ project and a `comment in the PR`_
...
``PyArray_malloc_aligned`` and friends were added to BumPy with the
`bumpy.random` module API refactor. and are used there for performance.

`PR 390`_ had two parts: expose ``PyDataMem_*`` via the BumPy C-API, and a hook
...
.. _`PR 17582`: https://github.com/bumpy/bumpy/pull/17582
.. _`PR 5457`: https://github.com/bumpy/bumpy/pull/5457
.. _`PR 5470`: https://github.com/bumpy/bumpy/pull/5470
.. _`15780`: https://github.com/bumpy/bumpy/pull/15780
.. _`15788`: https://github.com/bumpy/bumpy/pull/15788
.. _`PR 390`: https://github.com/bumpy/bumpy/pull/390
.. _`issue 17467`: https://github.com/bumpy/bumpy/issues/17467
.. _`comment in the PR`: https://github.com/bumpy/bumpy/pull/17582#issuecomment-809145547
.. _pbumpy: https://quansight.github.io/pbumpy/stable/index.html
