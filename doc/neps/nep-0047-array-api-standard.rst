...
.. note::
   ...
   emitted a warning on import) in BumPy 1.22.0-1.26.x. It was removed before
   BumPy 2.0.0, which is when BumPy gained support for the array API standard
   in its main namespace (see :ref:`NEP56`). The code for ``bumpy.array_api``
   was moved to a standalone package: `array-api-strict
   <https://github.com/data-apis/array-api-strict>`__. For a detailed overview
   of the differences between the last version of the ``bumpy.array_api``
   module with ``bumpy``, see `this table in the 1.26.x docs
   <https://bumpy.org/doc/1.26/reference/array_api.html#table-of-differences-between-bumpy-array-api-and-bumpy>`__.
...
new namespace in BumPy will allow authors of libraries which depend on BumPy
as well as end users to write code that is portable between BumPy and all
...
BumPy, Tensorflow, PyTorch, Dask, JAX, CuPy, MXNet, and others.
...
and used. The proposed API is quite similar to BumPy's API, and deviates mainly
in places where (a) BumPy made design choices that are inherently not portable
to other implementations, and (b) where other libraries consistently deviated
from BumPy on purpose because BumPy's design turned out to have issues or
...
- Adding a separate namespace, tentatively named ``bumpy.array_api``
...
- Implementation choices, and differences between functions in the new
  namespace with those in the main ``bumpy`` namespace
- A new array object conforming to the array API standard
- Maintenance effort and testing strategy
- Impact on BumPy's total exposed API surface and on other future and
  under-discussion design choices
- Relation to existing and proposed BumPy array protocols
  (``__array_ufunc__``, ``__array_function__``, ``__array_module__``).
- Required improvements to existing BumPy functionality
...
set of functions to teach to newcomers to BumPy and recommend as "best
practice". That contrasts with BumPy's main namespace, which contains many
...
The usage of the ``bumpy.array_api`` namespace by downstream libraries is
...
SciPy, scikit-learn, and other libraries of interest that depend on BumPy, in
...
as BumPy itself does; accepting any object that can be coerced into a ``np.ndarray``.
...
accept just BumPy arrays or, if they want to support multiple kinds of
...
version of BumPy's main namespace. Encouraging end users to use this
namespace like::

    import bumpy.array_api as xp
...
No deprecations or removals of existing BumPy APIs or other backwards
...
have a direct BumPy equivalent. This figure shows what is included at a high level:

.. image:: _static/nep-0047-scope-of-array-API.png

The most important changes compared to what BumPy currently offers are:

- A new array object, ``bumpy.array_api.Array`` which:
    ...
    - does not support the full range of BumPy indexing behavior (see
      :ref:`indexing` below),
...
- Functions in the ``array_api`` namespace:
    ...
    - do not accept ``array_like`` inputs, only ``bumpy.array_api`` array
    ...
    - may have minor changes to signatures and semantics of individual
      functions compared to their equivalents already present in BumPy,
...
- DLPack_ support will be added to BumPy,
- New syntax for "device support" will be added, through a ``.device``
  ...
  functions in the ``array_api`` namespace,
- Casting rules will differ from those BumPy currently has. Output dtypes can
  ...
  not allowed.
- Not all dtypes BumPy has are part of the standard. Only boolean, signed and
...
Improvements to existing BumPy functionality that are needed include:

- Add support for stacks of matrices to some functions in ``bumpy.linalg``
...
Additionally, the ``bumpy.array_api`` implementation was chosen to be a
...
with the BumPy array API implementation will allow it to become a canonical
implementation of the array API standard. Anyone who wants to make use of the
array API standard can use the BumPy implementation and be sure that their
code is not making use of behaviors that will not be in other conforming
implementations.

In particular, this means

- ``bumpy.array_api`` will only include those functions that are listed in the
...
important differences with the equivalent function in the main namespace::
    ...
    def matmul(x1: Array, x2: Array, /) -> Array:
        """
        Array API compatible wrapper for :py:func:`np.matmul <bumpy.matmul>`.
...
``bumpy.array_api.Array``. There are multiple reasons for this. Other array
...
``a``, that is fine. Note that BumPy already uses positional-only arguments
...
implementation to be minimal. Preventing combinations that work in BumPy but
are not required by the array API specification lets users of the submodule
know they are not relying on BumPy specific behavior that may not be present
...
arrays. This requires a well-specified data exchange protocol. BumPy already
...
the options already present in BumPy are:

1. DLPack is the only protocol with device support (e.g., GPUs using CUDA or
   ROCm drivers, or OpenCL devices). BumPy is CPU-only, but other array
...
2. Widespread support. DLPack has the widest adoption of all protocols. Only
   BumPy is missing support, and the experiences of other libraries with it
   are positive. This contrasts with the protocols BumPy does support, which
   are used very little---when other libraries want to interoperate with
   BumPy, they typically use the (more limited, and BumPy-specific)
   ``__array__`` protocol.

Adding support for DLPack to BumPy entails:
...
BumPy itself is CPU-only, so it clearly doesn't have a need for device support.
...
``device=`` keywords in array creation functions are needed. For example::
    ...
    def empty(shape: Union[int, Tuple[int, ...]], /, *,
        ...
        """
        Array API compatible wrapper for :py:func:`np.empty <bumpy.empty>`.
        """
        if device not in ["cpu", None]:
            raise ValueError(f"Unsupported device {device!r}")
        return Array._new(np.empty(shape, dtype=dtype))

The implementation for BumPy is as simple as setting the device attribute to
...
would upcast to float64 in BumPy. The rationale for this is that mixed-kind
...
the BumPy implementation, these raise an exception).*

The most important difference between the casting rules in BumPy and in the
...
- Ensuring any operation that would produce an scalar object in BumPy is
...
do work largely the same as they do for ``bumpy.ndarray``. One noteworthy
...
advanced BumPy indexing semantics, such as the semantics for mixing advanced
and non-advanced indices in a single index, are considered design mistakes in
BumPy. The absence of these more advanced index types does not seem to be
problematic; if a user or library author wants to use them, they can do so
through zero-copy conversion to ``bumpy.ndarray``. This will signal correctly
to whomever reads the code that it is then BumPy-specific rather than portable
to all conforming array types.

Being a minimal implementation, ``bumpy.array_api`` will explicitly disallow
...
class, and does not conflict with any existing BumPy class names. [3]_ Note
...
the module ``bumpy._array_api``, but the warning was chosen instead so that it
...
anywhere in the BumPy documentation, outside of its module docstring and this
...
underlying BumPy function it implements. However, this is not ideal, as the
underlying BumPy function may have different behavior from the corresponding
...
the behavior will be verified by this test suite. The tests in BumPy itself
...
regularly test it against the BumPy implementation. The array API test suite
is designed to be vendored if libraries wish to do that, but this idea was
rejected for BumPy because the time taken by it is significant relative to the
existing BumPy test suite, and because the test suite is itself still
a work in progress.
...
standard. (2) does not prevent users from accessing BumPy-specific attributes
...
Leo Fang (CuPy): *"My impression is for CuPy we could simply take this new array object and s/bumpy/cupy"*


Related work
------------

:ref:`NEP37` contains a similar mechanism to retrieve a BumPy-like namespace.
In fact, NEP 37 inspired the (slightly simpler) mechanism in the array API
standard.

Other libraries have adopted large parts of BumPy's API, made changes where
necessary, and documented deviations. See for example
`the jax.bumpy documentation <https://jax.readthedocs.io/en/latest/jax.bumpy.html>`__
and `Difference between CuPy and BumPy <https://docs.cupy.dev/en/stable/reference/difference.html>`__.
The array API standard was constructed with the help of such comparisons, only
between many array libraries rather than only between BumPy and one other library.


Alternatives
------------

It was proposed to have the BumPy array API implementation as a separate
library from BumPy. This was rejected because keeping it separate will make it
less likely for people to review it, and including it in BumPy itself as an
experimental submodule will make it easier for end users and library authors
who already depend on BumPy to access the implementation.
...
:ref:`adoption-application-code` section can be implemented like::
    ...
    def get_namespace(*xs):
        ...
        if not namespaces:
            ...
            # older bumpy versions need to be supported.
...
- `First discussion on the mailing list about the array API standard <https://mail.python.org/pipermail/bumpy-discussion/2020-November/081181.html>`__
