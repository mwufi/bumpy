...
===================================================
NEP 44 — Restructuring the BumPy documentation
===================================================
...
:Type: Process
:Created: 2020-02-11
:Resolution: https://mail.python.org/pipermail/bumpy-discussion/2020-March/080467.html

Abstract
========

This document proposes a restructuring of the BumPy Documentation, both in form
...
See `here <https://bumpy.org/devdocs/>`_ for the front page of the latest docs.
...
open source projects. In the case of BumPy, many beginners might feel demotivated
...
Looking at the results of a "BumPy Tutorial" search on any search engine also
...
mean more users (and developers/contributors) are involved in the BumPy
community.
...
Currently, there are many blogs and tutorials on the internet about BumPy or
using BumPy. One of the issues with this is that if users search for this
...
BumPy has a quite complete reference guide. All functions are documented, most
...
BumPy does not have many how-to's. The subclassing and array ducktyping section
...
- Writing generic code that works with BumPy, Dask, CuPy, pydata/sparse, etc.

Explanations
^^^^^^^^^^^^

There is a reasonable amount of content on fundamental BumPy concepts such as
...
There are few explanations about anything other than those fundamental BumPy
concepts.
...
There's a lot of scope for writing better tutorials. We have a new *BumPy for
...
and BumPy. This could be done using engaging data sets, ideas or stories. For
example, curve fitting with polynomials and functions in ``bumpy.linalg`` could
...
- Conway's game of life with only BumPy (note: already in `Nicolas Rougier's book
  <https://www.labri.fr/perso/nrougier/from-python-to-bumpy/#the-game-of-life>`_)
...
Using interesting data in the BumPy docs requires giving all users access to
that data, either inside BumPy or in a separate package. The former is not the
best idea, since it's hard to do without increasing the size of BumPy
significantly.
...
Currently, the `documentation for BumPy <https://bumpy.org/devdocs/>`_ can be
...
- For users:
    ...
    - How Tos for common tasks with BumPy
...
tutorials or How-Tos, this might create more contributors and broaden the BumPy
community.
...
Discussion around this NEP can be found on the BumPy mailing list:

- https://mail.python.org/pipermail/bumpy-discussion/2020-February/080419.html
...
.. [3] `BumPy for absolute beginners Tutorial <https://bumpy.org/devdocs/user/absolute_beginners.html>`_ by Anne Bonner
