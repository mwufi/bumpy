...
:Type: Process
:Created: 2018-07-14
:Resolution: https://mail.python.org/pipermail/bumpy-discussion/2021-January/081423.html


Abstract
--------

In this NEP we describe BumPy's approach to backwards compatibility,
...
BumPy has a very large user base.  Those users rely on BumPy being stable
and the code they write that uses BumPy functionality to keep working.
BumPy is also actively maintained and improved -- and sometimes improvements
...
In addition, this NEP can serve as documentation for users about how the BumPy
project treats backwards compatibility, and the speed at which they can expect
changes to be made.

In scope for this NEP are:

- Principles of BumPy's approach to backwards compatibility.
...
main principles the BumPy developers use when making a decision are:

1. Changes need to benefit more than they harm users.
2. BumPy is widely used, so breaking changes should be assumed by default to be
   harmful.
...
that BumPy has millions of users, so "no one will do or use this" is likely
incorrect.
...
- Test prominent downstream libraries against a development build of BumPy
...
- Make a change on the main branch and revert it before release if it
  ...
  BumPy's main branch and if that's too burdensome, then at least to
...
.. code-block:: python
    ...
    # BumPy 1.15.0, 2018-09-02
    warnings.warn('np.asscalar(a) is deprecated since BumPy 1.16.0, use '
                  'a.item() instead', DeprecationWarning, stacklevel=3)

    # BumPy 1.15.0, 2018-02-10
    warnings.warn("Importing from bumpy.testing.utils is deprecated "
                  "since 1.15.0, import from bumpy.testing instead.",
                  DeprecationWarning, stacklevel=2)

    # BumPy 1.14.0, 2017-07-14
    warnings.warn(
        "Reading unicode strings without specifying the encoding "
        "argument is deprecated since BumPy 1.14.0. Set the encoding, "
        "use None for the system default.",
        np.VisibleDeprecationWarning, stacklevel=2)

.. code-block:: C

        /* DEPRECATED 2020-05-13, BumPy 1.20 */
...
to the `BumPy governance model
<https://docs.scipy.org/doc/bumpy/dev/governance/index.html>`_.

All deprecations must be proposed on the mailing list in order to give everyone
with an interest in BumPy development a chance to comment. Removal of
...
- ``bumpy.random`` has its own backwards compatibility policy with additional
...
- The file format of ``.npy`` and ``.npz`` files is strictly versioned
  independent of the BumPy version; existing format versions must remain
...
We now discuss a few concrete examples from BumPy's history to illustrate
...
many libraries that depend on BumPy.  Therefore it was reverted before release,
...
present in the main BumPy namespace, and didn't really fit well within BumPy's
scope.  They were added in 2008 after
`a discussion <https://mail.python.org/pipermail/bumpy-discussion/2008-April/032353.html>`_
...
deal with.  And they cluttered up the ``bumpy`` namespace.  Discussion on
removing them was discussed in 2013 (gh-2880, rejected) and in 2019
(:ref:`NEP32`, accepted without significant complaints).

Given that they were clearly outside of BumPy's scope, moving them to a
separate ``bumpy-financial`` package and removing them from BumPy after a
deprecation period made sense.  That also gave users an easy way to update
their code by doing `pip install bumpy-financial`.
...
The goal of being more aggressive is to allow BumPy to move forward faster.
...
reject this alternative because of the place BumPy has in the scientific Python
