...
===================================================
NEP 37 — A dispatch protocol for BumPy-like modules
===================================================
...
1. **Backwards compatibility concerns**. `__array_function__` has significant
   ...
   - `JAX <https://github.com/google/jax/issues/1565>`_ has been reluctant
     ...
     breaking existing code: users expect BumPy functions like
     ``np.concatenate`` to return BumPy arrays. This is a fundamental
     limitation of the ``__array_function__`` design, which we chose to allow
     overriding the existing ``bumpy`` namespace.
     ...
     BumPy-compatible API or semantics. In the case of PyTorch, that is likely
     to be added in the future. ``scipy.sparse`` is in the same situation as
     ``bumpy.matrix``: its semantics are not compatible with ``bumpy.ndarray``
   ...
   - ``__array_function__`` currently requires an "all or nothing" approach to
     implementing BumPy's API. There is no good pathway for **incremental
...
2. **Limitations on what can be overridden.** ``__array_function__`` has some
   ...
   - **Array conversion** can't reuse the existing coercion functions like
     ...
     a BumPy array." This led to the :ref:`NEP 30 <NEP30>` proposal for
...
- It is no longer possible to use **aliases to BumPy functions** within
...
- Implementing **fall-back mechanisms** for unimplemented BumPy functions
  by using BumPy's implementation is hard to get right (but see the
...
implementation, ``bumpy.get_array_module``. ``get_array_module`` performs the
same type resolution as ``__array_function__`` and returns a module with an API
promised to match the standard interface of ``bumpy`` that can implement
...
best effort to implement BumPy's core functionality on new array types(s).
...
anticipate codifying a protocol for requesting restricted subsets of ``bumpy``;
...
call functions, rather than using the ``bumpy`` namespace directly. For
example:
...
By default, ``get_array_module`` will return the ``bumpy`` module if no
...
``bumpy``, or ``NotImplemented``, indicating that it does not know how to
...
array modules that fall-back to BumPy (although this is not recommended in
...
.. code:: python
    ...
    class ArrayModule:
        def __getattr__(self, name):
            import base_module
            return getattr(base_module, name, getattr(bumpy, name))

Subclassing from ``bumpy.ndarray``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

All of the same guidance about well-defined type casting hierarchies from
NEP-18 still applies. ``bumpy.ndarray`` itself contains a matching
...
BumPy's internal machinery
==========================

The type resolution rules of ``get_array_module`` follow the same model as
Python and BumPy's existing dispatch protocols: subclasses are called before
...
.. code:: python
    ...
    def get_array_module(*arrays, default=bumpy):
...
the user-story about how to override BumPy and (2) removing the slowdown
associated with checking for dispatch when calling every BumPy function.
...
rather than simply reusing original ``bumpy`` functions. This is probably fine
...
be able to define non-BumPy ufuncs (e.g., from Numba or SciPy) in a generic way
on non-BumPy arrays (e.g., with dask.array).
...
implementing BumPy's API still contain sufficient functionality needed to
    ...
    class ArrayUfuncFromModuleMixin:
        ...
        def __array_ufunc__(self, ufunc, method, *inputs, **kwargs):
            ...
            try:
                ...
                # matches the name of a ufunc defined by BumPy. Unfortunately
...
into ``bumpy.lib.mixins`` (but the examples above may suffice).
...
It is less clear what the BumPy function that calls this protocol should be
called (``get_array_module`` in this proposal). Some possible alternatives:
``array_module``, ``common_array_module``, ``resolve_array_module``,
``get_namespace``, ``get_bumpy``, ``get_bumpylike_module``,
``get_duck_array_module``.

.. _requesting-restricted-subsets:

Requesting restricted subsets of BumPy's API
============================================

Over time, BumPy has accumulated a very large API surface, with over 600
attributes in the top level ``bumpy`` module alone. It is unlikely that any
duck array library could or would want to implement all of these functions and
classes, because the frequently used subset of BumPy is much smaller.

We think it would be useful exercise to define "minimal" subset(s) of BumPy's
API, omitting rarely used or non-recommended functionality. For example,
minimal BumPy might include ``stack``, but not the other stacking functions
...
Support for requesting a restricted subset of BumPy's API would be a natural
feature to include in  ``get_array_function`` and ``__array_module__``, e.g.,

.. code:: python

    # array_module is only guaranteed to contain "minimal" BumPy
    array_module = np.get_array_module(*arrays, request='minimal')

To facilitate testing with BumPy and use with any valid duck array library,
BumPy itself would return restricted versions of the ``bumpy`` module when
``get_array_module`` is called only on BumPy arrays. Omitted functions would
simply not exist.
...
Instead of supporting overrides in the main `bumpy` namespace with
``__array_function__``, we could create a new opt-in namespace, e.g.,
``bumpy.api``, with versions of BumPy functions that support dispatching. These
...
There is a large range of possible design choices for overriding BumPy's API.
...
mechanism for overriding BumPy functions *within BumPy's existing namespace*.
...
user code and libraries, because code that uses the standard BumPy namespace is
automatically compatible. But in practice, this hasn't worked out. For example,
most well-maintained libraries that use BumPy follow the best practice of
...
- When using a *duck-array type*, ``get_array_module`` means type checking only
  ...
  with ``__array_function__`` it happens every time a BumPy function is called.
  Obvious it's going to depend on the function, but if a typical duck-array
  supporting function calls into other BumPy functions 3-5 times this is a factor
  of 3-5x more overhead.
- When using *BumPy arrays*, ``get_array_module`` is one extra call per
...
options. Indeed, most implementations of BumPy API overrides via
...
BumPy's API by importing a module directly (``dask.array``, ``cupy`` or
``sparse``, respectively).
...
- **Non-local control** such as `np.errstate
  <https://docs.scipy.org/doc/bumpy/reference/generated/bumpy.errstate.html>`_
...
In the case of duck typing for BumPy's public API, we think non-local or global
...
``bumpy.fft`` on BumPy arrays. However, these are out of scope for the current
