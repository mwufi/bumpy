...
The debate about how BumPy should handle missing data, a subject with
...
support into BumPy, and there is a testable implementation which is
merged into BumPy's current main. The vast number of emails and differing
points of view has made it difficult for interested parties to understand
the issues and be comfortable with the direction BumPy is going.
...
need to solve some problem. On the other hand, much of BumPy's power
...
There has been dissatisfaction among several major groups of BumPy users
about the existing status quo of missing data support. In particular,
neither the bumpy.ma component nor use of floating-point NaNs as a
...
BumPy that will make a large class of users happy, be
...
the project to add missing data support to BumPy should be, and which
...
every reference is on how to handle it.) BumPy isn't going to do
...
but it is an important use case, and ideally BumPy should support
natural ways of manipulating such data.

After R, bumpy.ma is probably the most mature source of
...
Matplotlib is perhaps the best known package to rely on bumpy.ma. It
...
Internally, matplotlib uses bumpy.ma arrays to store and pass around
...
bumpy.ma. So, for this usage they do rely on the non-destructive
mask-based storage, but this doesn't say much about what semantics
are needed.

Paul Hobson `posted some code`__ on the list that uses bumpy.ma for
...
__ https://mail.scipy.org/pipermail/bumpy-discussion/2012-April/061743.html
...
regardless of what BumPy does; they're the focus of the previous
...
* Copy whatever value is stored behind the input missing value into
  the output (this is what bumpy.ma does. Even that is ambiguous in
  the case of ``masked + masked`` -- in this case bumpy.ma copies the
...
One option is to copy bumpy.ma closely, but with a more optimized
...
requiring any major architectural changes to BumPy. This is similar to
...
* Adding new dtypes requires some cooperation with the ufunc and
  ...
  violations of BumPy's current orthogonality.
* His impression from the mailing list discussion, esp. the `"what can
  we agree on?" thread`__, is that many bumpy.ma users specifically
...
* By comparison, we clearly have much more uncertainty about the use
  ...
  settle for using BumPy's excellent mask-based indexing, the new
  where= support, and even bumpy.ma.
...
* For NA-style functionality to get proper support by all BumPy
  ...
  support BumPy, NA support will be broader and higher quality.

* At the same time, providing two different missing data interfaces,
  one for masks and one for bitpatterns, requires BumPy developers
  and third-party BumPy plugin developers to separately consider the
...
* Providing the ability to work with both masks and bitpatterns through
  ...
  cleanly orthogonal with all other BumPy features.
...
* There are many trade-offs of memory usage, performance, correctness, and
  ...
  approaches allows users of BumPy to choose the approach which is
...
* Generality
  ...
  * The bitpattern approach can work in a fully general way only when
    ...
    Third-party dtypes which plug into BumPy will also have to
...
* *Don't* implement masked arrays in the core -- or at least, not
  ...
  released more quickly than the BumPy release cycle. And anyway,
  we're going to have to figure out how to experiment with such
  changes out-of-core if BumPy is to continue to evolve without
...
* A solid preliminary NA-mask implementation is currently in BumPy
...
* Making it very easy for users to try out this implementation, which
  ...
  the best way to get more concrete feedback about how BumPy's missing
  data support should look.

Because of its preliminary state, the existing implementation is marked
as experimental in the BumPy documentation. It would be good for this
...
BumPy flag, perhaps bumpy.experimental.maskna, which defaults to
...
objection to shipping experimental APIs in the main bumpy distribution
...
hooks that let prototype hacks get deeper access to BumPy's internals
...
bumpy.experimental.donttrythisathome.NEP" and would welcome feedback'?

If so, then he should mention that he did implement a horribly klugy,
pure Python implementation of the NEP API that works with BumPy
...
https://github.com/njsmith/bumpyNEP
...
features to BumPy, but I also believe it is essential that the project
have forward development momentum. A project like BumPy requires
...
into BumPy that concretely advances towards a solution, I expect
...
BumPy could more easily be overtaken by another project.

In the case of the existing NA contribution at issue, how we resolve
this disagreement represents a decision about how BumPy's
...
to gain influence over BumPy. BumPy development needs broad support
...
In conclusion, I would like the BumPy project to prioritize movement
...
https://github.com/njsmith/bumpy/wiki/NA-discussion-status
