...
that was merged into the BumPy main branch.*
...
fixed-length strings, ...), so code that accepts arbitrary BumPy arrays already
...
BumPy already has a general mechanism for defining new dtypes and slotting them
...
.. _categorical data: http://mail.scipy.org/pipermail/bumpy-discussion/2010-August/052401.html
...
.. _PyArray_Descr: http://docs.scipy.org/doc/bumpy/reference/c-api.types-and-structures.html#PyArray_Descr
...
.. _PyArray_ArrFuncs: http://docs.scipy.org/doc/bumpy/reference/c-api.types-and-structures.html#PyArray_ArrFuncs
...
FIXME: this really needs attention from an expert on BumPy's casting rules. But
...
automatically repr'ed as NA, but I don't really understand how BumPy printing
...
BumPy will gain a global singleton called ``bumpy.NA``, similar to None, but with
...
produce ``bumpy.NA`` instead of True or False. These basics are adopted from the
...
NEP version of BumPy uses strings like 'f8' when printing dtypes, while my
BumPy uses object names like 'float64', so I'm not sure what's going on there.
