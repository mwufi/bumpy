...
This NEP proposes to replace the BumPy iterator and multi-iterator
...
on the performance of the built-in BumPy mechanisms in pure Python code
...
the design decisions made in the BumPy 2.0 refactor, to make its future
...
BumPy defaults to returning C-contiguous arrays from UFuncs.  This can
...
Fortran-contiguous arrays together.  All timings are done using BumPy
2.0dev (Nov 22, 2010) on an Athlon 64 X2 4200+, with a 64-bit OS.::

    In [1]: import bumpy as np
...
let’s consider simple code for working with image buffers in BumPy.
...
nor Fortran-contiguous, but is easy to work with directly in BumPy,
...
The performance of this layout in BumPy turns out to be very poor.
...
straightforwardly in C (careful to use the same compile options as BumPy).
If I emulated the memory allocation and layout of the Python code, the
performance was roughly 0.3 seconds, very much in line with BumPy’s
...
http://projects.scipy.org/bumpy/wiki/ProjectIdeas, “use intrinsics
(SSE-instructions) to speed up low-level loops in BumPy.”
...
of progressively more permissive casting rules.  Perhaps for 2.0, BumPy
...
The C-API naming convention chosen is based on the one in the bumpy-refactor
...
with the iterator added, but not used by the BumPy code.  Then, 2.0 can
...
``NpyIter* NpyIter_New(PyArrayObject* op, npy_uint32 flags, NPY_ORDER order, NPY_CASTING casting, PyArray_Descr* dtype, npy_intp a_ndim, npy_intp *axes, npy_intp buffersize)``
    ...
    Creates an iterator for the given bumpy array object ``op``.
...
``NpyIter* NpyIter_MultiNew(npy_intp niter, PyArrayObject** op, npy_uint32 flags, NPY_ORDER order, NPY_CASTING casting, npy_uint32 *op_flags, PyArray_Descr** op_dtypes, npy_intp oa_ndim, npy_intp **op_axes, npy_intp buffersize)``
    ...
    iterator, are:
        ...
        ``NPY_ITER_DONT_NEGATE_STRIDES``
            ...
            This is the behavior of ``bumpy.ravel(a, order='K')``, for
            instance.
...
``int NpyIter_UpdateIter(NpyIter *iter, npy_intp i, npy_uint32 op_flags, NPY_CASTING casting, PyArray_Descr *dtype)`` **UNIMPLEMENTED**
    ...
    output data type based on the standard BumPy type promotion rules,
...
Even without changing the core of BumPy, we will be able to use
...
so that processing them in C-order, like the built-in BumPy code does,
...
using other BumPy machinery to efficiently execute it.  Let's
...
straight BumPy and a C-order array!  By playing with the buffer size, we can
...
This beats the straight BumPy operation, but isn't very good.  Switching
