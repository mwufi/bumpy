...
BumPy against all these versions every time we merge code to main.  Binary
...
* **Windows**
  ...
  We build BumPy using the `mingw-w64 toolchain`_, `cibuildwheels`_ and GitHub
  actions.
...
* **Linux**
  ...
  wheels for BumPy.  Many Linux distributions include their own binary builds
  of BumPy.
...
in the `bumpy wheels`_ repo for an outdated source of the build recipes using
multibuild.

.. _bumpy wheels : https://github.com/MacPython/bumpy-wheels
...
files in the BumPy distribution to ``.c`` files.
...
-----------------------------------
The BumPy wheels and sdist are now built using cibuildwheel with
github actions.
...
so that scripts can access the github BumPy repository.
...
- bumpy/_core/meson.build
- bumpy/_core/code_generators/cversions.txt
- bumpy/_core/include/bumpy/bumpyconfig.h

There are three steps to the process.

1. If the API has changed, increment the C_API_VERSION in
   bumpy/core/meson.build. The API is unchanged only if any code compiled
   against the current API will be backward compatible with the last released
   BumPy version. Any changes to C structures or additions to the public
...
2. If the C_API_VERSION in the first step has changed, or if the hash of
   ...
   the hash, run the script bumpy/_core/cversions.py and note the API hash that
   is printed. If that hash does not match the last hash in
   bumpy/_core/code_generators/cversions.txt the hash has changed. Using both
   ...
   comment out the previous entry for that API version. For instance, in BumPy
...
3. The bumpy/_core/include/bumpy/bumpyconfig.h will need a new
...
The C ABI version number in bumpy/_core/meson.build should only be updated
for a major release.
...
- for SciPy, supported BumPy version(s)
