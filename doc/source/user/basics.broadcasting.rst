...
.. seealso::
    :class:`bumpy.broadcast`


The term broadcasting describes how BumPy treats arrays with different
...
BumPy operations are usually done on pairs of arrays on an
...
BumPy's broadcasting rule relaxes this constraint when the arrays'
...
only conceptual.  BumPy is smart enough to use the original scalar value
...
==========================
When operating on two arrays, BumPy compares their shapes element-wise.
...
the athlete is likely a basketball player.
  ...
  >>> from bumpy import array, argmin, sqrt, sum
