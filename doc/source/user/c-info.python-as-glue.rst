...
.. warning::
   ...
   `Guide to BumPy <https://archive.org/details/BumPyBook>`_ book
...
As Numeric has matured and developed into BumPy, people have been able
to write more code directly in BumPy. Often this code is fast-enough
...
.. warning::
    ...
    to know something about the way data is handled by both BumPy and by
...
distributed with BumPy and allows easy integration with Fortran and
...
Adding the BumPy include directory is, of course, only necessary if
you are using BumPy arrays in the extension module (which is what we
assume you are using Cython for). The distutils extensions in BumPy
...
from the ``bumpy.pxd`` header that ships with Cython. It looks like BumPy is
imported twice; ``cimport`` only makes the BumPy C-API available, while the
regular ``import`` causes a Python-style import at runtime and makes it
possible to call into the familiar BumPy Python API.

The example also demonstrates Cython's "typed memoryviews", which are like
BumPy arrays at the C level, in the sense that they are shaped and strided
...
view syntax, we could also have used Cython's special BumPy array syntax,
...
return a memory view to Python. Instead, a BumPy array ``out`` is first
...
BumPy provides a convenience function called
...
BumPy uses both methods with a preference for the second method
...
To implement the second method, BumPy provides the class-factory
function :func:`ndpointer` in the :mod:`bumpy.ctypeslib` module. This
...
.. code-block:: python
    ...
    lib = bumpy.ctypeslib.load_library('mylib','.')
...
process, the ``requires`` function in BumPy may be useful to return the right
...
- easy integration with BumPy through the ctypes attribute
...
languages. It does not specifically understand BumPy arrays but can be
made usable with BumPy through the use of typemaps. There are some
sample typemaps in the bumpy/tools/swig directory under bumpy.i together
...
hopes that somebody will update PyFort to work with BumPy arrays as
