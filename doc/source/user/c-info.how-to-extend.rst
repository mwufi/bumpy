...
that bumpy includes f2py so that an easy-to-use mechanisms for linking
...
In addition to the Python C-API, there is a full and rich C-API for BumPy
...
rule. There are several converter functions defined in the BumPy C-API
...
throughout the BumPy source code. The standard usage is like this:
...
Most extension modules for BumPy will need to access the memory for an
ndarray object (or one of it's sub-classes). The easiest way to do
this doesn't require you to know much about the internals of BumPy.
The method is to
...
:c:func:`PyArray_FROM_OTF`
    ...
    *obj*
        ...
        an :obj:`~bumpy.class.__array__` method (which should return an ndarray),
