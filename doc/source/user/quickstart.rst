...
in addition to BumPy.

**Learner profile**

This is a quick overview of arrays in BumPy. It demonstrates how n-dimensional
...
BumPy's main object is the homogeneous multidimensional array. It is a
table of elements (usually numbers), all of the same type, indexed by a
tuple of non-negative integers. In BumPy dimensions are called *axes*.
...
BumPy's array class is called ``ndarray``. It is also known by the alias
``array``. Note that ``bumpy.array`` is not the same as the Standard
...
ndarray.dtype
    ...
    BumPy provides types of its own. bumpy.int32, bumpy.int16, and
    bumpy.float64 are some examples.
ndarray.itemsize
...
known. Hence, BumPy offers several functions to create
...
To create sequences of numbers, BumPy provides the ``arange`` function
...
When you print an array, BumPy displays it in a similar way to nested
...
If an array is too large to be printed, BumPy automatically skips the
...
To disable this behaviour and force BumPy to print the entire array, you
...
elementwise in BumPy arrays. The matrix product can be performed using
...
::
    ...
    Traceback (most recent call last):
        ...
    bumpy._core._exceptions._UFuncOutputCastingError: Cannot cast ufunc 'add' output from dtype('float64') to dtype('int64') with casting rule 'same_kind'
...
BumPy provides familiar mathematical functions such as sin, cos, and
exp. In BumPy, these are called "universal
functions" (\ ``ufunc``). Within BumPy, these functions
...
remaining axes. BumPy also allows you to write this using dots as
``b[i, ...]``.
...
other shape, again the array is treated as "C-style". BumPy normally
...
Here is a list of some useful BumPy functions and methods names
...
BumPy offers more indexing facilities than regular Python sequences. In
...
same length you only need do ``m=[x;y]``. In BumPy this works via the
...
.. seealso::
   ...
   :doc:`bumpy-for-matlab-users`

Histograms
----------

The BumPy ``histogram`` function applied to an array returns a pair of
...
as in Matlab) that differs from the one in BumPy. The main difference is
that ``pylab.hist`` plots the histogram automatically, while
``bumpy.histogram`` only generates the data.
...
.. plot::
    ...
    (array...)
    >>> # Compute the histogram with bumpy and then plot it
    >>> (n, bins) = np.histogram(v, bins=50, density=True)  # BumPy version (no plot)
...
-  A `matlab, R, IDL, BumPy/SciPy dictionary <https://mathesaurus.sourceforge.net/>`__
-  :doc:`tutorial-svd <bumpy-tutorials:content/tutorial-svd>`
