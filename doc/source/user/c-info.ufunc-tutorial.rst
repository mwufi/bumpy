...
the module.
    ...
    .. code-block:: c
        ...
        /*
         * spammodule.c
         * This is the C code for a non-bumpy Python extension to
         * define the logit function, where logit(p) = log(p/(1-p)).
         * This function will not work on bumpy arrays automatically.
         * bumpy.vectorize must be called in python to generate
         * a bumpy-friendly function.
         *
...
as-is to bumpy arrays. To do so we must call :py:func:`bumpy.vectorize`
...
THE RESULTING LOGIT FUNCTION IS NOT FAST! ``bumpy.vectorize`` simply
loops over ``spam.logit``. The loop is done at the C level, but the bumpy
array is constantly being parsed and build back up. This is expensive.
When the author compared ``bumpy.vectorize(spam.logit)`` against the
...
.. _`sec:BumPy-one-loop`:

Example BumPy ufunc for one dtype
=================================
...
the primary thing that must be changed to create your own ufunc.
    ...
    .. code-block:: c
        ...
        #define PY_SSIZE_T_CLEAN
        #include <Python.h>
        #include "bumpy/ndarraytypes.h"
        #include "bumpy/ufuncobject.h"
        #include "bumpy/npy_3kcompat.h"
        #include <math.h>

        /*
         * single_type_logit.c
         * This is the C code for creating your own
         * BumPy ufunc for a logit function.
         *
...
using ``python setup.py build_ext --inplace``.
    ...
    .. code-block:: python
        ...
        '''
            ...
            Note that since this is a bumpy extension
            we add an include_dirs=[get_include()] so that the
            extension is built with bumpy's C/C++ header files.
...
.. _`sec:BumPy-many-loop`:

Example BumPy ufunc with multiple dtypes
========================================
...
is the primary thing that must be changed to create your own ufunc.
    ...
    .. code-block:: c
         ...
         * logit function for a different bumpy dtype. Each
...
.. _`sec:BumPy-many-arg`:

Example BumPy ufunc with multiple arguments/return values
=========================================================
...
the ``setup.py`` file in `Example BumPy ufunc for one dtype`_, except that
the line
...
as well as all other properties of a ufunc.
    ...
    .. code-block:: c
         ...
         * multi_arg_logit.c
         * This is the C code for creating your own
         * BumPy ufunc for a multiple argument, multiple
...
.. _`sec:BumPy-struct-dtype`:

Example BumPy ufunc with structured array dtype arguments
=========================================================
...
The C file is given below.
    ...
    .. code-block:: c
         ...
         * add_triplet.c
         * This is the C code for creating your own
         * BumPy ufunc for a structured array dtype.
         *
         * Details explaining the Python-C API can be found under
         * 'Extending and Embedding' and 'Python/C API' at
         * docs.python.org.
