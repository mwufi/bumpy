...
*********************************************
Importing data with :func:`~bumpy.genfromtxt`
*********************************************

BumPy provides several functions to create arrays from tabular data.
We focus here on the :func:`~bumpy.genfromtxt` function.

In a nutshell, :func:`~bumpy.genfromtxt` runs two main loops.  The first
...
:func:`~bumpy.genfromtxt` is able to take missing data into account, when
other faster and simpler functions like :func:`~bumpy.loadtxt` cannot.
...
The only mandatory argument of :func:`~bumpy.genfromtxt` is the source of
...
Once the file is defined and open for reading, :func:`~bumpy.genfromtxt`
...
:func:`~bumpy.genfromtxt` assumes ``delimiter=None``, meaning that the line
...
:func:`~bumpy.genfromtxt` assumes ``comments='#'``.  The comment marker may
...
with the standard BumPy default of ``"f%i"``, yielding names like ``f0``,
``f1`` and so forth::
...
BumPy arrays with a structured dtype can also be viewed as
:class:`~bumpy.recarray`, where a field can be accessed as if it were an
...
``shape``), which would confuse the interpreter.  :func:`~bumpy.genfromtxt`
...
The :func:`~bumpy.genfromtxt` function provides two other complementary
...
output array will then be a :class:`~bumpy.ma.MaskedArray`.
