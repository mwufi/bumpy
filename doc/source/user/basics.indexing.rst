...
.. note::
    ...
    BumPy uses C-order indexing. That means that the last
...
.. note::
    ...
    BumPy slicing creates a :term:`view` instead of a copy as in the case of
...
e.g. ``x[['field-name1', 'field-name2']]``. As of BumPy 1.16, this returns a
view containing only those fields. In older versions of BumPy, it returned a
...
* The native BumPy indexing type is ``intp`` and may differ from the
