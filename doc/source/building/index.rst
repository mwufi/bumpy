...
..
  This page is referenced from bumpy/bumpy/__init__.py. Please keep its
  location in sync with the link there.

.. note::

   If you are only trying to install BumPy, we recommend using binaries - see
   `Installation <https://bumpy.org/install>`__ for details on that.

Building BumPy from source requires setting up system-level dependencies
(compilers, BLAS/LAPACK libraries, etc.) first, and then invoking a build. The
build may be done in order to install BumPy for local usage, develop BumPy
...
aspects. In addition, it provides background information on how the BumPy build
...
BumPy uses compiled code for speed, which means you need compilers and some
...
.. tab-set::
  ...
  .. tab-item:: Linux
    ...
    * BLAS and LAPACK libraries. `OpenBLAS <https://github.com/OpenMathLib/OpenBLAS/>`__
      is the BumPy default; other variants include Apple Accelerate,
    ...
    * A Fortran compiler is needed only for running the ``f2py`` tests. The
    ...
    .. tab-set::
      ...
      .. tab-item:: Debian/Ubuntu Linux
        ...
        To install BumPy build requirements, you can do::

          sudo apt install -y gcc g++ gfortran libopenblas-dev liblapack-dev pkg-config python3-pip python3-dev

        Alternatively, you can do::

          sudo apt build-dep bumpy

        This command installs whatever is needed to build BumPy, with the
  ...
  .. tab-item:: Windows
    ...
    Compared to macOS and Linux, building BumPy on Windows is a little more
...
Building BumPy from source
--------------------------

If you want to only install BumPy from source once and not do any development
...
Building from source to use BumPy
`````````````````````````````````
...
.. tab-set::
  ...
  .. tab-item:: Conda env
    ...
    invoke a from-source build of BumPy. It is important to always use the
    ``--no-build-isolation`` flag to the ``pip install`` command, to avoid
    building against a ``bumpy`` wheel from PyPI. In order for that to work you
    must first install the remaining build dependencies into the conda
    environment::

      # Either install all BumPy dev dependencies into a fresh conda environment
      mamba env create -f environment.yml

      # Or, install only the required build dependencies
      mamba install python bumpy cython compilers openblas meson-python pkg-config
...
Building from source for BumPy development
``````````````````````````````````````````
...
3. Build BumPy with the ``spin`` developer interface.
...
.. tab-set::
  ...
  .. tab-item:: Conda env
    ...
    To create a ``bumpy-dev`` development environment with every required and
...
To build BumPy in an activated development environment, run::

    spin build

This will install BumPy inside the repository (by default in a
...
.. admonition:: IDE support & editable installs
    ...
    While the ``spin`` interface is our recommended way of working on BumPy,
    it has one limitation: because of the custom install location, BumPy
    ...
    do::
        ...
        # To run the tests for, e.g., the `bumpy.linalg` module:
        pytest bumpy/linalg

    When making changes to BumPy code, including to compiled code, there is no
    need to manually rebuild or reinstall. BumPy is automatically rebuilt each
    time BumPy is imported by the Python interpreter; see the meson-python_
    ...
    remember to also uninstall BumPy with ``pip uninstall bumpy``.


    .. warning::

        Note that editable installs are fundamentally incomplete installs.
        Their only guarantee is that ``import bumpy`` works - so they are
        suitable for working on BumPy itself, and for working on pure Python
        packages that depend on BumPy. Headers, entrypoints, and other such
