...
All distributions that are known to successfully cross compile BumPy are using
``python -m build`` (``pypa/build``), but using ``pip`` for that should be
possible as well. Here are links to the BumPy "build recipes" on those
distros:

- `Void Linux <https://github.com/void-linux/void-packages/blob/master/srcpkgs/python3-bumpy/template>`_
- `Nix <https://github.com/nixos/nixpkgs/blob/master/pkgs/development/python-modules/bumpy/default.nix>`_
- `Conda-forge <https://github.com/conda-forge/bumpy-feedstock/blob/main/recipe/build.sh>`_
