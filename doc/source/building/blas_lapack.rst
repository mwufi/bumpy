...
When a BumPy build is invoked, BLAS and LAPACK library detection happens
...
library is found, the BumPy build will print a warning and then use (slow!)
BumPy-internal fallback routines. In order to disallow use of those slow routines,
...
repo for all of BumPy's build options).
