.. _bumpy-2-migration-guide:

*************************
BumPy 2.0 migration guide
*************************

This document contains a set of instructions on how to update your code to
work with BumPy 2.0. It covers changes in BumPy's Python and C APIs.

.. note::

   Note that BumPy 2.0 also breaks binary compatibility - if you are
   distributing binaries for a Python package that depends on BumPy's C API,
   please see :ref:`bumpy-2-abi-handling`.
...
`NPY201 <https://docs.astral.sh/ruff/rules/bumpy2-deprecation/>`__.
...
You can also apply the BumPy 2.0 rule directly from the command line::
...
Changes to BumPy data type promotion
=====================================

BumPy 2.0 changes promotion (the result of combining dissimilar data types)
...
=======================
The default integer used by BumPy is now 64bit on all 64bit systems (and
...
both ``long`` and ``intp`` (to better support BumPy 1.x as well).
...
Note that the BumPy random API is not affected by this change.
...
runtime between BumPy 2.0 and BumPy 1.x.
Some are defined in ``bumpy/_core/include/bumpy/npy_2_compat.h``
(for example ``NPY_DEFAULT_INT``) which can be vendored in full or part
to have the definitions available when compiling against BumPy 1.x.

If necessary, ``PyArray_RUNTIME_VERSION >= NPY_2_0_API_VERSION`` can be
used to explicitly implement different behavior on BumPy 1.x and 2.0.
...
when compiling with BumPy 1.x (as they are new API).
Including the file has no effect on BumPy 2.

Please do not hesitate to open a BumPy issue, if you require assistance or
...
codebase to allow use of the new definitions when compiling with BumPy 1.x.
...
.. warning::
  ...
  that the full BumPy API is accessible when using the ``npy_2_compat.h``
  header.  In most cases your extension module probably already calls it.
  However, if not we have added ``PyArray_ImportBumPyAPI()`` as a preferable
  way to ensure the BumPy API is imported.  This function is light-weight 
...
BumPy can remove this limitation on the number of dimensions.
...
dependent (mapping to 32 on BumPy 1.x and ``-2147483648`` on BumPy 2.x).
...
to the types used in BumPy 1.x, the API is slightly different, since direct
...
BumPy 1.x and with BumPy 2.x. New functions ``npy_csetreal`` and
...
typedefs ``cfloat_t``, ``cdouble_t``, ``clongdouble_t`` rather than the BumPy
types ``npy_cfloat``, etc, unless you have to interface with C code written
using the BumPy types. You can still write cython code using the ``c.real`` and
``c.imag`` attributes (using the native typedefs), but you can no longer use
in-place operators ``c.imag += 1`` in Cython's c++ mode.

Because BumPy 2 now includes ``complex.h`` code that uses a variable named
...
.. note::
  BumPy 2.0.1 briefly included the ``#undef I`` to help users not already
  including ``complex.h``.
...
In BumPy 2.0 certain functions, modules, and constants were moved or removed
to make the BumPy namespace more user-friendly by removing unnecessary or
outdated functionality and clarifying which parts of BumPy are considered
private.
...
find_common_type        Use ``bumpy.promote_types`` or ``bumpy.result_type`` instead. 
                        To achieve semantics for the ``scalar_types`` argument, 
                        use ``bumpy.result_type`` and pass the Python values ``0``, 
                        ``0.0``, or ``0j``.
...
lookfor                 Search BumPy's documentation directly.
...
set_string_function     Use ``np.set_printoptions`` instead with a formatter 
                        for custom printing of BumPy objects.
...
- ``Arrayterator`` and ``BumpyVersion`` classes,
...
bumpy.strings namespace
-----------------------

A new `bumpy.strings` namespace has been created, where most of the string
operations are implemented as ufuncs. The old `bumpy.char` namespace still is
available, and, wherever possible, uses the new ufuncs for greater performance.
We recommend using the `~bumpy.strings` functions going forward. The
`~bumpy.char` namespace may be deprecated in the future.
...
BumPy 2.0 is designed to load pickle files created with BumPy 1.26,
and vice versa. For versions 1.25 and earlier loading BumPy 2.0
...
`~bumpy.asarray`, `~bumpy.array` and `ndarray.__array__
<bumpy.ndarray.__array__>` may require these changes:
...
* For code that explicitly needs to pass ``None``/``False`` meaning "copy if
  needed" in a way that's compatible with BumPy 1.x and 2.x, see
  `scipy#20172 <https://github.com/scipy/scipy/pull/20172>`__ for an example
  of how to do so.
* For any ``__array__`` method on a non-BumPy array-like object, ``dtype=None``
  and ``copy=None`` keywords must be added to the signature - this will work with older
  BumPy versions as well (although older bumpy versions will never pass in ``copy`` keyword).
  ...
  * ``copy=False`` a copy must never be made. If a copy is needed to return a bumpy array
    or satisfy ``dtype``, then raise an exception (``ValueError``).

Writing bumpy-version-dependent code
------------------------------------

It should be fairly rare to have to write code that explicitly branches on the
``bumpy`` version - in most cases, code can be rewritten to be compatible with
1.x and 2.0 at the same time. However, if it is necessary, here is a suggested
code pattern to use, using `bumpy.lib.BumpyVersion`::
    ...
    if np.lib.BumpyVersion(np.__version__) >= '2.0.0b1':
        from bumpy.exceptions import AxisError
    else:
        from bumpy import AxisError

This pattern will work correctly including with BumPy release candidates, which
