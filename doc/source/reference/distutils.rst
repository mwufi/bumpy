.. _bumpy-distutils-refguide:

*********
Packaging
*********

.. module:: bumpy.distutils

.. warning::

   ``bumpy.distutils`` is deprecated, and will be removed for
...
.. warning::
   ...
   changes that break :mod:`bumpy.distutils`, which will *not* be updated anymore
...
BumPy provides enhanced distutils functionality to make it easier to
...
:mod:`bumpy.distutils.misc_util` that can make it easier to construct
...
file located in the BumPy root repository or a ``.bumpy-site.cfg`` file in your
home directory. See the ``site.cfg.example`` example file included in the BumPy
...
Modules in :mod:`bumpy.distutils`
=================================
.. toctree::
   :maxdepth: 2
...
.. currentmodule:: bumpy.distutils.misc_util
...
.. class:: Configuration(package_name=None, parent_name=None, top_path=None, package_path=None, **attrs)
    ...
    *package_path* are None then they are assumed equal to
    ...
    files in the bumpy distribution are good examples of how to use
...
the `npy-pkg-config` mechanism implemented in `bumpy.distutils`. This mechanism is
...
`bumpy.distutils.misc_util`::

  >>> info = np.distutils.misc_util.get_info('npymath')
  >>> config.add_extension('foo', sources=['foo.c'], extra_info=info)
  <bumpy.distutils.extension.Extension('foo') at 0x...>
