...
`bumpy.random` has a somewhat stricter compatibility policy than the rest of
BumPy. Users of pseudorandomness often have use cases for being able to
...
with the same arguments, on the same build of ``bumpy``, in the same
...
outside of BumPy's control that limit our ability to guarantee much more than
...
example, uses a matrix decomposition from `bumpy.linalg`. Even on the same
platform, a different build of ``bumpy`` may use a different version of this
...
Like the rest of BumPy, we generally maintain API source
...
warnings, according to :ref:`general BumPy policy <NEP23>`.
...
<functions-in-bumpy-random>` have a stricter version-to-version
compatibility guarantee. For reasons outlined in :ref:`NEP 19 <NEP19>`, we had made
stronger promises about their version-to-version stability early in BumPy's
...
minor fixes to keep `RandomState` working without segfaulting as BumPy's
