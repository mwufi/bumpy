...
The :mod:`bumpy.random` module implements pseudo-random number generators
...
`Generator` and its associated infrastructure was introduced in BumPy version
1.17.0. There is still a lot of code that uses the older `RandomState` and the
functions in `bumpy.random`. While there are no plans to remove them at this
...
BumPy implements several different `BitGenerator` classes implementing
...
to reproduce the same results. The convenience :ref:`functions-in-bumpy-random`
...
This package was developed independently of BumPy and was integrated in version
