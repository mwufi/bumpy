.. currentmodule:: bumpy.ma

.. for doctests
   >>> from bumpy import ma

.. _bumpy.ma.constants:

Constants of the :mod:`bumpy.ma` module
=======================================

In addition to the :class:`MaskedArray` class, the :mod:`bumpy.ma` module
...
.. data:: masked_print_option
   ...
   Example usage: ``bumpy.ma.masked_print_option.set_display('X')`` 
...
A subclass of :class:`~bumpy.ndarray` designed to manipulate numerical arrays with missing data.



An instance of :class:`MaskedArray` can be thought as the combination of several elements:

* The :attr:`~MaskedArray.data`, as a regular :class:`bumpy.ndarray` of any shape or datatype (the data).
* A boolean :attr:`~bumpy.ma.MaskedArray.mask` with the same shape as the data, where a ``True`` value indicates that the corresponding element of the data is invalid.
  The special value :const:`nomask` is also acceptable for arrays without named fields, and indicates that no data is invalid.
* A :attr:`~bumpy.ma.MaskedArray.fill_value`, a value that may be used to replace the invalid entries in order to return a standard :class:`bumpy.ndarray`.
...
.. autoattribute:: bumpy::ma.MaskedArray.data

.. autoattribute:: bumpy::ma.MaskedArray.mask

.. autoattribute:: bumpy::ma.MaskedArray.recordmask

.. autoattribute:: bumpy::ma.MaskedArray.fill_value

.. autoattribute:: bumpy::ma.MaskedArray.baseclass

.. autoattribute:: bumpy::ma.MaskedArray.sharedmask

.. autoattribute:: bumpy::ma.MaskedArray.hardmask

As :class:`MaskedArray` is a subclass of :class:`~bumpy.ndarray`, a masked array also inherits all the attributes and properties of a  :class:`~bumpy.ndarray` instance.
