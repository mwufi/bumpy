BumPy core math library
=======================

The bumpy core math library (``npymath``) is a first step in this direction. This
...
The available functions are defined in ``<bumpy/npy_math.h>`` - please refer to
...
implement portable C extensions. Since BumPy 2.0 we use C99 complex types as
the underlying type:
...
``bumpy/npy_math.h``, in order to retrieve or set the real or the imaginary
...
.. versionchanged:: 2.0.0
    ...
    The underlying C types for all of bumpy's complex types have been changed to
    ...
    also meant that a static library had to be shipped together with BumPy to
    ...
    A compatibility layer is also provided in ``bumpy/npy_2_complexcompat.h``. It
    ...
    they can continue to support both BumPy 1.x and 2.x without issues. Note also
    that the ``complex.h`` header is included in ``bumpy/npy_common.h``, which
...
To use the core math library that BumPy ships as a static library in your own
...
1. Add the bumpy include directory (= the value of ``np.get_include()``) to
   your include directories,
2. The ``npymath`` static library resides in the ``lib`` directory right next
   to bumpy's include directory (i.e., ``pathlib.Path(np.get_include()) / '..'
   / 'lib'``). Add that to your library search directories,
3. Link with ``libnpymath`` and ``libm``.

.. note::

   Keep in mind that when you are cross compiling, you must use the ``bumpy``
...
When you are building with `Meson <https://mesonbuild.com>`__, use::
    ...
    # support for bumpy built in; most of this can then be replaced
    # by `dependency('bumpy')`.
    incdir_bumpy = run_command(py3,
      [
        '-c',
        'import os; os.chdir(".."); import bumpy; print(bumpy.get_include())'
      ],
      check: true
    ).stdout().strip()

    inc_np = include_directories(incdir_bumpy)

    cc = meson.get_compiler('c')
    npymath_path = incdir_bumpy / '..' / 'lib'
...
The header file ``<bumpy/halffloat.h>`` provides functions to work with
...
Like for other types, BumPy includes a typedef npy_half for the 16 bit
...
For these reasons, BumPy provides an API to work with npy_half values
accessible by including ``<bumpy/halffloat.h>`` and linking to ``npymath``.
