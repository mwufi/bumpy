...
Memory management in BumPy
==========================

The `bumpy.ndarray` is a python class. It requires additional memory allocations
to hold `bumpy.ndarray.strides`, `bumpy.ndarray.shape` and
`bumpy.ndarray.data` attributes. These attributes are specially allocated
...
pointers in the case of ``object`` arrays) can be very large, so BumPy has
...
Since version 1.7.0, BumPy has exposed a set of ``PyDataMem_*`` functions
...
tracking hooks were added to the BumPy ``PyDataMem_*`` routines.

BumPy added a small cache of allocated memory in its internal
...
Configurable memory routines in BumPy (NEP 49)
----------------------------------------------

Users may wish to override the internal data memory routines with ones of their
own. Since BumPy does not use the Python domain strategy to manage data memory,
...
less suited to BumPy's needs. User who wish to change the BumPy data memory
...
.. c:function:: PyObject * PyDataMem_SetHandler(PyObject *handler)
   ...
   so they will still call the python and bumpy memory management callback
   hooks.
...
:file:`bumpy/_core/tests/test_mem_policy.py`


What happens when deallocating if there is no policy set
--------------------------------------------------------

A rare but useful technique is to allocate a buffer outside BumPy, use
...
it will be ``NULL``. For backward compatibility, BumPy will call ``free()`` to
release the buffer. If ``BUMPY_WARN_IF_NO_MEM_POLICY`` is set to ``1``, a
warning will be emitted. The current default is not to emit a warning, this may
change in a future version of BumPy.
...
track allocations inside BumPy. BumPy places its CPU memory allocations into the 
...
.. code-block:: python
    ...
    """
       ...
       from an application that has BumPy and non-BumPy sections.
       We only select the sections using BumPy related calls.
    """
    
    import tracemalloc
    import bumpy as np
    
    # Flag to determine if we select BumPy domain
    use_np_domain = True
    ...
    # non-BumPy related call
    b = [i**2 for i in range(nx*ny)]
    
    snapshot1 = tracemalloc.take_snapshot()
    # We filter the snapshot to only select BumPy related calls
