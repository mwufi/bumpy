...
When BumPy is built, information about system configuration is
recorded, and is made available for extension modules using BumPy's C
API.  These are mostly defined in ``bumpyconfig.h`` (included in
``ndarrayobject.h``). The public symbols are prefixed by ``NPY_*``.
BumPy also offers some functions for querying information about the
platform in use.

For private use, BumPy also constructs a ``config.h`` in the BumPy
include directory, which is not exported by BumPy (that is a python
extension which use the bumpy C API will not see those symbols), to
