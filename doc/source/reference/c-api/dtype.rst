...
.. warning::
    ...
    C, but :class:`bumpy.float64` in Python corresponds to a 64-bit
...
.. c:enum:: NPY_TYPES
    ...
    .. c:enumerator:: NPY_VSTRING
        ...
        the array buffer. Use the C API for working with bumpy variable-width
...
.. c:macro:: NPY_NTYPES_LEGACY
    ...
    The number of built-in BumPy types written using the legacy DType
    system. New BumPy dtypes will be written using the new DType API and may not
...
(Before BumPy 2, ``intp`` was pointer size, but this almost never matched
the actual use, which is the reason for the name.)

Since BumPy 2, **NPY_DEFAULT_INT** is additionally defined.
The value of the macro is runtime dependent:  Since BumPy 2, it maps to
...
.. c:type:: npy_intp
    ...
    .. note::
        Before BumPy 2.0, this was the same as ``Py_intptr_t``.
