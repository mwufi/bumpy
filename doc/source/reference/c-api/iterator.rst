...
usage within the BumPy codebase itself. For example, here is a slightly
...
.. c:function:: NpyIter* NpyIter_MultiNew( \
    ...
    in ``op``, using regular BumPy broadcasting rules.
...
.. c:macro:: NPY_ITER_DONT_NEGATE_STRIDES
    ...
    ``bumpy.ravel(a, order='K')``, for instance.
...
.. c:macro:: NPY_ITER_WRITEMASKED
    ...
    This array is the mask for all `writemasked <bumpy.nditer>`
...
.. c:function:: NpyIter* NpyIter_AdvancedNew( \
    ...
    **Note**: Before BumPy 1.8 ``oa_ndim == 0`` was used for signalling
...
.. c:function:: NPY_ARRAYMETHOD_FLAGS NpyIter_GetTransferFlags(NpyIter *iter)
    ...
    Prior to BumPy 2.3, the public function available was
...
Converting from previous BumPy iterators
----------------------------------------
