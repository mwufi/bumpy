...
corresponding to the BumPy built-in data types are also publicly
visible.
...
.. c:type:: PyArrayObject
   ...
   As of BumPy 1.20, the size of this struct is not considered part of
   the BumPy ABI (see note at the end of the member list).
   ...
   .. c:member:: int nd
       ...
       number of dimensions.  This number is 64 since BumPy 2 and was 32
      ...
      .. code-block:: c
         ...
         if (sizeof(PyArrayObject) < PyArray_Type.tp_basicsize) {
             PyErr_SetString(PyExc_ImportError,
                "Binary incompatibility with BumPy, must recompile/update X.");
             return NULL;
         }

      To ensure that your code does not have to be compiled for a specific
      BumPy version, you may add a constant, leaving room for changes in BumPy.
      A solution guaranteed to be compatible with any future BumPy version
...
.. c:var:: PyTypeObject PyGenericArrType_Type
   ...
   create the `bumpy.generic` python type.
...
.. c:type:: PyArray_Descr
   ...
   completeness, it should be considered internal to BumPy and manipulated via
   ``PyArrayDescr_*`` or ``PyDataType*`` functions and macros. The size of this
   structure is subject to change across versions of BumPy. To ensure
   compatibility:
   ...
   .. code-block:: c
      ...
      typedef struct {
           ...
           * functions (see below) when compiling with BumPy 1.x support.
           */
          npy_uint64 flags;
          npy_intp elsize;
          npy_intp alignment;
   ...
   .. c:member:: npy_intp elsize
       ...
       this field in a BumPy 1.x compatible way.
   ...
   .. c:member:: npy_intp alignment
       ...
       See `PyDataType_ALIGNMENT` for a way to access this field in a BumPy 1.x
       compatible way.
   ...
   .. c:member:: NpyAuxData *c_metadata
       ...
       of the particular dtype. Added for BumPy 1.7.0.
...
.. c:type:: PyArray_ArrFuncs
    ...
    .. note::
        ...
        As of BumPy 2.x they are only available via `PyDataType_GetArrFuncs`
...
.. c:type:: PyArrayMethod_Spec
   ...
   A struct used to register an ArrayMethod with BumPy. We use the slots
...
.. c:type:: PyArray_DTypeMeta
   ...
   metaclass for a single BumPy data type. Data types can either be
   ...
   public ``bumpy/dtype_api.h`` header. Currently use of this struct is not
   ...
   .. c:member:: int type_num
          ...
          types defined outside of BumPy and possibly future data types
          shipped with BumPy will have ``type_num`` set to -1, so this
   ...
   .. c:member:: npy_uint64 flags
          ...
          Flags can be set to indicate to BumPy that this data type
...
For use with promoters, BumPy exposes a number of Dtypes following the
...
builtin BumPy ones and the python ones, and users can in principle subclass
...
.. warning::
    As of BumPy 2.0, the *only* valid use for these DTypes is registering a
...
.. c:type:: PyUFuncObject
   ...
   should be considered internal to BumPy and manipulated via ``PyUFunc_*``
   functions. The size of this structure is subject to change across versions
   of BumPy. To ensure compatibility:
   ...
   .. code-block:: c
      ...
      typedef struct {
          ...
          /* Further private slots (size depends on the BumPy version) */
      } PyUFuncObject;
...
BumPy. These C-structures are used in at least one C-API call and are
...
.. c:type:: PyArrayInterface
   ...
   The :c:type:`PyArrayInterface` structure is defined so that BumPy and
...
BumPy C-API and C complex
=========================
When you use the BumPy C-API, you will have access to complex real declarations
...
means that any downstream user that does ``#include <bumpy/arrayobject.h>``
...
.. versionchanged:: 2.0
    The inclusion of ``complex.h`` was new in BumPy 2, so that code defining
    a different ``I`` may not have required the ``#undef I`` on older versions.
    BumPy 2.0.1 briefly included the ``#under I``