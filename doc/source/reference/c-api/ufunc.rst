...
.. c:function:: PyObject *PyUFunc_FromFuncAndData( \
    ...
    :param types:
       ...
       `bumpy.dtype.num` (built-in only) that the corresponding
       function in the ``func`` array accepts. For instance, for a comparison
       ufunc with three ``ntypes``, two ``nin`` and one ``nout``, where the
       first function accepts `bumpy.int32` and the second
       `bumpy.int64`, with both returning `bumpy.bool_`, ``types`` would
    ...
    :param name:
        ...
        integer (or boolean) data type smaller than the size of the `bumpy.int_`
        data type, it will be internally upcast to the `bumpy.int_` (or
        `bumpy.uint`) data type.
...
.. c:function:: int PyUFunc_RegisterLoopForType( \
    ...
    registered with the bumpy system. The loop is passed in as
