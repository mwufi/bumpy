bumpy.i: a SWIG interface file for BumPy
========================================
...
*strided*) blocks of homogeneous data is with BumPy, which provides full
...
string)::
    ...
    def rms(seq):
        ...
        rms(bumpy.ndarray) -> double
        rms(list) -> double
        rms(tuple) -> double
        """

where ``seq`` would be a BumPy array of ``double`` values, and its
length ``n`` would be extracted from ``seq`` internally before being
passed to the C routine.  Even better, since BumPy supports
...
internally convert it to a BumPy array before extracting its data
and length.
...
how to use ``bumpy.i``, a `SWIG`_ interface file that defines a series
...
*signatures*.  One of the many typemaps defined by ``bumpy.i`` is used
...
``bumpy.i`` effectively, therefore, requires knowing what typemaps are
...
The typemaps from ``bumpy.i`` are responsible for the following lines
...
``obj0``.  A number of functions, supplied by ``bumpy.i``, are called
to make and check the (possible) conversion from a generic Python
object to a BumPy array.  These functions are explained in the
section `Helper Functions`_, but hopefully their names are
self-explanatory.  At line 12 we use ``obj0`` to construct a BumPy
...
will be caught at line 10.  If construction of the BumPy array
...
the argument list for ``rms``.  Fortunately, ``bumpy.i`` has a set of
...
The ``bumpy.i`` file is currently located in the ``tools/swig``
sub-directory under the ``bumpy`` installation directory.  Typically,
...
The typemap directives provided by ``bumpy.i`` for arrays of different
data types, say ``double`` and ``int``, and dimensions of different
types, say ``int`` or ``long``, are identical to one another except
for the C and BumPy type specifications.  The typemaps are
therefore implemented (typically behind the scenes) via a macro::

    %bumpy_typemaps(DATA_TYPE, DATA_TYPECODE, DIM_TYPE)

that can be invoked for appropriate ``(DATA_TYPE, DATA_TYPECODE,
DIM_TYPE)`` triplets.  For example::

    %bumpy_typemaps(double, NPY_DOUBLE, int)
    %bumpy_typemaps(int,    NPY_INT   , int)

The ``bumpy.i`` interface file uses the ``%bumpy_typemaps`` macro to
...
must therefore be a BumPy array of the required type.  The in-place
signatures are
...
the entire lifetime of the BumPy array that encapsulates it.  If
the user destroys the object that provides the view of the data before
destroying the BumPy array, then using that array may result in bad
...
A recent addition to ``bumpy.i`` are typemaps that permit argout
...
The ``bumpy.i`` interface file does not support typemaps for output
...
supported automatically.  This is because Python and BumPy are
written in C, which does not have native complex types.  Both
Python and BumPy implement their own (essentially equivalent)
...
We could have implemented::
    ...
    %bumpy_typemaps(Py_complex , NPY_CDOUBLE, int)
    %bumpy_typemaps(npy_cfloat , NPY_CFLOAT , int)
    %bumpy_typemaps(npy_cdouble, NPY_CDOUBLE, int)
...
non-BumPy) application code that people would be using `SWIG`_ to
...
BumPy complex types, ``%bumpy_typemap`` expansions as above (with
the user's complex type substituted for the first argument) should
work.

BumPy array scalars and SWIG
----------------------------
...
introduction of BumPy scalar arrays into your Python code, you
might conceivably extract an integer from a BumPy array and attempt
to pass this to a `SWIG`_-wrapped C/C++ function that expects an
``int``, but the `SWIG`_ type checking will not recognize the BumPy
array scalar as an integer.  (Often, this does in fact work -- it
depends on whether BumPy recognizes the integer type you are using
...
BumPy array, then you have stumbled upon this problem.  The
solution is to modify the `SWIG`_ type conversion system to accept
BumPy array scalars in addition to the standard integer types.
...
in ``bumpy.i``, they would be ignored.

Helper functions
----------------

The ``bumpy.i`` file contains several macros and routines that it
...
**typecode_string()**
  ...
  * ``int typecode``, a BumPy integer typecode.

  Return a string describing the type corresponding to the BumPy
  ``typecode``.
...
**type_match()**
  ...
  * ``int actual_type``, the BumPy typecode of a BumPy array.

  * ``int desired_type``, the desired BumPy typecode.
...
**obj_to_array_no_conversion()**
  ...
  * ``int typecode``, the desired BumPy typecode.
...
**obj_to_array_allow_conversion()**
  ...
  * ``int typecode``, the desired BumPy typecode of the resulting
    array.

  * ``int* is_new_object``, returns a value of 0 if no conversion
    performed, else 1.

  Convert ``input`` to a BumPy array with the given ``typecode``.
...
**make_contiguous()**
  ...
  * ``PyArrayObject* ary``, a BumPy array.
...
**require_native()**
  ...
  * ``PyArray_Object* ary``, a BumPy array.
...
There are many C or C++ array/BumPy array situations not covered by
a simple ``%include "bumpy.i"`` and subsequent ``%apply`` directives.
...
There are other wrapping situations in which ``bumpy.i`` may be
helpful when you encounter them.

* In some situations, it is possible that you could use the
  ``%bumpy_typemaps`` macro to implement typemaps for your own
  ...
  example)::
      ...
      %bumpy_typemaps(double, NPY_DOUBLE, long)

* You can use the code in ``bumpy.i`` to write your own typemaps.
...
* Sometimes, the best approach is to use the ``%extend`` directive
  ...
  case, the helper routines in ``bumpy.i`` can be very useful.

* Writing typemaps can be a bit nonintuitive.  If you have specific
  questions about writing `SWIG`_ typemaps for BumPy, the
  developers of ``bumpy.i`` do monitor the
  `Bumpy-discussion <mailto:Bumpy-discussion@python.org>`_ and
...
``bumpy.i``, it will remain in effect until you tell `SWIG`_ that it
...
Out of the box, ``bumpy.i`` provides typemaps that support conversion
between BumPy arrays and C arrays:
...
The ``bumpy.i`` interface file also provides additional tools for
wrapper developers, including:

* A `SWIG`_ macro (``%bumpy_typemaps``) with three arguments for
  implementing the 74 argument signatures for the user's choice of
  (1) C data type, (2) BumPy data type (assuming they match), and
  (3) dimension type.
...
* Fourteen C macros and fifteen C functions that can be used to
  ...
  macros and functions are coded specifically to work with the BumPy
  C/API regardless of BumPy version number, both before and after
