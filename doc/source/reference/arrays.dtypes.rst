...
A data type object (an instance of :class:`bumpy.dtype` class)
...
scalar types <arrays.scalars.built-in>` in BumPy for various precision
...
Whenever a data-type is required in a BumPy function or method, either
...
Array-protocol type strings (see :ref:`arrays.interface`)
   ...
   .. admonition:: Note on string types
    ...
    For unicode strings, use ``U``, `bumpy.str_`.  For signed bytes that do not
...
String with comma-separated fields
   ...
   on the shape if it has more than one dimension. BumPy allows a modification
...
Type strings
   Any string name of a BumPy dtype, e.g.:
...
``(fixed_dtype, shape)``
    ...
    deprecated since BumPy 1.17 and will raise an error in the future.
...
``(base_dtype, new_dtype)``
    In BumPy 1.7 and later, this form allows `base_dtype` to be interpreted as
...
First, BumPy treats data type specifications (everything that can be passed to
...
BumPy data type descriptions are instances of the :class:`dtype` class.
