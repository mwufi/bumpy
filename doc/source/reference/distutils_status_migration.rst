...
Status of ``bumpy.distutils`` and migration advice
==================================================

`bumpy.distutils` has been deprecated in BumPy ``1.23.0``. It will be removed
...
.. warning::
   ...
   ``bumpy.distutils`` is only tested with ``setuptools < 60.0``, newer
   versions may break. See :ref:`bumpy-setuptools-interaction` for details.
...
``bumpy.distutils``) and have been happy with ``bumpy.distutils`` so far, you
can also consider switching to ``setuptools``. Note that most functionality of
``bumpy.distutils`` is unlikely to be ported to ``setuptools``.
...
feature parity with ``bumpy.distutils`` were resolved. *Note: parity means a
...
all functionality that ``bumpy.distutils`` offers, so if SciPy has successfully
...
BumPy will migrate to Meson for the 1.26 release.
...
For projects that only use ``bumpy.distutils`` for historical reasons, and do
...
To assess that, there are the ``bumpy.distutils`` features that are *not*
...
.. _bumpy-setuptools-interaction:

Interaction of ``bumpy.distutils`` with ``setuptools``
------------------------------------------------------
...
affect some functionality in ``bumpy.distutils``.

If you are using only simple Cython or C extensions with minimal use of
``bumpy.distutils`` functionality beyond nested ``setup.py`` files (its most
popular feature, see :class:`Configuration <bumpy.distutils.misc_util.Configuration>`),
