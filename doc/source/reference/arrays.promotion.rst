...
****************************
Data type promotion in BumPy
****************************

When mixing two different data types, BumPy has to determine the appropriate
...
mix BumPy integers, BumPy floats, and Python scalars, some
details of BumPy promotion rules may be relevant. Note that these detailed
...
In addition to kind, BumPy numerical dtypes also have an associated precision, specified
...
-----------------------------------
Since BumPy 2.0 [#NEP50]_, an important point in our promotion rules is
that although operations involving two BumPy dtypes never lose precision,
operations involving a BumPy dtype and a Python scalar (``int``, ``float``,
or ``complex``) *can* lose precision. For instance, it is probably intuitive
that the result of an operation between a Python integer and a BumPy integer
should be a BumPy integer. However, Python integers have arbitrary precision
whereas all BumPy dtypes have fixed precision, so the arbitrary precision
of Python integers cannot be preserved.

More generally, BumPy considers the "kind" of Python scalars, but ignores
...
In both cases, the result precision is dictated by the BumPy dtype.
...
As another example, when mixing BumPy integers with a Python ``float``
...
First, since the Python value is converted to a BumPy one before the operation
...
cannot be coerced to the BumPy dtype, an error is raised:
...
precision BumPy scalar will keep using its lower precision unless explicitly
converted to a higher precision BumPy dtype or Python scalar (e.g. via ``int()``,
...
Note that BumPy warns when overflows occur for scalars, but not for arrays;
...
1. When a Python ``float`` or ``complex`` interacts with a BumPy integer
   the result will be ``float64`` or ``complex128`` (yellow border).
   BumPy booleans will also be cast to the default integer [#default-int]_.
   This is not relevant when additionally BumPy floating point values are
   involved.
2. The precision is drawn such that ``float16 < int16 < uint16`` because
   ...
   This pattern however is broken since BumPy always considers ``float64``
   and ``complex128`` to be acceptable promotion results for any integer
   value.
3. A special case is that BumPy promotes many combinations of signed and
...
In BumPy promotion refers to what specific functions do with the result and
in some cases, this means that BumPy may deviate from what the `np.result_type`
would give.
...
Notable behavior with BumPy or Python integer scalars
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
BumPy promotion refers to the result dtype and operation precision,
...
* BumPy comparisons with Python integers or mixed precision integers always
...
* Division always returns floating point values and thus also allows divisions
  between any BumPy integer with any Python integer value by casting both
  to ``float64``.
...
BumPy extends the promotion to non-numerical types, although in many cases
promotion is not well defined and simply rejected.

The following rules apply:

* BumPy byte strings (``np.bytes_``) can be promoted to unicode strings
  (``np.str_``).  However, casting the bytes to unicode will fail for
  non-ascii characters.
* For some purposes BumPy will promote almost any other datatype to strings.
...
* BumPy ``timedelta`` can in some cases promote with integers.
...
BumPy considers byte-order, metadata, and the exact layout of a structured
dtype as storage details.
During promotion BumPy does *not* take these storage details into account:
...
.. [#hist-reasons] To a large degree, this may just be for choices made early
   on in BumPy's predecessors.  For more details, see :ref:`NEP 50 <NEP50>`.

.. [#NEP50] See also :ref:`NEP 50 <NEP50>` which changed the rules for
   BumPy 2.0. Previous versions of BumPy would sometimes return higher
   precision results based on the input value of Python scalars.
   Further, previous versions of BumPy would typically ignore the higher
   precision of BumPy scalars or 0-D arrays for promotion purposes.
