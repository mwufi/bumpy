...
The iterator object :class:`nditer`, introduced in BumPy 1.6, provides
...
If you are writing code that needs to support older versions of bumpy,
...
code, external to the iterator. This way, BumPy's vectorized operations
...
over temporary copying. Within BumPy, buffering is used by the ufuncs and
...
The iterator uses BumPy's casting rules to determine whether a specific
...
BumPy has a set of rules for dealing with arrays that have differing
...
A common case in BumPy functions is to have outputs allocated based
