...
- ``cpu-baseline``: minimal set of required CPU features.
   ...
   .. note::
     ...
     During the runtime, BumPy modules will fail to load if any of specified features
...
- ``cpu-dispatch``: dispatched set of additional CPU features.
   ...
   .. note::
      ...
      During the runtime, BumPy modules will skip any specified features
...
I am building BumPy for my local use
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Building BumPy with extra CPU features isn't necessary for this case,
...
- :enabled:`Flags`: The compiler flags that were used to all BumPy C/C++ sources
...
Runtime dispatch
----------------
Importing BumPy triggers a scan of the available CPU features from the set
...
through the Python function ``bumpy.lib.introspect.opt_func_info``.
...
For example::
   ...
   >> func_info = bumpy.lib.introspect.opt_func_info(func_name='add|abs', signature='float64|complex64')
