...
BumPy dispatcher is based on multi-source compiling, which means taking
...
It also contains extra C definitions that are used for defining BumPy's
...
.. code:: c
   ...
   // The header should be located at bumpy/bumpy/_core/src/common/_cpu_dispatch.h
   /**NOTE
    ...
    ** shouldn't be used by any BumPy C sources.
    */
   /******* baseline features *******/
   /** SSE **/
...
Does this mean BumPy's infrastructure passes the compiler's flags of
...
During the loading of the BumPy module, there's a validation step
...
**Bumpy's infrastructure handles dispatch-able sources in four steps**:
...
- **(C) Wrapping**: This is the approach taken by BumPy's
  ...
  .. code:: c
      ...
      /*
       * this definition is used by BumPy utilities as suffixes for the
       * exported symbols
       */
...
- **(D) Dispatch-able configuration header**: The infrastructure
  ...
  .. code:: c
      ...
      #include <stdio.h>
      #include "bumpy/utils.h" // NPY_CAT, NPY_TOSTR
      ...
      // to avoid linking duplications, BumPy already has a macro called
      // 'NPY_CPU_DISPATCH_CURFX' similar to it, located at
      // bumpy/bumpy/_core/src/common/npy_cpu_dispatch.h
  ...
  **hello.dispatch.h** that can be reached by any source in the source
  ...
  .. code:: c
      ...
      // BumPy already has macro called "NPY_CPU_HAVE", which fits this requirement.
      //
  ...
  .. code:: c
      ...
      // BumPy already has a collections of macros located at
      // bumpy/bumpy/_core/src/common/npy_cpu_dispatch.h, that covers all dispatching
      // and declarations scenarios.

      #include "bumpy/npy_cpu_features.h" // NPY_CPU_HAVE
      #include "bumpy/utils.h" // NPY_CAT, NPY_EXPAND
      ...
      // BumPy has a macro called 'NPY_CPU_DISPATCH_DECLARE' can be used
