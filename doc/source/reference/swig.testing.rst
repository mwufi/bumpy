Testing the bumpy.i typemaps
============================

Introduction
------------

Writing tests for the ``bumpy.i`` `SWIG <https://www.swig.org/>`_
...
``make test`` is run in the ``bumpy/tools/swig`` subdirectory.
...
the testing infrastructure employed to verify that the ``bumpy.i``
...
in ``bumpy.i`` to correctly handle the C arrays.
...
``Vector``.  It follows the conventions for using ``bumpy.i`` as
described in this chapter.  It defines a `SWIG`_ macro
``%apply_bumpy_typemaps`` that has a single argument ``TYPE``.
...
``bumpy.i``.  It then does a ``%include "Vector.h"`` to wrap all of
...
**self.typeCode**
  ...
  data type in bumpy and corresponds to ``self.typeStr``.  For
