...
The :mod:`bumpy.ma` module provides a nearly work-alike replacement for bumpy
...
recorded an invalid value. The :mod:`bumpy.ma` module provides a convenient
way to address this issue, by introducing masked arrays.

A masked array is the combination of a standard :class:`bumpy.ndarray` and a
...
The main feature of the :mod:`bumpy.ma` module is the :class:`MaskedArray`
class, which is a subclass of :class:`bumpy.ndarray`. The class, its
attributes and methods are described in more details in the
:ref:`MaskedArray class <maskedarray.baseclass>` section.

The :mod:`bumpy.ma` module can be used as an addition to :mod:`bumpy`:
...
* through the :attr:`~MaskedArray.data` attribute. The output is a view of the
  array as a :class:`bumpy.ndarray` or one of its subclasses, depending on the
...
* by directly taking a view of the masked array as a :class:`bumpy.ndarray`
...
:func:`bumpy.logical_not` function or simply with the ``~`` operator:
   ...
   method, which returns a one-dimensional :class:`~bumpy.ndarray` (or one of its
...
As a :class:`MaskedArray` is a subclass of :class:`bumpy.ndarray`, it inherits
...
:class:`bumpy.void` object if none of the fields are masked, or a 0d masked
...
The :mod:`bumpy.ma` module comes with a specific implementation of most
ufuncs. Unary and binary functions that have a validity domain (such as
:func:`~bumpy.log` or :func:`~bumpy.divide`) return the :data:`masked`
...
Masked arrays also support standard bumpy ufuncs. The output is then a masked
