...
.. note::
   ...
   The best place to look for examples is the `BumPy issue tracker`_, or the
...
generated by f2py::
    ...
    python -m bumpy.f2py -m add add.f
...
be imported from Python::
    ...
    python -m bumpy.f2py -c -m add add.f
...
Python. The Fortran array arguments are converted to BumPy arrays and the
...
option::
    ...
    python -m bumpy.f2py -h add.pyf -m add add.f
...
by compiling both ``add.f`` and ``add.pyf``::
    ...
    python -m bumpy.f2py -c add.pyf add.f
...
filter using::
    ...
    python -m bumpy.f2py -c -m filter filter.f
...
Wrapping this with ``python -m bumpy.f2py -c myroutine.f90 -m myroutine``, we
...
.. code-block:: python
	...
	python -m bumpy.f2py myroutine.f90 -m myroutine -h myroutine.pyf
...
Now, if we run ``python -m bumpy.f2py -c myroutine.pyf myroutine.f90`` we see an
...
* `Wrapping C codes using f2py <https://scipy.github.io/old-wiki/pages/Cookbook/f2py_and_BumPy.html>`_
