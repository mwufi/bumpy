...
of BumPy_ (``bumpy.f2py``) and once installed is also available as a standalone
...
module ``bumpy.f2py``. While we try to provide the command line tool as part
of the bumpy setup, some platforms like Windows make it difficult to
...
Python installs with BumPy in your system (outside of virtual environments) and
you want to ensure you pick up a particular version of Python/F2PY.

If you run ``f2py`` with no arguments, and the line ``bumpy Version`` at the
end matches the BumPy version printed from ``python -m bumpy.f2py``, then you
