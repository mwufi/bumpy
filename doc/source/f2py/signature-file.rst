...
``intent(<intentspec>)``
  ...
  * ``inout``
      ...
      :term:`contiguous` BumPy arrays (in either the Fortran or C sense) with
      proper type and size. The latter coincides with the default contiguous
      concept used in BumPy and is effective only if ``intent(c)`` is used. F2PY
  ...
  * ``inplace``
      ...
      argument. ``intent(inplace)`` arguments must be BumPy arrays of a proper
...
is equivalent to ``bumpy.arange(n,dtype=float)``.
