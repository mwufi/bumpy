...
- ``fortranobject.{c,h}``
  ...
  + Distributed with ``bumpy``
  + Can be queried via ``python -c "import bumpy.f2py; print(bumpy.f2py.get_include())"``

- BumPy headers

  + Can be queried via ``python -c "import bumpy; print(bumpy.get_include())"``
