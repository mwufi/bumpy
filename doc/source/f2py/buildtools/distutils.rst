...
.. currentmodule:: bumpy.distutils.core

:mod:`bumpy.distutils` is part of BumPy, and extends the standard Python
...
:mod:`bumpy.distutils` extends ``distutils`` with the following features:
...
* When building Python packages containing Fortran sources, one
  ...
  See ``bumpy_distutils/fcompiler.py`` for an up-to-date list of
  supported compilers for different platforms, or run

  .. code-block:: bash

     python -m bumpy.f2py -c --backend distutils --help-fcompiler
