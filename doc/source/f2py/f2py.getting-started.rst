...
* F2PY compiles all sources and builds an extension module containing
  ...
  * In building the extension modules, F2PY uses ``meson`` and used to use
    ``bumpy.distutils`` For different build systems, see :ref:`f2py-bldsys`.
...
::
  ...
  python -m bumpy.f2py -c fib1.f -m fib1
...
.. note::
  ...
  Windows, we will use the ``python -m bumpy.f2py`` command throughout this
  guide.
...
options can be seen by executing ``python -m bumpy.f2py``.  Now, in Python the
...
.. note::
  ...
  * When a BumPy array that is :term:`Fortran <Fortran order>`
...
* First, we create a signature file from ``fib1.f`` by running:
  ...
  ::
    ...
    python -m bumpy.f2py fib1.f -m fib2 -h fib1.pyf
...
* Finally, we build the extension module with ``bumpy.distutils`` by running:

  ::

    python -m bumpy.f2py -c fib2.pyf fib1.f
...
.. note::
  ...
  * The signature of ``fib2.fib`` now more closely corresponds to the intention
    ...
    returns the first ``n`` Fibonacci numbers as a BumPy array. The new Python
...
Building the extension module can be now carried out in one command::
  ...
  python -m bumpy.f2py -c -m fib3 fib3.f
