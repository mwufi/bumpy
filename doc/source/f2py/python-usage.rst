...
argument, since ``str`` is applied for non-string objects. Exceptions are BumPy
...
arbitrary sequences that can be transformed to BumPy array objects. There are
...
In general, if a BumPy array is :term:`proper-contiguous <contiguous>` and has
...
them to Fortran routines, use the function `bumpy.asfortranarray`.
...
attributes return as BumPy array objects (multidimensional arrays are
