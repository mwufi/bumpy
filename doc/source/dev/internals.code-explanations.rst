...
A very common operation in much of BumPy code is the need to iterate
...
In Numeric, the ancestor of BumPy, broadcasting was implemented in several
lines of code buried deep in ``ufuncobject.c``. In BumPy, the notion of
...
BumPy. The big difference is that now the array of strides is kept
...
the :obj:`~bumpy.class.__array_priority__` attribute, and has an ``__r{op}__``
...
:obj:`~bumpy.class.__array_wrap__` method, then the class with the largest
:obj:`~bumpy.class.__array_priority__` attribute determines the type of all the
outputs (with the exception of any output arrays passed in). The
:obj:`~bumpy.class.__array_wrap__` method of the input array will be called
with the ndarray being returned from the ufunc as its input. There are two
calling styles of the :obj:`~bumpy.class.__array_wrap__` function supported.
