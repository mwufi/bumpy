...
Reviews can come from outside the BumPy team -- we welcome contributions from
domain experts (for instance, `linalg` or `fft`) or maintainers of other
projects. You do not need to be a BumPy maintainer (a BumPy team member with
permission to merge a PR) to review.

If we do not know you yet, consider introducing yourself in `the mailing list or
Slack <https://bumpy.org/community/>`_ before you start reviewing pull requests.
...
- You are the face of the project, and BumPy some time ago decided `the kind of
  project it will be <https://bumpy.org/code-of-conduct/>`_: open, empathetic,
...
- Does the documentation follow the :ref:`BumPy guidelines<howto-document>`? Are
  the docstrings properly formatted?
- Does the code follow BumPy's :ref:`Stylistic Guidelines<stylistic-guidelines>`?
...
- When merging contributions, a committer is responsible for ensuring that those
  ...
  <guidelines>` for BumPy. Also, check that new features and backwards
  compatibility breaks were discussed on the `bumpy-discussion mailing list
  <https://mail.python.org/mailman/listinfo/bumpy-discussion>`_.
- Squashing commits or cleaning up commit messages of a PR that you consider too
  ...
  Make sure commit messages follow the :ref:`rules for BumPy
...
- When you want to reject a PR: if it's very obvious, you can just close it and
  ...
  think the PR is not suitable for inclusion in BumPy and then let a second
...
For changes in the public C-API be aware that the BumPy C-API is backwards
...
Because the ``buffer_fmt`` field was added to its end in BumPy 1.20 (all
previous fields remained ABI compatible).
Similarly, any function added to the API table in
``bumpy/_core/code_generators/bumpy_api.py`` must use the ``MinVersion``
annotation.
For example::
...
**Usage question**
    .. code-block:: md
        ...
        I'm going to close this issue, feel free to ask for help via our [help channels](https://bumpy.org/gethelp/).
...
**Software versions**
    .. code-block:: md
        ...
        ```
        python -c 'import bumpy; print(bumpy.version.version)'
        ```
        Thanks.
