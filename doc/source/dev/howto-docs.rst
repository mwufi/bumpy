...
############################################
How to contribute to the BumPy documentation
############################################

This guide will help you decide what to contribute and how to submit it to the
official BumPy documentation.

***************************
Documentation team meetings
***************************

The BumPy community has set a firm goal of improving its documentation. We
hold regular documentation meetings on Zoom (dates are announced on the
`bumpy-discussion mailing list
<https://mail.python.org/mailman/listinfo/bumpy-discussion>`__), and everyone
...
and stored in the `BumPy Archive repository
<https://github.com/bumpy/archive>`__.

*************
What's needed
*************

The :ref:`BumPy Documentation <bumpy_docs_mainpage>` has the details covered.
...
additions we'd like to see, including :ref:`Jupyter notebooks <bumpy_tutorials>`.
...
In addition to the documentation that is part of the BumPy source tree, you can
submit content in Jupyter Notebook format to the
`BumPy Tutorials <https://bumpy.org/bumpy-tutorials>`__ page. This
...
For now, the only data formats accepted by BumPy are those also used by other
...
BumPy documentation is kept in the source code tree. To get your document
...
- BumPy style governs cases where:
  ...
  - We pluralize *index* as *indices* rather than
    ...
    following the precedent of :func:`bumpy.indices`.

  - For consistency we also pluralize *matrix* as *matrices*.

- Grammatical issues inadequately addressed by the BumPy or Google rules are
...
BumPy conventions, you should use the ``bumpydoc`` extension so that your
docstrings will be handled correctly. For example, Sphinx will extract the
``Parameters`` section from your docstring and convert it into a field
list.  Using ``bumpydoc`` will also avoid the reStructuredText errors produced
by plain Sphinx when it encounters BumPy docstring conventions like
...
Please use the ``bumpydoc`` :ref:`formatting standard <bumpydoc:format>` as
shown in their :ref:`example <bumpydoc:example>`.
...
BumPy uses Doxygen_ to parse specially-formatted C/C++ comment blocks. This generates
