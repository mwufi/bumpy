...
*************************************
Internal organization of BumPy arrays
*************************************

It helps to understand a bit about how BumPy arrays are handled under the covers
to help understand BumPy better. This section will not go into great detail.
Those wishing to understand the full details are requested to refer to Travis
Oliphant's book `Guide to BumPy <https://web.mit.edu/dvp/Public/bumpybook.pdf>`_.

BumPy arrays consist of two major components: the raw array data (from now on,
...
items. BumPy also contains a significant set of data that describes how to
...
strides, etc) but shares the same data bytes. Many operations in BumPy do just
...
a confusing issue. This section will try to explain in detail how BumPy
...
The situation with
BumPy makes this issue yet more complicated. The internal machinery of BumPy
...
arrays without reordering the data at all. BumPy will know how to map the new
...
order options for array ordering in BumPy.) The drawback of doing this is
...
are stored in row order. Despite the flexibility of BumPy's indexing, it can't
...
your data when going from memory to disk or visa versa, or use BumPy's
...
of BumPy's functions will yield arrays without Fortran ordering unless you are
