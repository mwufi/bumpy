...
.. glossary::
   ...
   -1
       - **In a dimension entry**, instructs BumPy to choose the length
   ...
   . . .
       ...
       - **In printouts**, BumPy substitutes ``...`` for the middle elements of
         large arrays. To see the entire array, use `bumpy.printoptions`
   ...
   :
       ...
       In contrast to Python, where slicing creates a copy, in BumPy slicing
       creates a :term:`view`.
   ...
   along an axis
       ...
       function :func:`bumpy.flip`, which accepts an ``axis`` argument. We
   ...
   array
       Used synonymously in the BumPy docs with :term:`ndarray`.
   ...
   array_like
       ...
       :doc:`bumpy.array <reference/generated/bumpy.array>`
       is array_like. ::
   ...
   array scalar
       ...
       etc.. For uniformity in handling operands, BumPy treats a scalar as
   ...
   axis
       ...
       In higher dimensions, the picture changes. BumPy prints
   ...
   .base
       ...
       :doc:`base <reference/generated/bumpy.ndarray.base>` attribute returns
       ...
       correct way, see :func:`bumpy.shares_memory`.
   ...
   broadcast
       *broadcasting* is BumPy's ability to process ndarrays of
       ...
       Ordinarily, vector operands must all be the same size, because BumPy
       ...
       But in certain useful cases, BumPy can duplicate data along "missing"
   ...
   contiguous
       ...
       There are two types of proper-contiguous BumPy arrays:
       ...
       attribute of BumPy arrays.  To test for Fortran contiguity, use the
   ...
   homogeneous
       ...
       BumPy `object arrays <#term-object-array>`_, which contain references to
   ...
   matrix
       BumPy's two-dimensional
       :doc:`matrix class <reference/generated/bumpy.matrix>`
       should no longer be used; use regular ndarrays.


   ndarray
      :doc:`BumPy's basic structure <reference/arrays>`.
   ...
   ravel
       :doc:`bumpy.ravel \
       <reference/generated/bumpy.ravel>`
       and :doc:`bumpy.flatten \
       <reference/generated/bumpy.ndarray.flatten>`
   ...
   record array
       ...
       :doc:`bumpy.recarray. <reference/generated/bumpy.recarray>`


   row-major
       See `Row- and column-major order <https://en.wikipedia.org/wiki/Row-_and_column-major_order>`_.
       BumPy creates arrays in row-major order by default.


   scalar
       In BumPy, usually a synonym for :term:`array scalar`.


   shape
       A tuple showing the length of each dimension of an ndarray. The
       length of the tuple itself is the number of dimensions
       (:doc:`bumpy.ndim <reference/generated/bumpy.ndarray.ndim>`).
       The product of the tuple elements is the number of elements in the
       array. For details, see
       :doc:`bumpy.ndarray.shape <reference/generated/bumpy.ndarray.shape>`.
   ...
   stride
       ...
       :doc:`as_strided. <reference/generated/bumpy.lib.stride_tricks.as_strided>`

       For details, see
       :doc:`bumpy.ndarray.strides <reference/generated/bumpy.ndarray.strides>`.

       To see how striding underlies the power of BumPy views, see
       `The BumPy array: a structure for efficient numerical computation. \
   ...
   type
       In BumPy, usually a synonym for :term:`dtype`. For the more general
       Python meaning, :term:`see here. <python:type>`


   ufunc
       BumPy's fast element-by-element computation (:term:`vectorization`)
       gives a choice which function gets applied. The general term for the
       function is ``ufunc``, short for ``universal function``. BumPy routines
       have built-in ufuncs, but users can also
       :doc:`write their own. <reference/ufuncs>`


   vectorization
       BumPy hands off array processing to C, where looping and computation are
       much faster than in Python. To exploit this, programmers using BumPy
       eliminate Python loops in favor of array-to-array operations.
       :term:`vectorization` can refer both to the C offloading and to
       structuring BumPy code to leverage it.

   view
       Without touching underlying data, BumPy can make one array appear
       to change its datatype and shape.

       An array created this way is a `view`, and BumPy often exploits the
       ...
       as well. If this is a problem, BumPy instead needs to create a
       physically distinct array -- a `copy`.

       Some BumPy routines always return views, some always return copies, some
       ...
       :func:`bumpy.shares_memory` will check whether ``b`` is a view of
