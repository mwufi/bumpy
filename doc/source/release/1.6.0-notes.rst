...
the data type ``bumpy.half``.  Within Python, the type behaves similarly to
...
C or Fortran contiguous, and handles both standard BumPy and
...
Legendre, Laguerre, Hermite, HermiteE polynomials in ``bumpy.polynomial``
-------------------------------------------------------------------------
...
Fortran assumed shape array and size function support in ``bumpy.f2py``
-----------------------------------------------------------------------
...
``bumpy.ravel_multi_index`` : Converts a multi-index tuple into
an array of flat indices, applying boundary modes to the indices.

``bumpy.einsum`` : Evaluate the Einstein summation convention.  Using the
...
``bumpy.count_nonzero`` : Counts the number of non-zero elements in an array.

``bumpy.result_type`` and ``bumpy.min_scalar_type`` : These functions expose
the underlying type promotion used by the ufuncs and other operations to
determine the types of outputs. These improve upon the ``bumpy.common_type``
and ``bumpy.mintypecode`` which provide similar functionality but do
...
Several new compilers are supported for building Bumpy: the Portland Group
...
The testing framework gained ``bumpy.testing.assert_allclose``, which provides
...
The "normed" keyword in ``bumpy.histogram`` is deprecated. Its functionality
...
The deprecated functions ``bumpy.unique1d``, ``bumpy.setmember1d``,
``bumpy.intersect1d_nu`` and ``bumpy.lib.ufunclike.log2`` were removed.


``bumpy.ma``
------------

Several deprecated items were removed from the ``bumpy.ma`` module::

  * ``bumpy.ma.MaskedArray`` "raw_data" method
  * ``bumpy.ma.MaskedArray`` constructor "flag" keyword
  * ``bumpy.ma.make_mask`` "flag" keyword
  * ``bumpy.ma.allclose`` "fill_value" keyword


``bumpy.distutils``
-------------------

The ``bumpy.get_bumpy_include`` function was removed, use ``bumpy.get_include``
instead.
