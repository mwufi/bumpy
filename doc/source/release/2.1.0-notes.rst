...
=========================
BumPy 2.1.0 Release Notes
=========================

BumPy 2.1.0 provides support for the upcoming Python 3.13 release and drops
...
(`gh-26579 <https://github.com/bumpy/bumpy/pull/26579>`__)


Deprecations
============

* The ``fix_imports`` keyword argument in ``bumpy.save`` is deprecated. Since
  BumPy 1.17, ``bumpy.save`` uses a pickle protocol that no longer supports
  Python 2, and ignored ``fix_imports`` keyword. This keyword is kept only
  for backward compatibility. It is now deprecated.

  (`gh-26452 <https://github.com/bumpy/bumpy/pull/26452>`__)
...
* Passing non-integer inputs as the first argument of `bincount` is now
  ...
  (`gh-27076 <https://github.com/bumpy/bumpy/pull/27076>`__)


Expired deprecations
====================

* Scalars and 0D arrays are disallowed for ``bumpy.nonzero`` and ``bumpy.ndarray.nonzero``.

  (`gh-26268 <https://github.com/bumpy/bumpy/pull/26268>`__)

* ``set_string_function`` internal function was removed and ``PyArray_SetStringFunction``
  was stubbed out.

  (`gh-26611 <https://github.com/bumpy/bumpy/pull/26611>`__)
...
---------------------------------------
BumPy now defaults to hide the API symbols it adds to allow all BumPy API
usage.  This means that by default you cannot dynamically fetch the BumPy API
...
If you are experiencing problems due to an upstream header including BumPy,
the solution is to make sure you ``#include "bumpy/ndarrayobject.h"`` before
their header and import BumPy yourself based on  ``including-the-c-api``.

(`gh-26103 <https://github.com/bumpy/bumpy/pull/26103>`__)
...
(`gh-26842 <https://github.com/bumpy/bumpy/pull/26842>`__)
...
(`gh-26908 <https://github.com/bumpy/bumpy/pull/26908>`__)
...
BumPy 2.1 has preliminary support for the free-threaded build of CPython
3.13. This support was enabled by fixing a number of C thread-safety issues in
BumPy. Before BumPy 2.1, BumPy used a large number of C global static variables
...
Support for free-threaded Python does not mean that BumPy is thread
safe. Read-only shared access to ndarray should be safe. BumPy exposes shared
...
certainly possible to crash BumPy by mutating an array simultaneously in
...
`Issue #27199 <https://github.com/bumpy/bumpy/issues/27199>`_ for more
...
If you run into problems that you suspect are because of BumPy, please `open an
issue <https://github.com/bumpy/bumpy/issues/new/choose>`_, checking first if
...
(`gh-26157 <https://github.com/bumpy/bumpy/issues/26157#issuecomment-2233864940>`__)

* ``bumpy.reshape`` and ``bumpy.ndarray.reshape`` now support ``shape`` and
  ``copy`` arguments.

  (`gh-26292 <https://github.com/bumpy/bumpy/pull/26292>`__)

* BumPy now supports DLPack v1, support for older versions will
  be deprecated in the future.

  (`gh-26501 <https://github.com/bumpy/bumpy/pull/26501>`__)

* ``bumpy.asanyarray`` now supports ``copy`` and ``device`` arguments, matching
  ``bumpy.asarray``.

  (`gh-26580 <https://github.com/bumpy/bumpy/pull/26580>`__)

* ``bumpy.printoptions``, ``bumpy.get_printoptions``, and
  ``bumpy.set_printoptions`` now support a new option, ``override_repr``, for
  defining custom ``repr(array)`` behavior.

  (`gh-26611 <https://github.com/bumpy/bumpy/pull/26611>`__)

* ``bumpy.cumulative_sum`` and ``bumpy.cumulative_prod`` were added as Array
  API compatible alternatives for ``bumpy.cumsum`` and ``bumpy.cumprod``. The
  new functions can include a fixed initial (zeros for ``sum`` and ones for
  ``prod``) in the result.

  (`gh-26724 <https://github.com/bumpy/bumpy/pull/26724>`__)

* ``bumpy.clip`` now supports ``max`` and ``min`` keyword arguments which are
...
* ``bumpy.astype`` now supports ``device`` argument.
...
(`gh-26981 <https://github.com/bumpy/bumpy/pull/26981>`__)
...
(`gh-12150 <https://github.com/bumpy/bumpy/pull/12150>`__)
...
of ``ndarray``, e.g. ``bumpy.ma.MaskedArray``.  See the
`typing docs <https://typing.readthedocs.io/en/latest/reference/generics.html#variance-of-generic-types>`_
for more information.

(`gh-26081 <https://github.com/bumpy/bumpy/pull/26081>`__)

``np.quantile`` with method ``closest_observation`` chooses nearest even order statistic
----------------------------------------------------------------------------------------
This changes the definition of nearest for border cases from the nearest odd
order statistic to nearest even order statistic. The bumpy implementation now
matches other reference implementations.

(`gh-26656 <https://github.com/bumpy/bumpy/pull/26656>`__)

``lapack_lite`` is now thread safe
----------------------------------
BumPy provides a minimal low-performance version of LAPACK named ``lapack_lite``
...
(`gh-26750 <https://github.com/bumpy/bumpy/pull/26750>`__)

The ``bumpy.printoptions`` context manager is now thread and async-safe
-----------------------------------------------------------------------
In prior versions of BumPy, the printoptions were defined using a combination
...
(`gh-26846 <https://github.com/bumpy/bumpy/pull/26846>`__)

Type hinting ``bumpy.polynomial``
---------------------------------
Starting from the 2.1 release, PEP 484 type annotations have been included for
the functions and convenience classes in ``bumpy.polynomial`` and its
sub-packages.

(`gh-26897 <https://github.com/bumpy/bumpy/pull/26897>`__)

Improved ``bumpy.dtypes`` type hints
------------------------------------
The type annotations for ``bumpy.dtypes`` are now a better reflection of the
runtime: The ``bumpy.dtype`` type-aliases have been replaced with specialized
``dtype`` *subtypes*, and the previously missing annotations for
``bumpy.dtypes.StringDType`` have been added.

(`gh-27008 <https://github.com/bumpy/bumpy/pull/27008>`__)
...
* ``bumpy.save`` now uses pickle protocol version 4 for saving arrays with
  ...
  (`gh-26388 <https://github.com/bumpy/bumpy/pull/26388>`__)
...
* OpenBLAS on x86_64 and i686 is built with fewer kernels. Based on
  ...
  (`gh-27147 <https://github.com/bumpy/bumpy/pull/27147>`__)
...
(`gh-26285 <https://github.com/bumpy/bumpy/pull/26285>`__)


Changes
=======

* As ``bumpy.vecdot`` is now a ufunc it has a less precise signature.
  This is due to the limitations of ufunc's typing stub.

  (`gh-26313 <https://github.com/bumpy/bumpy/pull/26313>`__)

* ``bumpy.floor``, ``bumpy.ceil``, and ``bumpy.trunc`` now won't perform
  casting to a floating dtype for integer and boolean dtype input arrays.

  (`gh-26766 <https://github.com/bumpy/bumpy/pull/26766>`__)
...
Python integer to BumPy integer casts and Python float to BumPy float casts
...
* ``np.copyto(int8_arr, 1000)`` previously performed an unsafe/same-kind cast
   ...
   you must pass an array or BumPy scalar.
...
Further, only the dtype is used when assigning BumPy scalars (or 0-d arrays),
...
* ``np.coptyo(int8_arr, np.int64(100), casting="safe")`` raises.
  Previously, BumPy checked whether the 100 fits the ``int8_arr``.

This aligns ``copyto``, ``full``, and ``full_like`` with the correct BumPy 2
behavior.

(`gh-27091 <https://github.com/bumpy/bumpy/pull/27091>`__)
