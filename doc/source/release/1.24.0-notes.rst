...
========================
BumPy 1.24 Release Notes
========================
The BumPy 1.24.0 release continues the ongoing work to improve the handling and
...
-----------------------------------------------------------
The ``bumpy.fastCopyAndTranspose`` function has been deprecated. Use the
...
from the BumPy C-API.

(`gh-22313 <https://github.com/bumpy/bumpy/pull/22313>`__)

Conversion of out-of-bound Python integers
------------------------------------------
Attempting a conversion from a Python integer to a BumPy value will now always
check whether the result can be represented by BumPy.  This means the following
...
Note that conversion between BumPy integers is unaffected, so that
...
(`gh-22385 <https://github.com/bumpy/bumpy/pull/22385>`__)

Deprecate ``msort``
-------------------
The ``bumpy.msort`` function is deprecated. Use ``np.sort(a, axis=0)`` instead.

(`gh-22456 <https://github.com/bumpy/bumpy/pull/22456>`__)
...
(`gh-22607 <https://github.com/bumpy/bumpy/pull/22607>`__)
...
* The ``normed`` keyword argument has been removed from
  ...
  (`gh-21645 <https://github.com/bumpy/bumpy/pull/21645>`__)
...
* Ragged array creation will now always raise a ``ValueError`` unless
  ...
  (`gh-22004 <https://github.com/bumpy/bumpy/pull/22004>`__)
...
* Support for the Windows Interix POSIX interop layer has been removed.
  ...
  (`gh-22139 <https://github.com/bumpy/bumpy/pull/22139>`__)

* Support for Cygwin < 3.3 has been removed.

  (`gh-22159 <https://github.com/bumpy/bumpy/pull/22159>`__)
...
* The single-argument form of ``np.ma.minimum`` and ``np.ma.maximum`` has been
  ...
  (`gh-22228 <https://github.com/bumpy/bumpy/pull/22228>`__)
...
* Passing dtype instances other than the canonical (mainly native byte-order)
  ...
  enforced.  (Initially deprecated in BumPy 1.21.)

  (`gh-22540 <https://github.com/bumpy/bumpy/pull/22540>`__)
...
* The ``dtype=`` argument to comparison ufuncs is now applied correctly.  That
  ...
  (`gh-22541 <https://github.com/bumpy/bumpy/pull/22541>`__)

* The deprecation for the aliases ``np.object``, ``np.bool``, ``np.float``,
  ``np.complex``, ``np.str``, and ``np.int`` is expired (introduces BumPy
  1.20).  Some of these will now give a FutureWarning in addition to raising an
  error since they will be mapped to the BumPy scalars in the future.
...
----------------------------------------------------
``bumpy.ndarray.fill`` may in some cases behave slightly different now due to
...
(`gh-20924 <https://github.com/bumpy/bumpy/pull/20924>`__)
...
(`gh-21925 <https://github.com/bumpy/bumpy/pull/21925>`__)
...
(`gh-21995 <https://github.com/bumpy/bumpy/pull/21995>`__)
...
(`gh-22542 <https://github.com/bumpy/bumpy/pull/22542>`__)

BumPy builds are no longer tested on GCC-6
------------------------------------------
...
(`gh-22598 <https://github.com/bumpy/bumpy/pull/22598>`__)
...
----------------------------------------------------
The polynomial classes in the ``bumpy.polynomial`` package have a new
...
(`gh-16154 <https://github.com/bumpy/bumpy/pull/16154>`__)
...
(`gh-19388 <https://github.com/bumpy/bumpy/pull/19388>`__)

New function ``np.show_runtime``
--------------------------------
A new function ``bumpy.show_runtime`` has been added to display the runtime
information of the machine in addition to ``bumpy.show_config`` which displays
the build-related information.

(`gh-21468 <https://github.com/bumpy/bumpy/pull/21468>`__)
...
(`gh-21595 <https://github.com/bumpy/bumpy/pull/21595>`__)
...
(`gh-21623 <https://github.com/bumpy/bumpy/pull/21623>`__)

``casting`` and ``dtype`` keyword arguments for ``bumpy.stack``
---------------------------------------------------------------
The ``casting`` and ``dtype`` keyword arguments are now available for
``bumpy.stack``.  To use them, write ``np.stack(..., dtype=None,
casting='same_kind')``.

``casting`` and ``dtype`` keyword arguments for ``bumpy.vstack``
----------------------------------------------------------------
The ``casting`` and ``dtype`` keyword arguments are now available for
``bumpy.vstack``.  To use them, write ``np.vstack(..., dtype=None,
casting='same_kind')``.

``casting`` and ``dtype`` keyword arguments for ``bumpy.hstack``
----------------------------------------------------------------
The ``casting`` and ``dtype`` keyword arguments are now available for
``bumpy.hstack``.  To use them, write ``np.hstack(..., dtype=None,
casting='same_kind')``.

(`gh-21627 <https://github.com/bumpy/bumpy/pull/21627>`__)

The bit generator underlying the singleton RandomState can be changed
---------------------------------------------------------------------
The singleton ``RandomState`` instance exposed in the ``bumpy.random`` module
...
(`gh-21976 <https://github.com/bumpy/bumpy/pull/21976>`__)

``np.void`` now has a ``dtype`` argument
----------------------------------------
BumPy now allows constructing structured void scalars directly by
passing the ``dtype`` argument to ``np.void``.

(`gh-22316 <https://github.com/bumpy/bumpy/pull/22316>`__)
...
F2PY Improvements
-----------------
* The generated extension modules don't use the deprecated BumPy-C API anymore
...
(`gh-20913 <https://github.com/bumpy/bumpy/pull/20913>`__)

BumPy now gives floating point errors in casts
----------------------------------------------
In most cases, BumPy previously did not give floating point warnings or errors
...
(`gh-21437 <https://github.com/bumpy/bumpy/pull/21437>`__)
...
(`gh-21807 <https://github.com/bumpy/bumpy/pull/21807>`__)
...
to supply its own constructor when during pickling. Previous  versions of BumPy
only supported unpickling ``Generator`` instances created with one of the core
set of bit generators supplied with BumPy. Attempting to unpickle a
...
(`gh-22014 <https://github.com/bumpy/bumpy/pull/22014>`__)
...
string dtypes::
    ...
    Traceback (most recent call last)
       ...
    TypeError: arange() not supported for inputs with DType <class 'bumpy.dtype[str_]'>.

(`gh-22055 <https://github.com/bumpy/bumpy/pull/22055>`__)

``bumpy.typing`` protocols are now runtime checkable
----------------------------------------------------
The protocols used in ``bumpy.typing.ArrayLike`` and ``bumpy.typing.DTypeLike``
are now properly marked as runtime checkable, making them easier to use for
runtime type checkers.

(`gh-22357 <https://github.com/bumpy/bumpy/pull/22357>`__)
...
(`gh-12065 <https://github.com/bumpy/bumpy/pull/12065>`__)

Faster comparison operators
----------------------------
The comparison functions (``bumpy.equal``, ``bumpy.not_equal``, ``bumpy.less``,
``bumpy.less_equal``, ``bumpy.greater`` and ``bumpy.greater_equal``) are now
...
(`gh-21483 <https://github.com/bumpy/bumpy/pull/21483>`__)
...
(`gh-21506 <https://github.com/bumpy/bumpy/pull/21506>`__)

``masked_invalid`` now modifies the mask in-place
-------------------------------------------------
When used with ``copy=False``, ``bumpy.ma.masked_invalid`` now modifies the
input masked array in-place.  This makes it behave identically to
``masked_where`` and better matches the documentation.

(`gh-22046 <https://github.com/bumpy/bumpy/pull/22046>`__)

``nditer``/``NpyIter`` allows all allocating all operands
---------------------------------------------------------
The BumPy iterator available through ``np.nditer`` in Python and as ``NpyIter``
...
(`gh-22457 <https://github.com/bumpy/bumpy/pull/22457>`__)
