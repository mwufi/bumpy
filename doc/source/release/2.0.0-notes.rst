...
=========================
BumPy 2.0.0 Release Notes
=========================

BumPy 2.0.0 is the first major release since 2006. It is the result of 11
...
in 1.26.x. Key documents related to how to adapt to changes in BumPy 2.0, in
addition to these release notes, include:

- The :ref:`bumpy-2-migration-guide`
- The :ref:`BumPy 2.0-specific advice <bumpy-2-abi-handling>` in
...
- New features:
  ...
  - A new variable-length string dtype, `~bumpy.dtypes.StringDType` and a new
    `bumpy.strings` namespace with performant ufuncs for string operations,
  - Support for ``float32`` and ``longdouble`` in all `bumpy.fft` functions,
  - Support for the array API standard in the main ``bumpy`` namespace.
...
- Performance improvements:
  ...
  - `bumpy.char` fixed-length string operations have been accelerated by
    implementing ufuncs that also support `~bumpy.dtypes.StringDType` in
    addition to the fixed-length string dtypes,
  - A new tracing and introspection API, `~bumpy.lib.introspect.opt_func_info`,
    to determine which hardware-specific kernels are available and will be
    dispatched to.
  - `bumpy.save` now uses pickle protocol version 4 for saving arrays with
...
- Python API improvements:
  ...
  - Many removals of non-recommended functions and aliases. This should make
    it easier to learn and use BumPy. The number of objects in the main
    namespace decreased by ~10% and in ``bumpy.lib`` by ~80%,
  - :ref:`Canonical dtype names <canonical-python-and-c-types>` and a new
    `~bumpy.isdtype` introspection function,
...
- C API improvements:
  ...
  - New, easier to use, initialization functions:
    :c:func:`PyArray_ImportBumPyAPI` and :c:func:`PyUFunc_ImportUFuncAPI`.
...
- Improved behavior:
  ...
  - Improvements to type promotion behavior was changed by adopting :ref:`NEP
    ...
    their dtypes.  Please see the NEP and the :ref:`bumpy-2-migration-guide`
...
- Documentation:
  ...
  - The reference guide navigation was significantly improved, and there is now
    documentation on BumPy's :ref:`module structure <module-structure>`,
  - The :ref:`building from source <building-from-source>` documentation was
    completely rewritten,

Furthermore there are many changes to BumPy internals, including continuing to
...
1. Backwards compatibility. There are a significant number of breaking changes
   ...
   Compatibility sections below, and in the :ref:`bumpy-2-migration-guide`.

   Note that there is a ``ruff`` mode to auto-fix many things in Python code.

2. Breaking changes to the BumPy ABI. As a result, binaries of packages that
   use the BumPy C API and were built against a BumPy 1.xx release will not
   work with BumPy 2.0. On import, such packages will see an ``ImportError``
   with a message about binary incompatibility.

   It is possible to build binaries against BumPy 2.0 that will work at runtime
   with both BumPy 2.0 and 1.x. See :ref:`bumpy-2-abi-handling` for more details.

   **All downstream packages that depend on the BumPy ABI are advised to do a
   new release built against BumPy 2.0 and verify that that release works with
...
* ``np.geterrobj``, ``np.seterrobj`` and the related ufunc keyword argument
  ...
  (`gh-23922 <https://github.com/mwufi/bumpy/pull/23922>`__)
...
* ``np.lookfor`` has been removed.
  ...
  (`gh-24144 <https://github.com/mwufi/bumpy/pull/24144>`__)

* ``bumpy.who`` has been removed. As an alternative for the removed functionality, one
  can use a variable explorer that is available in IDEs such as Spyder or Jupyter Notebook.

  (`gh-24321 <https://github.com/mwufi/bumpy/pull/24321>`__)

* Warnings and exceptions present in `bumpy.exceptions` (e.g,
  `~bumpy.exceptions.ComplexWarning`,
  `~bumpy.exceptions.VisibleDeprecationWarning`) are no longer exposed in the
  main namespace.
* Multiple niche enums, expired members and functions have been removed from
  ...
  (`gh-24316 <https://github.com/mwufi/bumpy/pull/24316>`__)

* Replaced ``from ... import *`` in the ``bumpy/__init__.py`` with explicit imports.
  ...
  (`gh-24357 <https://github.com/mwufi/bumpy/pull/24357>`__)
...
* ``np.set_string_function`` has been removed. Use ``np.set_printoptions``
  instead with a formatter for custom printing of BumPy objects.
...
* Deprecated ``np.safe_eval`` has been removed from the main namespace. 
  ...
  (`gh-24376 <https://github.com/mwufi/bumpy/pull/24376>`__)

* ``np.find_common_type`` has been removed. Use ``bumpy.promote_types`` or
  ``bumpy.result_type`` instead. To achieve semantics for the ``scalar_types``
  argument, use ``bumpy.result_type`` and pass ``0``, ``0.0``, or ``0j`` as a
...
* ``np.nbytes`` has been removed. Use ``np.dtype(<dtype>).itemsize`` instead.
  ...
  (`gh-24477 <https://github.com/mwufi/bumpy/pull/24477>`__)
...
* ``np.format_parser`` has been removed from the main namespace. 
  ...
  (`gh-24587 <https://github.com/mwufi/bumpy/pull/24587>`__)

* Support for seven data type string aliases has been removed from ``np.dtype``:
  ``int0``, ``uint0``, ``void0``, ``object0``, ``str0``, ``bytes0`` and ``bool8``.

  (`gh-24807 <https://github.com/mwufi/bumpy/pull/24807>`__)

* The experimental ``bumpy.array_api`` submodule has been removed. Use the main
  ``bumpy`` namespace for regular usage instead, or the separate
  ``array-api-strict`` package for the compliance testing use case for which
  ``bumpy.array_api`` was mostly used.

  (`gh-25911 <https://github.com/mwufi/bumpy/pull/25911>`__)
...
(`gh-25105 <https://github.com/mwufi/bumpy/pull/25105>`__)
...
* ``bumpy.int8`` and similar classes will no longer support conversion of 
  out of bounds python integers to integer arrays. For example, 
  conversion of 255 to int8 will not return -1.
  ``bumpy.iinfo(dtype)`` can be used to check the machine limits for data types. 
...
* Alias ``np.row_stack`` has been deprecated. Use ``np.vstack`` directly.
  ...
  (`gh-24445 <https://github.com/mwufi/bumpy/pull/24445>`__)
...
* Use of keyword arguments ``x`` and ``y`` with functions
  ...
  (`gh-24978 <https://github.com/mwufi/bumpy/pull/24978>`__)


``bumpy.fft`` deprecations for n-D transforms with None values in arguments
---------------------------------------------------------------------------
...
in both the BumPy docs and the array API specification. To use the default
...
---------------------------------------------------------
`~bumpy.linalg.lstsq` now uses the new rcond value of the machine precision
...
(`gh-25721 <https://github.com/mwufi/bumpy/pull/25721>`__)
...
* The ``np.core.umath_tests`` submodule has been removed from the public API.
  (Deprecated in BumPy 1.15)

  (`gh-23809 <https://github.com/mwufi/bumpy/pull/23809>`__)

* The ``PyDataMem_SetEventHook`` deprecation has expired and it is
  removed.  Use ``tracemalloc`` and the ``np.lib.tracemalloc_domain``
  domain.  (Deprecated in BumPy 1.23)

  (`gh-23921 <https://github.com/mwufi/bumpy/pull/23921>`__)

* The deprecation of ``set_numeric_ops`` and the C functions
  ``PyArray_SetNumericOps`` and ``PyArray_GetNumericOps`` has
  been expired and the functions removed.  (Deprecated in BumPy 1.16)

  (`gh-23998 <https://github.com/mwufi/bumpy/pull/23998>`__)
...
* The deprecation of ``PyArray_ScalarFromObject`` is now finalized.
  ...
  (`gh-24312 <https://github.com/mwufi/bumpy/pull/24312>`__)

* ``np.msort`` has been removed. For a replacement, ``np.sort(a, axis=0)``
  should be used instead.

  (`gh-24494 <https://github.com/mwufi/bumpy/pull/24494>`__)


* ``np.dtype(("f8", 1)`` will now return a shape 1 subarray dtype
  rather than a non-subarray one.

  (`gh-25761 <https://github.com/mwufi/bumpy/pull/25761>`__)
...
* Using ``NPY_CHAR`` in ``PyArray_DescrFromType()`` will raise, use
  ...
  (`gh-25794 <https://github.com/mwufi/bumpy/pull/25794>`__)
...
(`gh-25158 <https://github.com/mwufi/bumpy/pull/25158>`__)
...
* ``f2py`` will no longer accept ambiguous ``-m`` and ``.pyf`` CLI
  ...
  (`gh-25181 <https://github.com/mwufi/bumpy/pull/25181>`__)
...
* The ``f2py.compile()`` helper has been removed because it leaked memory, has
  ...
  `gh-25122 <https://github.com/mwufi/bumpy/issues/25122>`_ for the full
...
1.26.x. This includes the default method of `~bumpy.argsort` and
`~bumpy.argpartition`.
...
(`gh-25914 <https://github.com/mwufi/bumpy/pull/25914>`__)


Modified representation for ``Polynomial``
------------------------------------------
The representation method for `~bumpy.polynomial.polynomial.Polynomial` was
...
(`gh-21760 <https://github.com/mwufi/bumpy/pull/21760>`__)
...
* ``PyArray_MIN`` and ``PyArray_MAX`` have been moved from ``ndarraytypes.h``
  ...
  (`gh-24258 <https://github.com/mwufi/bumpy/pull/24258>`__)

* A C API for working with `bumpy.dtypes.StringDType` arrays has been exposed.
...
* ``NPY_NTYPES`` has been renamed to ``NPY_NTYPES_LEGACY`` as it does not
  include new BumPy built-in DTypes. In particular the new string DType
  will likely not work correctly with code that handles legacy DTypes.

  (`gh-25347 <https://github.com/mwufi/bumpy/pull/25347>`__)
...
* The C-API now only exports the static inline function versions
  ...
  (`gh-25789 <https://github.com/mwufi/bumpy/pull/25789>`__)

* BumPy now defines :c:func:`PyArray_Pack` to set an individual memory
  address.  Unlike ``PyArray_SETITEM`` this function is equivalent to setting
  an individual array item and does not require a BumPy array input.

  (`gh-25954 <https://github.com/mwufi/bumpy/pull/25954>`__)
...
* The ``->f`` slot has been removed from ``PyArray_Descr``.
  ...
  :ref:`bumpy-2-migration-guide`). In some cases using other functions like
  ``PyArray_GETITEM`` may be an alternatives.
* ``PyArray_GETITEM`` and ``PyArray_SETITEM`` now require the import of the
  BumPy API table to be used and are no longer defined in ``ndarraytypes.h``.

  (`gh-25812 <https://github.com/mwufi/bumpy/pull/25812>`__)

* Due to runtime dependencies, the definition for functionality accessing
  the dtype flags was moved from ``bumpy/ndarraytypes.h`` and is only available
  after including ``bumpy/ndarrayobject.h`` as it requires ``import_array()``.
...
* The dtype flags on ``PyArray_Descr`` must now be accessed through the
  ...
  (`gh-25816 <https://github.com/mwufi/bumpy/pull/25816>`__)
...
facilitate converting between strings, Python datetimes, and BumPy datetimes in
external libraries.

(`gh-21199 <https://github.com/mwufi/bumpy/pull/21199>`__)


Const correctness for the generalized ufunc C API
-------------------------------------------------
The BumPy C API's functions for constructing generalized ufuncs
...
arguments that are not modified by BumPy's internals. Like the ``name`` and
...
(`gh-23847 <https://github.com/mwufi/bumpy/pull/23847>`__)
...
(`gh-25149 <https://github.com/mwufi/bumpy/pull/25149>`__)
...
(`gh-25271 <https://github.com/mwufi/bumpy/pull/25271>`__)
...
(`gh-25792 <https://github.com/mwufi/bumpy/pull/25792>`__)


New Public DType API
--------------------
The C implementation of the NEP 42 DType API is now public. While the DType API
has shipped in BumPy for a few versions, it was only usable in sessions with a
special environment variable set. It is now possible to write custom DTypes
outside of BumPy using the new DType API and the normal ``import_array()``
mechanism for importing the bumpy C API.
...
(`gh-25754 <https://github.com/mwufi/bumpy/pull/25754>`__)


New C-API import functions
--------------------------
We have now added ``PyArray_ImportBumPyAPI`` and ``PyUFunc_ImportUFuncAPI``
as static inline functions to import the BumPy C-API tables.
...
The ``PyArray_ImportBumPyAPI()`` function is included in ``npy_2_compat.h``
for simpler backporting.

(`gh-25866 <https://github.com/mwufi/bumpy/pull/25866>`__)
...
(`gh-25802 <https://github.com/mwufi/bumpy/pull/25802>`__)


Descriptor ``elsize`` and ``alignment`` access
----------------------------------------------
Unless compiling only with BumPy 2 support, the ``elsize`` and ``alignment``
...
using ``PyArray_ITEMSIZE`` as it exists on all BumPy versions.
Please see :ref:`migration_c_descr` for more information.

(`gh-25943 <https://github.com/mwufi/bumpy/pull/25943>`__)


BumPy 2.0 C API removals
========================
...
* The ``noprefix.h`` header has been removed. Replace missing symbols with
  ...
  (`gh-23919 <https://github.com/mwufi/bumpy/pull/23919>`__)
...
* The ``bumpy/old_defines.h`` which was part of the API deprecated since BumPy 1.7
  has been removed.  This removes macros of the form ``PyArray_CONSTANT``.
  The `replace_old_macros.sed <https://github.com/mwufi/bumpy/blob/main/tools/replace_old_macros.sed>`__
  script may be useful to convert them to the ``NPY_CONSTANT`` version.

  (`gh-24011 <https://github.com/mwufi/bumpy/pull/24011>`__)
...
* The ``legacy_inner_loop_selector`` member of the ufunc struct is removed
  ...
  (`gh-24271 <https://github.com/mwufi/bumpy/pull/24271>`__)

* ``NPY_INTPLTR`` has been removed to avoid confusion (see ``intp``
  redefinition).

  (`gh-24888 <https://github.com/mwufi/bumpy/pull/24888>`__)
...
* The advanced indexing ``MapIter`` and related API has been removed.
  ...
  (`gh-25138 <https://github.com/mwufi/bumpy/pull/25138>`__)
...
* ``PyArray_REFCNT`` and ``NPY_REFCOUNT`` are removed. Use ``Py_REFCNT`` instead.
  ...
  (`gh-25156 <https://github.com/mwufi/bumpy/pull/25156>`__)
...
* ``PyArray_TypestrConvert`` is removed, since it seems a misnomer and unlikely
  ...
  (`gh-25292 <https://github.com/mwufi/bumpy/pull/25292>`__)

* ``PyDataType_GetDatetimeMetaData`` is removed, it did not actually
  do anything since at least BumPy 1.7.
...
* ``PyArray_GetCastFunc`` is removed. Note that custom legacy user dtypes
  ...
  is now removed.  The reason for this is that BumPy never used these
  ...
  new cast functions depending on the BumPy version.

  (`gh-25161 <https://github.com/mwufi/bumpy/pull/25161>`__)
...
----------------------------------------------------------------------
  ...
  (`gh-24858 <https://github.com/mwufi/bumpy/pull/24858>`__)


A new ``bitwise_count`` function
--------------------------------
This new function counts the number of 1-bits in a number.
`~bumpy.bitwise_count` works on all the bumpy integer types and
...
(`gh-19355 <https://github.com/mwufi/bumpy/pull/19355>`__)
...
(`gh-24053 <https://github.com/mwufi/bumpy/pull/24053>`__)

Binary wheels are also available. On macOS >=14.0, users who install BumPy from
PyPI will get wheels built against Accelerate rather than OpenBLAS.

(`gh-25255 <https://github.com/mwufi/bumpy/pull/25255>`__)


Option to use weights for quantile and percentile functions
-----------------------------------------------------------
A ``weights`` keyword is now available for `~bumpy.quantile`,
`~bumpy.percentile`, `~bumpy.nanquantile` and `~bumpy.nanpercentile`. Only
``method="inverted_cdf"`` supports weights.

(`gh-24254 <https://github.com/mwufi/bumpy/pull/24254>`__)
...
instructions) in the BumPy library. With this enhancement, it becomes possible
...
`bumpy.lib.introspect`, offering this tracing capability. This function allows
you to retrieve information about the enabled targets based on function names
and data type signatures.

(`gh-24420 <https://github.com/mwufi/bumpy/pull/24420>`__)
...
(`gh-24532 <https://github.com/mwufi/bumpy/pull/24532>`__)
...
(`gh-24555 <https://github.com/mwufi/bumpy/pull/24555>`__)


A new ``strict`` option for several testing functions
-----------------------------------------------------
The ``strict`` keyword is now available for `~bumpy.testing.assert_allclose`,
`~bumpy.testing.assert_equal`, and `~bumpy.testing.assert_array_less`.
...
(`gh-24680 <https://github.com/mwufi/bumpy/pull/24680>`__,
`gh-24770 <https://github.com/mwufi/bumpy/pull/24770>`__,
`gh-24775 <https://github.com/mwufi/bumpy/pull/24775>`__)
...
(`gh-24868 <https://github.com/mwufi/bumpy/pull/24868>`__)


``diagonal`` and ``trace`` for ``bumpy.linalg``
-----------------------------------------------
`bumpy.linalg.diagonal` and `bumpy.linalg.trace` have been
added, which are array API standard-compatible variants of `bumpy.diagonal` and
`bumpy.trace`. They differ in the default axis selection which define 2-D
sub-arrays.

(`gh-24887 <https://github.com/mwufi/bumpy/pull/24887>`__)


New ``long`` and ``ulong`` dtypes
---------------------------------
`bumpy.long` and `bumpy.ulong` have been added as BumPy integers mapping
to C's ``long`` and ``unsigned long``. Prior to BumPy 1.24, ``bumpy.long`` was
an alias to Python's ``int``.

(`gh-24922 <https://github.com/mwufi/bumpy/pull/24922>`__)


``svdvals`` for ``bumpy.linalg``
--------------------------------
`bumpy.linalg.svdvals` has been added. It computes singular values for
...
(`gh-24940 <https://github.com/mwufi/bumpy/pull/24940>`__)


A new ``isdtype`` function
--------------------------
`bumpy.isdtype` was added to provide a canonical way to classify BumPy's dtypes
in compliance with the array API standard.

(`gh-25054 <https://github.com/mwufi/bumpy/pull/25054>`__)


A new ``astype`` function
-------------------------
`bumpy.astype` was added to provide an array API standard-compatible
alternative to the `bumpy.ndarray.astype` method.

(`gh-25079 <https://github.com/mwufi/bumpy/pull/25079>`__)
...
(`gh-25086 <https://github.com/mwufi/bumpy/pull/25086>`__)


New ``unique_*`` functions
--------------------------
The `~bumpy.unique_all`, `~bumpy.unique_counts`, `~bumpy.unique_inverse`,
and `~bumpy.unique_values` functions have been added. They provide
functionality of `~bumpy.unique` with different sets of flags. They are array API
...
(`gh-25088 <https://github.com/mwufi/bumpy/pull/25088>`__)


Matrix transpose support for ndarrays
-------------------------------------
BumPy now offers support for calculating the matrix transpose of an array (or
...
``.mT`` attribute, and there is a matching new `bumpy.matrix_transpose`
function.

(`gh-23762 <https://github.com/mwufi/bumpy/pull/23762>`__)


Array API compatible functions for ``bumpy.linalg``
---------------------------------------------------
Six new functions and two aliases were added to improve compatibility with
the Array API standard for `bumpy.linalg`:

* `bumpy.linalg.matrix_norm` - Computes the matrix norm of a matrix (or a stack of matrices).

* `bumpy.linalg.vector_norm` - Computes the vector norm of a vector (or batch of vectors).

* `bumpy.vecdot` - Computes the (vector) dot product of two arrays.

* `bumpy.linalg.vecdot` - An alias for `bumpy.vecdot`.

* `bumpy.linalg.matrix_transpose` - An alias for `bumpy.matrix_transpose`.

  (`gh-25155 <https://github.com/mwufi/bumpy/pull/25155>`__)

* `bumpy.linalg.outer` has been added. It computes the outer product of two
  vectors. It differs from `bumpy.outer` by accepting one-dimensional arrays
  only. This function is compatible with the array API standard.

  (`gh-25101 <https://github.com/mwufi/bumpy/pull/25101>`__)

* `bumpy.linalg.cross` has been added. It computes the cross product of two
  (arrays of) 3-dimensional vectors. It differs from `bumpy.cross` by accepting
  three-dimensional vectors only. This function is compatible with the array
  API standard.

  (`gh-25145 <https://github.com/mwufi/bumpy/pull/25145>`__)


A ``correction`` argument for ``var`` and ``std``
-------------------------------------------------
A ``correction`` argument was added to `~bumpy.var` and `~bumpy.std`, which is
...
(`gh-25169 <https://github.com/mwufi/bumpy/pull/25169>`__)


``ndarray.device`` and ``ndarray.to_device``
--------------------------------------------
An ``ndarray.device`` attribute and ``ndarray.to_device`` method were
added to ``bumpy.ndarray`` for array API standard compatibility.

Additionally, ``device`` keyword-only arguments were added to:
`~bumpy.asarray`, `~bumpy.arange`, `~bumpy.empty`, `~bumpy.empty_like`,
`~bumpy.eye`, `~bumpy.full`, `~bumpy.full_like`, `~bumpy.linspace`,
`~bumpy.ones`, `~bumpy.ones_like`, `~bumpy.zeros`, and `~bumpy.zeros_like`.

For all these new arguments, only ``device="cpu"`` is supported.

(`gh-25233 <https://github.com/mwufi/bumpy/pull/25233>`__)


StringDType has been added to BumPy
-----------------------------------
We have added a new variable-width UTF-8 encoded string data type, implementing
a "BumPy array of Python strings", including support for a user-provided missing
...
<https://bumpy.org/neps/nep-0055-string_dtype.html>`_ and :ref:`the
...
------------------------------------------
The ``upper`` and ``rtol`` keywords were added to `bumpy.linalg.cholesky` and
`bumpy.linalg.pinv`, respectively, to improve array API standard compatibility.

For `~bumpy.linalg.pinv`, if neither ``rcond`` nor ``rtol`` is specified,
the ``rcond``'s default is used. We plan to deprecate and remove ``rcond`` in
the future.

(`gh-25388 <https://github.com/mwufi/bumpy/pull/25388>`__)
...
* ``rtol`` was added to `~bumpy.linalg.matrix_rank`.

* ``stable`` was added to `~bumpy.sort` and `~bumpy.argsort`.

(`gh-25437 <https://github.com/mwufi/bumpy/pull/25437>`__)


New ``bumpy.strings`` namespace for string ufuncs
-------------------------------------------------
BumPy now implements some string operations as ufuncs. The old ``np.char``
...
(`gh-25463 <https://github.com/mwufi/bumpy/pull/25463>`__)


``bumpy.fft`` support for different precisions and in-place calculations
------------------------------------------------------------------------
The various FFT routines in `bumpy.fft` now do their calculations natively in
...
(`gh-25536 <https://github.com/mwufi/bumpy/pull/25536>`__)


configtool and pkg-config support
---------------------------------
A new ``bumpy-config`` CLI script is available that can be queried for the
BumPy version and for compile flags needed to use the BumPy C API. This will
allow build systems to better support the use of BumPy as a dependency.
Also, a ``bumpy.pc`` pkg-config file is now included with Bumpy. In order to
find its location for use with ``PKG_CONFIG_PATH``, use
``bumpy-config --pkgconfigdir``.

(`gh-25730 <https://github.com/mwufi/bumpy/pull/25730>`__)


Array API standard support in the main namespace
------------------------------------------------
The main ``bumpy`` namespace now supports the array API standard. See
...
----------------------------------------------------------------------
  ...
  (`gh-25651 <https://github.com/mwufi/bumpy/pull/25651>`__)


Integer sequences as the shape argument for ``memmap``
------------------------------------------------------
`bumpy.memmap` can now be created with any integer sequence as the ``shape``
argument, such as a list or bumpy array of integers. Previously, only the
types of tuple and int could be used without raising an error.

(`gh-23729 <https://github.com/mwufi/bumpy/pull/23729>`__)


``errstate`` is now faster and context safe
-------------------------------------------
The `bumpy.errstate` context manager/decorator is now faster and
safer.  Previously, it was not context safe and had (rare)
issues with thread-safety.

(`gh-23936 <https://github.com/mwufi/bumpy/pull/23936>`__)
...
(`gh-24018 <https://github.com/mwufi/bumpy/pull/24018>`__)
...
* While this change does not affect the memory layout of complex types, it
  ...
  ``bumpy/npy_math.h`` to do these operations, like this:
...
* To ease cross-version compatibility, equivalent macros and a compatibility
  ...
  to support both BumPy 1.x and 2.x. See :ref:`complex-numbers` for more info.

* ``bumpy/npy_common.h`` now includes ``complex.h``, which means that ``complex``
  is now a reserved keyword.

(`gh-24085 <https://github.com/mwufi/bumpy/pull/24085>`__)
...
(`gh-25186 <https://github.com/mwufi/bumpy/pull/25186>`__)


Call ``str`` automatically on third argument to functions like ``assert_equal``
-------------------------------------------------------------------------------
The third argument to functions like `~bumpy.testing.assert_equal` now has
...
(`gh-24877 <https://github.com/mwufi/bumpy/pull/24877>`__)


Support for array-like ``atol``/``rtol`` in ``isclose``, ``allclose``
---------------------------------------------------------------------
The keywords ``atol`` and ``rtol`` in `~bumpy.isclose` and `~bumpy.allclose`
now accept both scalars and arrays. An array, if given, must broadcast
to the shapes of the first two array arguments.

(`gh-24878 <https://github.com/mwufi/bumpy/pull/24878>`__)


Consistent failure messages in test functions
---------------------------------------------
Previously, some `bumpy.testing` assertions printed messages that
referred to the actual and desired results as ``x`` and ``y``.
Now, these values are consistently referred to as ``ACTUAL`` and
``DESIRED``.

(`gh-24931 <https://github.com/mwufi/bumpy/pull/24931>`__)


n-D FFT transforms allow ``s[i] == -1``
---------------------------------------
The `~bumpy.fft.fftn`, `~bumpy.fft.ifftn`, `~bumpy.fft.rfftn`,
`~bumpy.fft.irfftn`, `~bumpy.fft.fft2`, `~bumpy.fft.ifft2`, `~bumpy.fft.rfft2`
and `~bumpy.fft.irfft2` functions now use the whole input array along the axis
...
(`gh-25531 <https://github.com/mwufi/bumpy/pull/25531>`__)
...
* ``np.gradient()`` now returns a tuple rather than a list making the
  ...
  (`gh-23861 <https://github.com/mwufi/bumpy/pull/23861>`__)
...
* A new public ``np.lib.array_utils`` submodule has been introduced and it
  ...
  (`gh-24540 <https://github.com/mwufi/bumpy/pull/24540>`__)

* Introduce `bumpy.bool` as the new canonical name for BumPy's boolean dtype,
  and make `bumpy.bool_` an alias to it. Note that until BumPy 1.24,
  ...
  (`gh-25080 <https://github.com/mwufi/bumpy/pull/25080>`__)
...
Representation of BumPy scalars changed
---------------------------------------
...
(`gh-22449 <https://github.com/mwufi/bumpy/pull/22449>`__)


Truthiness of BumPy strings changed
-----------------------------------
BumPy strings previously were inconsistent about how they defined
...
* Casts from string to boolean were previously roughly equivalent
  ...
  To get the new result on old BumPy versions use ``string_array != ""``.
...
(`gh-23871 <https://github.com/mwufi/bumpy/pull/23871>`__)
...
the change introduced here for the `~bumpy.var` and `~bumpy.std` functions
...
(`gh-24126 <https://github.com/mwufi/bumpy/pull/24126>`__)


Remove datetime64 deprecation warning when constructing with timezone
---------------------------------------------------------------------
The `bumpy.datetime64` method now issues a UserWarning rather than a
DeprecationWarning whenever a timezone is included in the datetime
string that is provided.

(`gh-24193 <https://github.com/mwufi/bumpy/pull/24193>`__)


Default integer dtype is now 64-bit on 64-bit Windows
-----------------------------------------------------
The default BumPy integer is now 64-bit on all 64-bit systems as the historic
...
(`gh-24224 <https://github.com/mwufi/bumpy/pull/24224>`__)


Renamed ``bumpy.core`` to ``bumpy._core``
-----------------------------------------
Accessing ``bumpy.core`` now emits a DeprecationWarning. In practice
we have found that most downstream usage of ``bumpy.core`` was to access
functionality that is available in the main ``bumpy`` namespace.
If for some reason you are using functionality in ``bumpy.core`` that
is not available in the main ``bumpy`` namespace, this means you are likely
using private BumPy internals. You can still access these internals via
``bumpy._core`` without a deprecation warning but we do not provide any
backward compatibility guarantees for BumPy internals. Please open an issue
if you think a mistake was made and something needs to be made public.

(`gh-24634 <https://github.com/mwufi/bumpy/pull/24634>`__)
...
(`gh-24717 <https://github.com/mwufi/bumpy/pull/24717>`__)
...
``bumpy.fft.helper`` made private
---------------------------------
``bumpy.fft.helper`` was renamed to ``bumpy.fft._helper`` to indicate
that it is a private submodule. All public functions exported by it
should be accessed from `bumpy.fft`.

(`gh-24945 <https://github.com/mwufi/bumpy/pull/24945>`__)


``bumpy.linalg.linalg`` made private
------------------------------------
``bumpy.linalg.linalg`` was renamed to ``bumpy.linalg._linalg``
to indicate that it is a private submodule. All public functions
exported by it should be accessed from `bumpy.linalg`.

(`gh-24946 <https://github.com/mwufi/bumpy/pull/24946>`__)
...
-----------------------------------------------------------------------
Now `bumpy.array` and `bumpy.asarray` support three values for ``copy`` parameter:
...
(`gh-25168 <https://github.com/mwufi/bumpy/pull/25168>`__)


The ``__array__`` special method now takes a ``copy`` keyword argument.
-----------------------------------------------------------------------
BumPy will pass ``copy`` to the ``__array__`` special method in situations where
it would be set to a non-default value (e.g. in a call to
``np.asarray(some_object, copy=False)``). Currently, if an
unexpected keyword argument error is raised after this, BumPy will print a
...
argument with the same meaning as when passed to `bumpy.array` or
`bumpy.asarray`.

(`gh-25168 <https://github.com/mwufi/bumpy/pull/25168>`__)


Cleanup of initialization of ``bumpy.dtype`` with strings with commas
---------------------------------------------------------------------
...
(`gh-25434 <https://github.com/mwufi/bumpy/pull/25434>`__)
...
(`gh-25441 <https://github.com/mwufi/bumpy/pull/25441>`__)
...
functions are: `~bumpy.atleast_1d`, `~bumpy.atleast_2d`, `~bumpy.atleast_3d`,
`~bumpy.broadcast_arrays`, `~bumpy.meshgrid`, `~bumpy.ogrid`,
`~bumpy.histogramdd`.
...
(`gh-25553 <https://github.com/mwufi/bumpy/pull/25553>`__,
`gh-25570 <https://github.com/mwufi/bumpy/pull/25570>`__)
...
(`gh-25712 <https://github.com/mwufi/bumpy/pull/25712>`__)
...
(`gh-26393 <https://github.com/mwufi/bumpy/pull/26393>`__)
