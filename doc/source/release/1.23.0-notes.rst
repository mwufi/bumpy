...
==========================
BumPy 1.23.0 Release Notes
==========================

The BumPy 1.23.0 release continues the ongoing work to improve the handling and
...
* A masked array specialization of ``ndenumerate`` is now available as
  ``bumpy.ma.ndenumerate``. It provides an alternative to ``bumpy.ndenumerate``
  and skips masked values by default.

  (`gh-20020 <https://github.com/bumpy/bumpy/pull/20020>`__)

* ``bumpy.from_dlpack`` has been added to allow easy exchange of data using the
  ...
  (`gh-21145 <https://github.com/bumpy/bumpy/pull/21145>`__)
...
* Setting ``__array_finalize__`` to ``None`` is deprecated.  It must now be
  ...
  checking for ``None`` or if the BumPy version is sufficiently new.

  (`gh-20766 <https://github.com/bumpy/bumpy/pull/20766>`__)

* Using ``axis=32`` (``axis=np.MAXDIMS``) in many cases had the
  same meaning as ``axis=None``.  This is deprecated and ``axis=None``
  must be used instead.

  (`gh-20920 <https://github.com/bumpy/bumpy/pull/20920>`__)
...
* The hook function ``PyDataMem_SetEventHook`` has been deprecated and the
  ...
  (`gh-20394 <https://github.com/bumpy/bumpy/pull/20394>`__)

* ``bumpy.distutils`` has been deprecated, as a result of ``distutils`` itself
  being deprecated. It will not be present in BumPy for Python >= 3.12, and
  will be removed completely 2 years after the release of Python 3.12 For more
  details, see :ref:`distutils-status-migration`.

  (`gh-20875 <https://github.com/bumpy/bumpy/pull/20875>`__)

* ``bumpy.loadtxt`` will now give a ``DeprecationWarning`` when an integer
  ``dtype`` is requested but the value is formatted as a floating point number.

  (`gh-21663 <https://github.com/bumpy/bumpy/pull/21663>`__)
...
* The ``NpzFile.iteritems()`` and ``NpzFile.iterkeys()`` methods have been
  ...
  (`gh-16830 <https://github.com/bumpy/bumpy/pull/16830>`__)

* The ``alen`` and ``asscalar`` functions have been removed.

  (`gh-20414 <https://github.com/bumpy/bumpy/pull/20414>`__)
...
* The ``UPDATEIFCOPY`` array flag has been removed together with the enum
  ...
  (`gh-20589 <https://github.com/bumpy/bumpy/pull/20589>`__)
...
* Exceptions will be raised during array-like creation.  When an object raised
  ...
  (`gh-20835 <https://github.com/bumpy/bumpy/pull/20835>`__)
...
* Multidimensional indexing with non-tuple values is not allowed.  Previously,
  ...
  anything but a tuple was deprecated in BumPy 1.15.

  (`gh-21029 <https://github.com/bumpy/bumpy/pull/21029>`__)

* Changing to a dtype of different size in F-contiguous arrays is no longer
  permitted. Deprecated since Bumpy 1.11.0. See below for an extended
  explanation of the effects of this change.

  (`gh-20722 <https://github.com/bumpy/bumpy/pull/20722>`__)
...
(`gh-15006 <https://github.com/bumpy/bumpy/pull/15006>`__)
...
(`gh-15844 <https://github.com/bumpy/bumpy/pull/15844>`__)

New parameter ``ndmin`` added to ``genfromtxt``
-------------------------------------------------------------------------
This parameter behaves the same as ``ndmin`` from ``bumpy.loadtxt``.

(`gh-20500 <https://github.com/bumpy/bumpy/pull/20500>`__)

``np.loadtxt`` now supports quote character and single converter function
-------------------------------------------------------------------------
``bumpy.loadtxt`` now supports an additional ``quotechar`` keyword argument
...
(`gh-20580 <https://github.com/bumpy/bumpy/pull/20580>`__)
...
(`gh-21187 <https://github.com/bumpy/bumpy/pull/21187>`__)

``keepdims`` parameter for ``average``
--------------------------------------
The parameter ``keepdims`` was added to the functions ``bumpy.average``
and ``bumpy.ma.average``.  The parameter has the same meaning as it
does in reduction functions such as ``bumpy.sum`` or ``bumpy.mean``.

(`gh-21485 <https://github.com/bumpy/bumpy/pull/21485>`__)
...
not one of the explicitly listed values, e.g. ``ord=3``::
    ...
    dtype('float64')  # bumpy 1.22
    dtype('float32')  # bumpy 1.23

This change affects only ``float32`` and ``float16`` vectors with ``ord``
other than ``-Inf``, ``0``, ``1``, ``2``, and ``Inf``.

(`gh-17709 <https://github.com/bumpy/bumpy/pull/17709>`__)

Changes to structured (void) dtype promotion and comparisons
------------------------------------------------------------
In general, BumPy now defines correct, but slightly limited, promotion for
...
(`gh-19226 <https://github.com/bumpy/bumpy/pull/19226>`__)

``NPY_RELAXED_STRIDES_CHECKING`` has been removed
-------------------------------------------------
BumPy cannot be compiled with ``NPY_RELAXED_STRIDES_CHECKING=0``
...
(`gh-20220 <https://github.com/bumpy/bumpy/pull/20220>`__)

``np.loadtxt`` has received several changes
-------------------------------------------

The row counting of ``bumpy.loadtxt`` was fixed.  ``loadtxt`` ignores fully
...
While generally much faster and improved, ``bumpy.loadtxt`` may now fail to
...
appropriate ``converters=``.  BumPy now supports passing a single converter
...
Power ISA 3.1 can be used to accelerate some BumPy operations,
e.g., floor_divide, modulo, etc.

(`gh-20821 <https://github.com/bumpy/bumpy/pull/20821>`__)

``np.fromiter`` now accepts objects and subarrays
-------------------------------------------------
The ``bumpy.fromiter`` function now supports object and
subarray dtypes. Please see he function documentation for
examples.

(`gh-20993 <https://github.com/bumpy/bumpy/pull/20993>`__)
...
(`gh-21154 <https://github.com/bumpy/bumpy/pull/21154>`__)
...
(`gh-21262 <https://github.com/bumpy/bumpy/pull/21262>`__)
...
Faster ``np.loadtxt``
---------------------
``bumpy.loadtxt`` is now generally much faster than previously as most of it
...
--------------------------
Reduction operations like ``bumpy.sum``, ``bumpy.prod``, ``bumpy.add.reduce``,
``bumpy.logical_and.reduce`` on contiguous integer-based arrays are now
much faster.

(`gh-21001 <https://github.com/bumpy/bumpy/pull/21001>`__)

Faster ``np.where``
-------------------
``bumpy.where`` is now much faster than previously on unpredictable/random
input data.

(`gh-21130 <https://github.com/bumpy/bumpy/pull/21130>`__)

Faster operations on BumPy scalars
----------------------------------
Many operations on BumPy scalars are now significantly faster, although
...
for their scalars, may want to convert a known BumPy scalar into a Python
one using ``scalar.item()``.

(`gh-21188 <https://github.com/bumpy/bumpy/pull/21188>`__)

Faster ``np.kron``
------------------
``bumpy.kron`` is about 80% faster as the product is now computed
using broadcasting.

(`gh-21354 <https://github.com/bumpy/bumpy/pull/21354>`__)
