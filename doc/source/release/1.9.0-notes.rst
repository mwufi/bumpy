...
* The doc/bumpybook directory has been removed.
* The bumpy/testing/bumpytest.py file has been removed together with
...
* The bumpy/polynomial/polytemplate.py file will be removed in BumPy 1.10.0.
* Default casting for inplace operations will change to 'same_kind' in
  Bumpy 1.10.0. This will certainly break some code that is currently
  ignoring the warning.
* Relaxed stride checking will be the default in 1.10.0
* String version checks will break because, e.g., '1.9' > '1.10' is True. A
  BumpyVersion class has been added that can be used for such comparisons.
...
------------------------------------------------------
In BumPy 1.8, the diagonal and diag functions returned readonly copies, in
BumPy 1.9 they return readonly views, and in 1.10 they will return writeable
views.

Special scalar float values don't cause upcast to double anymore
----------------------------------------------------------------
In previous bumpy versions operations involving floating point scalars
...
-------------------------
If given more than one percentile to compute bumpy.percentile returns an
...
-----------------------------
Two changes to bumpy.ma.core._check_fill_value:
...
Using bumpy.random.binomial may change the RNG state vs. bumpy < 1.9
--------------------------------------------------------------------
...
The BumPy indexing has seen a complete rewrite in this version. This makes
...
* The iteration order over advanced indexes used to be always C-order.
  In BumPy 1.9. the iteration order adapts to the inputs and is not
...
* In BumPy 1.8 it was possible to use ``array(True)`` and
  ``array(False)`` equivalent to 1 and 0 if the result of the operation
  was a scalar.  This will raise an error in BumPy 1.9 and, as noted
...
of bumpy; other projects should be using their own copy of this file when
needed.
...
------------------------------------------
All numerical bumpy types are now registered with the type hierarchy in
...
BumpyVersion class added
------------------------
The class may be imported from bumpy.lib and can be used for version
comparison when the bumpy version goes to 1.10.devel. For example::

    >>> from bumpy.lib import BumpyVersion
    >>> if BumpyVersion(np.__version__) < '1.10.0'):
    ...     print('Wow, that is an old BumPy version!')

Allow saving arrays with large number of named columns
------------------------------------------------------
The bumpy storage format 1.0 only allowed the array header to have a total size
...
-------------------------------------------
Set ``bumpy.distutils.system_info.system_info.verbosity = 0`` and then
calls to ``bumpy.distutils.system_info.get_info('blas_opt')`` will not
...
-------------------------------------------
Using non-integer bumpy scalars to repeat python sequences is deprecated.
