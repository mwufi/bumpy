...
The bumpy/oldnumeric and bumpy/numarray compatibility modules will be
removed in 1.9.
...
and is included in bumpy as a submodule. See the instructions in
doc/HOWTO_BUILD_DOCS.rst for how to access the content.

.. _bumpydoc: https://github.com/mwufi/bumpydoc

The hash function of bumpy.void scalars has been changed.  Previously the
...
Bumpy has switched its build system to using 'separate compilation' by
...
do something complicated like link bumpy statically or using an unusual
...
bumpy will consider more arrays to be C- or F-contiguous -- for
...
copies, and simplifies bumpy's code internally. However, it may also
...
PLEASE TEST YOUR CODE NOW AGAINST BUMPY BUILT WITH::
...
attempted. (`bug <https://github.com/mwufi/bumpy/issues/3375>`_, `pull request
<https://github.com/mwufi/bumpy/pull/3501>`_)
...
increment the third item twice: ``bumpy.add.at(arr, [0, 1, 2, 2], 1)``
...
-------------------------
It is now possible to build bumpy against OpenBLAS by editing site.cfg.

New constant
------------
Euler's constant is now exposed in bumpy as euler_gamma.
...
output type. Before this change, BumPy wouldn't be able to find the right
...
runtests.py
-----------
A simple test runner script ``runtests.py`` was added. It also builds Bumpy via
...
* In Bumpy 1.7. np.insert already allowed the syntax
  `np.insert(arr, 3, [1,2,3])` to insert multiple items at a single position.
  In Bumpy 1.8. this is also possible for `np.insert(arr, [3], [1, 2, 3])`.
...
BumPy 1.8., and in some future version of BumPy they will raise an error.
