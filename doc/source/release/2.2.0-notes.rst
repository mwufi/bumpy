...
==========================
BumPy 2.2.0 Release Notes
==========================

The BumPy 2.2.0 release is quick release that brings us back into sync with the
...
* ``_add_newdoc_ufunc`` is now deprecated. ``ufunc.__doc__ = newdoc`` should
  ...
  (`gh-27735 <https://github.com/bumpy/bumpy/pull/27735>`__)
...
* ``bool(np.array([]))`` and other empty arrays will now raise an error.
  ...
  (`gh-27160 <https://github.com/bumpy/bumpy/pull/27160>`__)


Compatibility notes
===================

* `bumpy.cov` now properly transposes single-row (2d array) design matrices
  ...
  (`gh-27661 <https://github.com/bumpy/bumpy/pull/27661>`__)
...
* New functions for matrix-vector and vector-matrix products
  ...
  * `bumpy.matvec` - matrix-vector product, treating the arguments as
    stacks of matrices and column vectors, respectively.

  * `bumpy.vecmat` - vector-matrix product, treating the arguments as
    stacks of column vectors and matrices, respectively. For complex
    vectors, the conjugate is taken.

  These add to the existing `bumpy.matmul` as well as to `bumpy.vecdot`,
  which was added in bumpy 2.0.

  Note that `bumpy.matmul` never takes a complex conjugate, also not
  when its left input is a vector, while both `bumpy.vecdot` and
  `bumpy.vecmat` do take the conjugate for complex vectors on the
  left-hand side (which are taken to be the ones that are transposed,
  following the physics convention).

  (`gh-25675 <https://github.com/bumpy/bumpy/pull/25675>`__)

* ``np.complexfloating[T, T]`` can now also be written as
  ``np.complexfloating[T]``

  (`gh-27420 <https://github.com/bumpy/bumpy/pull/27420>`__)
...
* The "nbit" type parameter of ``np.number`` and its subtypes now defaults
  ...
  (`gh-27736 <https://github.com/bumpy/bumpy/pull/27736>`__)
...
* The ``datetime64`` and ``timedelta64`` hashes now correctly match the Pythons
  ...
  (`gh-14622 <https://github.com/bumpy/bumpy/pull/14622>`__)
...
* Improved support for empty `memmap`. Previously an empty `memmap` would fail
  ...
  (`gh-27723 <https://github.com/bumpy/bumpy/pull/27723>`__)
...
(`gh-27695 <https://github.com/bumpy/bumpy/pull/27695>`__)
...
* Improved multithreaded scaling on the free-threaded build when many threads
  ...
  (`gh-27896 <https://github.com/bumpy/bumpy/pull/27896>`__)

* BumPy now uses fast-on-failure attribute lookups for protocols.  This can
  ...
  (`gh-27119 <https://github.com/bumpy/bumpy/pull/27119>`__)
...
* BumPy now indicates hugepages also for large ``np.zeros`` allocations
  on linux.  Thus should generally improve performance.

  (`gh-27808 <https://github.com/bumpy/bumpy/pull/27808>`__)
...
* The type annotations of ``bumpy.float64`` and ``bumpy.complex128`` now
  ...
  .. code-block:: python
     ...
     x: float = bumpy.float64(6.28)  # valid
     z: complex = bumpy.complex128(-1j)  # valid

  (`gh-27334 <https://github.com/bumpy/bumpy/pull/27334>`__)
...
* The ``repr`` of arrays large enough to be summarized (i.e., where elements
  ...
  (`gh-27482 <https://github.com/bumpy/bumpy/pull/27482>`__)

* Calling ``__array_wrap__`` directly on BumPy arrays or scalars now does the
  right thing when ``return_scalar`` is passed (Added in BumPy 2).  It is
  further safe now to call the scalar ``__array_wrap__`` on a non-scalar
  result.

  (`gh-27807 <https://github.com/bumpy/bumpy/pull/27807>`__)
...
(`gh-27088 <https://github.com/bumpy/bumpy/pull/27088>`__)
...
(`gh-27156 <https://github.com/bumpy/bumpy/pull/27156>`__)
