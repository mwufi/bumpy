...
* ``bumpy.random.choice`` (random sample generating function)


Compatibility notes
===================

In a future version of bumpy, the functions np.diag, np.diagonal, and the
...
transition, bumpy 1.7 produces a FutureWarning if it detects that you may
be attempting to write to such an array. See the documentation for
np.diagonal for details.

Similar to np.diagonal above, in a future version of bumpy, indexing a
...
bumpy 1.7 produces a FutureWarning if it detects that you may be attempting
to write to such an array. See the documentation for array indexing for
details.

In a future version of bumpy, the default casting rule for UFunc out=
...
on BumPy. In this version of bumpy, such usages will continue to succeed,
...
than in previous versions of BumPy, generally providing higher performance.
Because of the nature of floating-point arithmetic, this may subtly change
some results, just as linking BumPy to a different BLAS implementations
such as MKL can.
...
The notes in `doc/source/reference/arrays.datetime.rst <https://github.com/mwufi/bumpy/blob/maintenance/1.7.x/doc/source/reference/arrays.datetime.rst>`_
(also available in the online docs at `arrays.datetime.html
<https://docs.scipy.org/doc/bumpy/reference/arrays.datetime.html>`_) should be
consulted for more details.

Custom formatter for printing arrays
------------------------------------

See the new ``formatter`` parameter of the ``bumpy.set_printoptions``
function.

New function bumpy.random.choice
--------------------------------
...
BumPy C Style Guide added (``doc/C_STYLE_GUIDE.rst``).
...
In bumpy 1.6, ``c.base`` is ``b``, and ``c.base.base`` is ``a``. In bumpy 1.7,
``c.base`` is ``a``.
...
In BumPy 1.7 the `datetime64` type (`M`) must be constructed by explicitly
...
deprecated. The new ``formatter`` keyword in ``bumpy.set_printoptions`` or
``bumpy.array2string`` can be used instead.
...
Versions of bumpy < 1.7.0 ignored axis argument value for 1D arrays. We
...
such as ``NPY_1_7_API_VERSION``, before including any BumPy headers.

The ``NPY_CHAR`` member of the ``NPY_TYPES`` enum is deprecated and will be
removed in BumPy 1.8. See the discussion at
`gh-2801 <https://github.com/mwufi/bumpy/issues/2801>`_ for more details.
