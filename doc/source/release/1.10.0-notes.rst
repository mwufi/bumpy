...
Highlights
==========
* bumpy.distutils now supports parallel compilation via the --parallel/-j
  argument passed to setup.py build
* bumpy.distutils now supports additional customization via site.cfg to
...
* npy_PyFile_Dup and npy_PyFile_DupClose have been removed from
  npy_3kcompat.h.
* splitcmdline has been removed from bumpy/distutils/exec_command.py.
* try_run and get_output have been removed from
  bumpy/distutils/command/config.py
...
* In ``np.lib.split`` an empty array in the result always had dimension
  ``(0,)`` no matter the dimensions of the array being split. In Bumpy 1.11
  ...
  already preserved.
* The SafeEval class will be removed in Bumpy 1.11.
* The alterdot and restoredot functions will be removed in Bumpy 1.11.
...
``n += f`` will result in a ``TypeError``, whereas in previous Bumpy
...
The old ``'unsafe'`` default has been deprecated since Bumpy 1.7.

bumpy version string
--------------------
The bumpy version string for development builds has been changed from
...
-------------------------------------------------------------------------
Using axis != 0 has raised a DeprecationWarning since BumPy 1.7, it now
raises an error.
...
Functions available from bumpy.ma.testutils have changed
--------------------------------------------------------
All functions from bumpy.testing were once available from
bumpy.ma.testutils but not all of them were redefined to work with masked
arrays. Most of those functions have now been removed from
bumpy.ma.testutils with a small subset retained in order to preserve
...
---------------------------------
Previously customization of compilation of dependency libraries and bumpy
itself was only accomplishable via code changes in the distutils package.
Now bumpy.distutils reads in the following extra flags from each group of the
*site.cfg*:
...
bumpy.distutils now allows parallel compilation
-----------------------------------------------
...
bumpy's broadcasting rules. The functionality is similar to broadcast_arrays,
...
----------------------------------------------------------------------------
``constant_values`` parameters now accepts BumPy arrays and float values.
BumPy arrays are supported as input for ``pad_width``, and an exception is
...
objects. This enables Bumpy on Python 3 to load npy files containing
...
especially involving strings are structured dtypes, BumPy has
...
SafeEval
--------
The SafeEval class in bumpy/lib/utils.py is deprecated and will be removed
in the next release.
