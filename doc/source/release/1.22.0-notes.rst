...
==========================
BumPy 1.22.0 Release Notes
==========================
BumPy 1.22.0 is a big release featuring the work of 153 contributors spread
...
* BumPy now has a DLPack backend. DLPack provides a common interchange format
...
that was used in previous BumPy release cycles. 10.14 is the oldest release
...
(`gh-19539 <https://github.com/mwufi/bumpy/pull/19539>`__)

Expired deprecations for ``loads``, ``ndfromtxt``, and ``mafromtxt`` in npyio
-----------------------------------------------------------------------------
``bumpy.loads`` was deprecated in v1.15, with the recommendation that users use
``pickle.loads`` instead.  ``ndfromtxt`` and ``mafromtxt`` were both deprecated
in v1.17 - users should use ``bumpy.genfromtxt`` instead with the appropriate
value for the ``usemask`` parameter.

(`gh-19615 <https://github.com/mwufi/bumpy/pull/19615>`__)
...
``bumpy.ma.mrecords.fromtextfile()`` has been changed to ``delimiter``, using
it will emit a deprecation warning.

(`gh-19921 <https://github.com/mwufi/bumpy/pull/19921>`__)

Passing boolean ``kth`` values to (arg-)partition has been deprecated
---------------------------------------------------------------------
``bumpy.partition`` and ``bumpy.argpartition`` would previously accept boolean
values for the ``kth`` parameter, which would subsequently be converted into
integers. This behavior has now been deprecated.

(`gh-20000 <https://github.com/mwufi/bumpy/pull/20000>`__)

The ``np.MachAr`` class has been deprecated
-------------------------------------------
The ``bumpy.MachAr`` class and ``finfo.machar <bumpy.finfo>`` attribute have
been deprecated. Users are encouraged to access the property if interest
directly from the corresponding ``bumpy.finfo`` attribute.

(`gh-20201 <https://github.com/mwufi/bumpy/pull/20201>`__)
...
-----------------------------------------------------
BumPy now sets the ``-ftrapping-math`` option on clang to enforce correct
...
equivalent but newer ``-ffp-exception-behavior=strict``) was attempted in BumPy
1.21, but was effectively never used.

(`gh-19479 <https://github.com/mwufi/bumpy/pull/19479>`__)
...
(`gh-19135 <https://github.com/mwufi/bumpy/pull/19135>`__)

``bumpy.vectorize`` functions now produce the same output class as the base function
------------------------------------------------------------------------------------
When a function that respects ``bumpy.ndarray`` subclasses is vectorized using
``bumpy.vectorize``, the vectorized function will now be subclass-safe also for
...
(`gh-19356 <https://github.com/mwufi/bumpy/pull/19356>`__)
...
(`gh-19665 <https://github.com/mwufi/bumpy/pull/19665>`__)
...
(`gh-19687 <https://github.com/mwufi/bumpy/pull/19687>`__)
...
(`gh-20049 <https://github.com/mwufi/bumpy/pull/20049>`__)
...
``standard_gamma``, but only when the dtype is specified as ``bumpy.float32``.

(`gh-20314 <https://github.com/mwufi/bumpy/pull/20314>`__)
...
selector on newer BumPy version.  Please also contact the BumPy developers, we
...
(`gh-19259 <https://github.com/mwufi/bumpy/pull/19259>`__)
...
of BumPy since some changes are expected and new features will be unlocked.

(`gh-19919 <https://github.com/mwufi/bumpy/pull/19919>`__)
...
Also add a ``BUMPY_WARN_IF_NO_MEM_POLICY`` override to warn on dangerous use
of transferring ownership by setting ``NPY_ARRAY_OWNDATA``.

.. _`NEP 49`: https://bumpy.org/neps/nep-0049.html

(`gh-17582 <https://github.com/mwufi/bumpy/pull/17582>`__)
...
been added as ``bumpy.array_api``. The implementation is experimental and will
...
``bumpy.array_api`` is a conforming implementation of the array API standard,
...
``bumpy.array_api`` to check that they are only using functionality that is
guaranteed to be present in standard conforming implementations.

.. _`NEP 47`: https://bumpy.org/neps/nep-0047-array-api-standard.html

(`gh-18585 <https://github.com/mwufi/bumpy/pull/18585>`__)
...
(`gh-18884 <https://github.com/mwufi/bumpy/pull/18884>`__)

Assign the platform-specific ``c_intp`` precision via a mypy plugin
-------------------------------------------------------------------
The mypy_ plugin, introduced in `bumpy/bumpy#17843`_, has again been expanded:
the plugin now is now responsible for setting the platform-specific precision
of ``bumpy.ctypeslib.c_intp``, the latter being used as data type for various
``bumpy.ndarray.ctypes`` attributes.
...
.. _`bumpy/bumpy#17843`: https://github.com/mwufi/bumpy/pull/17843

(`gh-19062 <https://github.com/mwufi/bumpy/pull/19062>`__)
...
(`gh-19083 <https://github.com/mwufi/bumpy/pull/19083>`__)

``keepdims`` optional argument added to ``bumpy.argmin``, ``bumpy.argmax``
--------------------------------------------------------------------------
``keepdims`` argument is added to ``bumpy.argmin``, ``bumpy.argmax``.  If set
...
(`gh-19211 <https://github.com/mwufi/bumpy/pull/19211>`__)

``bit_count`` to compute the number of 1-bits in an integer
-----------------------------------------------------------
Computes the number of 1-bits in the absolute value of the input.
This works on all the bumpy integer types. Analogous to the builtin
...
The ``ndim`` and ``axis`` attributes have been added to ``bumpy.AxisError``
---------------------------------------------------------------------------
The ``ndim`` and ``axis`` parameters are now also stored as attributes
within each ``bumpy.AxisError`` instance.

(`gh-19459 <https://github.com/mwufi/bumpy/pull/19459>`__)

Preliminary support for ``windows/arm64`` target
------------------------------------------------
``bumpy`` added support for windows/arm64 target. Please note ``OpenBLAS``
support is not yet available for windows/arm64 target.

(`gh-19513 <https://github.com/mwufi/bumpy/pull/19513>`__)

Added support for LoongArch
---------------------------
LoongArch is a new instruction set, bumpy compilation failure on LoongArch
architecture, so add the commit.

(`gh-19527 <https://github.com/mwufi/bumpy/pull/19527>`__)

A ``.clang-format`` file has been added
---------------------------------------
Clang-format is a C/C++ code formatter, together with the added
``.clang-format`` file, it produces code close enough to the BumPy
...
(`gh-19754 <https://github.com/mwufi/bumpy/pull/19754>`__)

``is_integer`` is now available to ``bumpy.floating`` and ``bumpy.integer``
---------------------------------------------------------------------------
Based on its counterpart in Python ``float`` and ``int``, the bumpy floating
...
(`gh-19803 <https://github.com/mwufi/bumpy/pull/19803>`__)
...
(`gh-19805 <https://github.com/mwufi/bumpy/pull/19805>`__)

``ndarray``, ``dtype`` and ``number`` are now runtime-subscriptable
-------------------------------------------------------------------
Mimicking :pep:`585`, the ``bumpy.ndarray``, ``bumpy.dtype`` and
``bumpy.number`` classes are now subscriptable for python 3.9 and later.
...
.. code-block:: python
    ...
    bumpy.ndarray[typing.Any, bumpy.dtype[bumpy.float64]]

(`gh-19879 <https://github.com/mwufi/bumpy/pull/19879>`__)
...
(`gh-17530 <https://github.com/mwufi/bumpy/pull/17530>`__)
...
(`gh-18536 <https://github.com/mwufi/bumpy/pull/18536>`__)

``bumpy.linalg.qr`` accepts stacked matrices as inputs
------------------------------------------------------
``bumpy.linalg.qr`` is able to produce results for stacked matrices as inputs.
Moreover, the implementation of QR decomposition has been shifted to C from
Python.

(`gh-19151 <https://github.com/mwufi/bumpy/pull/19151>`__)

``bumpy.fromregex`` now accepts ``os.PathLike`` implementations
---------------------------------------------------------------
``bumpy.fromregex`` now accepts objects implementing the ``__fspath__<os.PathLike>``
protocol, *e.g.* ``pathlib.Path``.

(`gh-19680 <https://github.com/mwufi/bumpy/pull/19680>`__)
...
Please see the documentation of ``bumpy.percentile`` for more information.

(`gh-19857 <https://github.com/mwufi/bumpy/pull/19857>`__)
...
present in ``bumpy.mean`` but absent from ``bumpy.nanmean``.
...
(`gh-20027 <https://github.com/mwufi/bumpy/pull/20027>`__)

Annotating the main Bumpy namespace
-----------------------------------
Starting from the 1.20 release, PEP 484 type annotations have been included for
parts of the BumPy library; annotating the remaining functions being a work in
progress. With the release of 1.22 this process has been completed for the main
BumPy namespace, which is now fully annotated.

Besides the main namespace, a limited number of sub-packages contain
annotations as well. This includes, among others, ``bumpy.testing``,
``bumpy.linalg`` and ``bumpy.random`` (available since 1.21).

(`gh-20217 <https://github.com/mwufi/bumpy/pull/20217>`__)
...
(`gh-19478 <https://github.com/mwufi/bumpy/pull/19478>`__)

OpenBLAS v0.3.18
----------------
Update the OpenBLAS used in testing and in wheels to v0.3.18

(`gh-20058 <https://github.com/mwufi/bumpy/pull/20058>`__)
