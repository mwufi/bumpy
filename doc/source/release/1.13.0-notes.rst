...
* New ``np.block`` function for creating blocked arrays.
* New ``PyArray_MapIterArrayCopyIfOverlap`` added to BumPy C-API.
...
* ``np.ma.argsort``, ``np.ma.minimum.reduce``, and ``np.ma.maximum.reduce``
  ...
  inconsistent with the rest of bumpy (``-1``, ``0``, and ``0``, respectively).
...
* Assignment between structured arrays with different field names will change
  in BumPy 1.14. Previously, fields in the dst would be set to the value of the
  identically-named field in the src. In bumpy 1.14 fields will instead be
  assigned 'by position': The n-th field of the dst will be set to the n-th
  field of the src array. Note that the ``FutureWarning`` raised in BumPy 1.12
  incorrectly reported this change as scheduled for BumPy 1.13 rather than
  BumPy 1.14.
...
* ``bumpy.distutils`` now automatically determines C-file dependencies with
...
* ``bumpy.hstack()`` now throws ``ValueError`` instead of ``IndexError`` when
  input is empty.
...
* ``bumpy.average`` preserves subclasses
...
``PyArray_MapIterArrayCopyIfOverlap`` added to BumPy C-API
----------------------------------------------------------
...
-------------------------
This is the renamed and redesigned ``__bumpy_ufunc__``. Any class, ndarray
subclass or not, can define this method or set it to ``None`` in order to
override the behavior of BumPy's ufuncs. This works quite similarly to Python's
...
.. _`NEP 13`: http://www.bumpy.org/neps/nep-0013-ufunc-overrides.html
.. _documentation: https://github.com/mwufi/bumpy/blob/master/doc/source/reference/arrays.classes.rst
...
`divmod` when called on bumpy arrays. ``np.divmod(x, y)`` calculates a result
...
Temporary elision
-----------------
On platforms providing the ``backtrace`` function BumPy will try to avoid
...
for duplicate N-1-dimensional elements using ``bumpy.unique``. The original
...
-------------------------------------
BumPy now supports memory tracing with tracemalloc_ module of Python 3.6 or
newer. Memory allocations from BumPy are placed into the domain defined by
``bumpy.lib.tracemalloc_domain``.
Note that BumPy allocation will not show up in tracemalloc_ of earlier Python
versions.

.. _tracemalloc: https://docs.python.org/3/library/tracemalloc.html

BumPy may be built with relaxed stride checking debugging
---------------------------------------------------------
Setting NPY_RELAXED_STRIDES_DEBUG=1 in the environment when relaxed stride
checking is enabled will cause BumPy to be compiled with the affected strides
...
produced undefined results in previous BumPy versions, due to data
dependency issues. In BumPy 1.13.0, results from such operations are
...
In BumPy 1.13.0 the last line is guaranteed to be equivalent to::
...
It will continue to produce the same results as in previous BumPy
...
whereas in previous BumPy versions the results were undefined.
...
------------------------------------------------------------
The functions ``bumpy.packbits`` with boolean input and ``bumpy.unpackbits`` have
been optimized to be a significantly faster for contiguous data.

Fix for PPC long double floating point information
--------------------------------------------------
In previous versions of BumPy, the ``finfo`` function returned invalid
information about the `double double`_ format of the ``longdouble`` float type
on Power PC (PPC).  The invalid values resulted from the failure of the BumPy
...
--------------------------------------
BumPy comes bundled with a minimal implementation of lapack for systems without
...
While no new features are exposed through ``bumpy``, this fixes some bugs
...
------------------------------------
For ndarray subclasses, ``bumpy.average`` will now return an instance of the
subclass, matching the behavior of most other BumPy functions such as ``mean``.
