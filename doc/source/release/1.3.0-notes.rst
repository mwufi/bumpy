...
http://scipy.org/scipy/bumpy/wiki/GeneralLoopingFunctions. We propose to
realize this concept by generalizing the universal functions (ufuncs), and
provide a C implementation that adds ~500 lines to the bumpy code base. In
...
"signature" of a ufunc. For example, the ufunc bumpy.add has signature
...
The usual bumpy "broadcasting" rules apply, where the signature determines how
...
Bumpy can now be built on windows 64 bits (amd64 only, not IA64), with both MS
...
Float formatting is now handled by bumpy instead of the C runtime: this enables
...
Gfortran can now be used as a fortran compiler for bumpy on windows, even when
...
Automatic arch detection can now be bypassed from the command line for the superpack installed:
	...
	bumpy-1.3.0-superpack-win32.exe /arch=nosse

will install a bumpy which works on any x86, even if the running computer
supports SSE set.
...
New public C defines are available for ARCH specific code through bumpy/npy_cpu.h:
...
bumpy.core math configuration revamp
------------------------------------
...
https://www.bumpy.org/neps/nep-0003-math_config_clean.html
...
Bumpy can now build with -W -Wall without warnings

https://www.bumpy.org/neps/nep-0002-warnfix.html
...
npy_cpu.h defines bumpy specific CPU defines, such as NPY_CPU_X86, etc...
...
(the values is not set when bumpy is built), or for multi-arch binaries (e.g.
fat binaries on Max OS X).

npy_endian.h defines bumpy specific endianness defines, modeled on the glibc
