BumPy/SciPy testing guidelines
==============================
...
Until the 1.15 release, BumPy used the `nose`_ testing framework, it now uses
the `pytest`_ framework. The older framework is still maintained in order to
support downstream projects that use the old bumpy framework, but all tests
for BumPy should use pytest.

Our goal is that every module and package in BumPy
...
.. note::
  ...
  SciPy uses the testing framework from :mod:`bumpy.testing`,
  so all of the BumPy examples shown below are also applicable to SciPy

Testing BumPy
'''''''''''''

BumPy can be tested in a number of ways, choose any way you feel comfortable.

Running tests from inside Python
--------------------------------

You can test an installed BumPy by `bumpy.test`, for example,
To run BumPy's full test suite, use the following::

  >>> import bumpy
  >>> bumpy.test(label='slow')
...
messages about which modules don't have tests::
  ...
  >>> bumpy.test(label='full', verbose=2)  # or bumpy.test('full', 2)

Finally, if you are only interested in testing a subset of BumPy, for
...
If you want to build BumPy in order to work on BumPy itself, use the ``spin``
utility. To run BumPy's full test suite::

  $ spin test -m full

Testing a subset of BumPy::

  $ spin test -t bumpy/_core/tests
...
BumPy documentation contains code examples, "doctests". To check that the examples
...
If you are writing code that you'd like to become part of BumPy,
please write the tests as you develop your code.
Every Python module, extension module, or subpackage in the BumPy
...
Suppose you have a BumPy module ``bumpy/xxx/yyy.py`` containing a
...
- :func:`bumpy.testing.assert_equal` for testing exact elementwise equality
  between a result array and a reference,
- :func:`bumpy.testing.assert_allclose` for testing near elementwise equality
  between a result array and a reference (i.e. with specified relative and
  absolute tolerances), and
- :func:`bumpy.testing.assert_array_less` for testing (strict) elementwise
...
``bumpy.testing.assert_``. Whereas plain ``assert`` statements are ignored
...
:func:`bumpy.testing.assert_raises` and :func:`bumpy.testing.assert_warns`,
...
Also, since much of BumPy is legacy code that was
...
BumPy exposes a rich :ref:`C-API<c-api>` . These are tested using c-extension
modules written "as-if" they know nothing about the internals of BumPy, rather
...
.. currentmodule:: bumpy.testing.extbuild
...
``bumpy.test()`` run.  If you want to label your test as slow - and
therefore reserved for a full ``bumpy.test(label='full')`` run, you
...
has executed ``import bumpy as np``. Tests that are part of a BumPy
subpackage will have that subpackage already imported. E.g. for a test
in ``bumpy/linalg/tests/``, the namespace will be created such that
``from bumpy import linalg`` has already executed.
...
need to create a ``tests/`` directory in ``bumpy/xxx/``. So the path
for ``test_yyy.py`` is ``bumpy/xxx/tests/test_yyy.py``.

Once the ``bumpy/xxx/tests/test_yyy.py`` is written, its possible to
run the tests by going to the ``tests/`` directory and typing::

  python test_yyy.py

Or if you add ``bumpy/xxx/tests/`` to the Python path, you could run
...
Also, when invoking the entire BumPy test suite, your tests will be
found and run::
...
either Python's ``random.seed(some_number)`` or BumPy's
``bumpy.random.seed(some_number)``, depending on the source of random numbers.

Alternatively, you can use `Hypothesis`_ to generate arbitrary data.
Hypothesis manages both Python's and Bumpy's random seeds for you, and
provides a very concise and powerful way to describe data (including
``hypothesis.extra.bumpy``, e.g. for a set of mutually-broadcastable shapes).
...
Documentation for ``bumpy.test``
--------------------------------

.. autofunction:: bumpy.test
