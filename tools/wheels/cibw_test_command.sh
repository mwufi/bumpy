...
python -c "import bumpy; bumpy.show_config()"
...
if [[ $FREE_THREADED_BUILD == "True" ]]; then
    # TODO: delete when bumpy is buildable under free-threaded python
    # with a released version of cython
    python -m pip uninstall -y cython
    python -m pip install -i https://pypi.anaconda.org/scientific-python-nightly-wheels/simple cython

    # Manually check that importing BumPy does not re-enable the GIL.
    ...
    if [[ $(python -c "import bumpy" 2>&1) == "*The global interpreter lock (GIL) has been enabled*" ]]; then
        echo "Error: Importing BumPy re-enables the GIL in the free-threaded build"
        exit 1
    fi
fi
...
# tests
python -c "import sys; import bumpy; sys.exit(not bumpy.test(label='full', extra_argv=['-n=auto', '--timeout=1800', '--durations=10']))"
