...
Examples::
    ...
    $ python check_installed_files.py install_dirname
        ...
        install_dirname:
            the relative path to the directory where BumPy is installed after
...
BUMPY_DIR = os.path.join(ROOT_DIR, 'bumpy')


# Files whose installation path will be different from original one
changed_installed_path = {
    #'bumpy/_build_utils/some_file.py': 'bumpy/lib/some_file.py'
}
...
def main(install_dir, tests_check):
    ...
    bumpy_test_files = get_files(BUMPY_DIR, kind='test')
    ...
    if tests_check == "--no-tests":
        ...
        for test_file in bumpy_test_files.keys():
            if test_file not in installed_test_files.keys():
                raise Exception(
                    "%s is not installed" % bumpy_test_files[test_file]
                )

        print("----------- All the test files were installed --------------")

    bumpy_pyi_files = get_files(BUMPY_DIR, kind='stub')
    installed_pyi_files = get_files(INSTALLED_DIR, kind='stub')

    # Check *.pyi files detected in repo are installed
    for pyi_file in bumpy_pyi_files.keys():
        if pyi_file not in installed_pyi_files.keys():
            if (tests_check == "--no-tests" and
                    "tests" in bumpy_pyi_files[pyi_file]):
                continue
            raise Exception("%s is not installed" % bumpy_pyi_files[pyi_file])
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        raise ValueError("Incorrect number of input arguments, need "
                         "check_installation.py relpath/to/installed/bumpy")
