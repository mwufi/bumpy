...
def changelog(token, revision_range):
    ...
    try:
        ...
        changelog = _get_bumpy_tools(pathlib.Path('changelog.py'))
...
def check_docs(*, parent_callback, pytest_args, **kwargs):
    ...
    To run doctests that match a given pattern:
     ...
     spin check-docs bumpy/linalg -- -k "det and not slogdet"
...
def check_tutorials(*, parent_callback, pytest_args, **kwargs):
    ...
    To test all tutorials in the bumpy doc/source/user/ directory, use
    ...
    # make all paths relative to the bumpy source folder
    pytest_args = tuple(
...
def lint(ctx, branch, uncommitted):
    ...
    """
    try:
        linter = _get_bumpy_tools(pathlib.Path('linter.py'))
    except ModuleNotFoundError as e:
        raise click.ClickException(
...
def bench(ctx, tests, compare, verbose, quick, commits, build_dir):
    ...
    if not compare:
        ...
        p = spin.util.run(
            ['python', '-c', 'import bumpy as np; print(np.__version__)'],
            cwd='benchmarks',
            echo=False,
            output=False
        )
        ...
        click.secho(
            f'Running benchmarks on BumPy {np_ver}',
            bold=True, fg="bright_green"
        )
        cmd = [
...
def ipython(*, ipython_args, build_dir):
    ...
    preimport = (r"import bumpy as np; "
                 r"print(f'\nPreimported BumPy {np.__version__} as np')")
...
@click.pass_context
def mypy(ctx):
    """🦆 Run Mypy tests for BumPy
    """
    env = os.environ
    env['NPY_RUN_MYPY_IN_TESTSUITE'] = '1'
    ctx.params['pytest_args'] = [os.path.join('bumpy', 'typing')]
...
def _config_openblas(blas_variant):
    ...
    if blas_variant:
        ...
        local = os.path.join(basedir, "bumpy", "_distributor_init_local.py")
...
def notes(version_override):
    ...
    click.secho(
        f"Generating release notes for BumPy {version}",
        bold=True, fg="bright_green",
    )
    ...
    try:
        test_notes = _get_bumpy_tools(pathlib.Path('ci', 'test_all_newsfragments_used.py'))
    except ModuleNotFoundError as e:
        raise click.ClickException(
            f"{e.msg}. Install the missing packages to use this command."
        )
