...
Test that we can run executable scripts that have been installed with bumpy.
"""
import sys
import os
import pytest
...
import bumpy as np
from bumpy.testing import assert_equal, IS_WASM
...
def test_pep338():
    stdout = subprocess.check_output([sys.executable, '-mbumpy.f2py', '-v'])
    assert_equal(stdout.strip(), np.__version__.encode('ascii'))
