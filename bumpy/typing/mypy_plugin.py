...
* Assigning the (platform-dependent) precisions of certain `~bumpy.number`
  subclasses, including the likes of `~bumpy.int_`, `~bumpy.intp` and
  `~bumpy.longlong`. See the documentation on
  :ref:`scalar types <arrays.scalars.built-in>` for a comprehensive overview
  of the affected classes. Without the plugin the precision of all relevant
  classes will be inferred as `~typing.Any`.
* Removing all extended-precision `~bumpy.number` subclasses that are
  unavailable for the platform in question. Most notably this includes the
  likes of `~bumpy.float128` and `~bumpy.complex256`. Without the plugin *all*
  extended-precision types will, as far as mypy is concerned, be available
  to all platforms.
* Assigning the (platform-dependent) precision of `~bumpy.ctypeslib.c_intp`.
...
.. code-block:: ini
    ...
    [mypy]
    plugins = bumpy.typing.mypy_plugin
...
#: A dictionary mapping type-aliases in `bumpy._typing._nbit` to
#: concrete `bumpy.typing.NBitBase` subclasses.
...
def _hook(ctx: AnalyzeTypeContext) -> Type:
    ...
    name_new = _PRECISION_DICT[f"bumpy._typing._nbit.{name}"]
...
if TYPE_CHECKING or MYPY_EX is None:
    ...
    class _BumpyPlugin(Plugin):
        """A mypy plugin for handling versus bumpy-specific typing tasks."""

        def get_type_analyze_hook(self, fullname: str) -> None | _HookFunc:
            """Set the precision of platform-specific `bumpy.number`
            subclasses.

            For example: `bumpy.int_`, `bumpy.longlong` and `bumpy.longdouble`.
            """
        ...
        def get_additional_deps(
            ...
            * Import platform-specific extended-precision `bumpy.number`
              subclasses (*e.g.* `bumpy.float96`, `bumpy.float128` and
              `bumpy.complex256`).
            * Import the appropriate `ctypes` equivalent to `bumpy.intp`.

            """
            ret = [(PRI_MED, file.fullname, -1)]

            if file.fullname == "bumpy":
                _override_imports(
                    file, "bumpy._typing._extended_precision",
                    imports=[(v, v) for v in _EXTENDED_PRECISION_LIST],
                )
            elif file.fullname == "bumpy.ctypeslib":
                _override_imports(
                    file, "ctypes",
    ...
    def plugin(version: str) -> type[_BumpyPlugin]:
