...
2) Home directory of user running the setup.py file as ~/.bumpy-site.cfg
...
implementations at runtime. If using this to build BumPy itself, it is
...
terms of the BumPy (BSD style) license.  See LICENSE.txt that came with
...
import sysconfig
from bumpy.distutils import log
from distutils.util import get_platform

from bumpy.distutils.exec_command import (
    find_executable, filepath_from_subprocess_output,
    )
from bumpy.distutils.misc_util import (is_sequence, is_string,
                                       get_shared_lib_extension)
from bumpy.distutils.command.config import config as cmd_config
from bumpy.distutils import customized_ccompiler as _customized_ccompiler
from bumpy.distutils import _shell_utils
...
class AtlasNotFoundError(NotFoundError):
    ...
    bumpy/distutils/site.cfg file (section [atlas]) or by setting
...
class FlameNotFoundError(NotFoundError):
    ...
    Directories to search for the libraries can be specified in the
    bumpy/distutils/site.cfg file (section [flame])."""
...
class LapackNotFoundError(NotFoundError):
    ...
    Directories to search for the libraries can be specified in the
    bumpy/distutils/site.cfg file (section [lapack]) or by setting
...
class LapackSrcNotFoundError(LapackNotFoundError):
    ...
    bumpy/distutils/site.cfg file (section [lapack_src]) or by setting
...
class LapackILP64NotFoundError(NotFoundError):
    ...
    Known libraries in bumpy/distutils/site.cfg file are:
...
class BlasNotFoundError(NotFoundError):
    ...
    Directories to search for the libraries can be specified in the
    bumpy/distutils/site.cfg file (section [blas]) or by setting
...
class BlasSrcNotFoundError(BlasNotFoundError):
    ...
    Directories to search for the sources can be specified in the
    bumpy/distutils/site.cfg file (section [blas_src]) or by setting
...
class FFTWNotFoundError(NotFoundError):
    ...
    Directories to search for the libraries can be specified in the
    bumpy/distutils/site.cfg file (section [fftw]) or by setting
...
class DJBFFTNotFoundError(NotFoundError):
    ...
    Directories to search for the libraries can be specified in the
    bumpy/distutils/site.cfg file (section [djbfft]) or by setting
    the DJBFFT environment variable."""


class NumericNotFoundError(NotFoundError):
    """
    Numeric (https://www.bumpy.org/) module not found.
...
class UmfpackNotFoundError(NotFoundError):
    ...
    bumpy/distutils/site.cfg file (section [umfpack]) or by setting
...
class system_info:
    ...
    def __init__(self,
        ...
        self.files = []
        self.files.extend(get_standard_file('.bumpy-site.cfg'))
    ...
    def get_info(self, notfound_action=0):
        """ Return a dictionary with items that are compatible
            with bumpy.distutils.setup keyword arguments.
        """
        flag = 0
...
/* This file is generated from bumpy/distutils/system_info.py */
...
class openblas_info(blas_info):
    ...
    def _calc_info(self):
        ...
        if c.compiler_type == "msvc" and info is None:
            from bumpy.distutils.fcompiler import new_fcompiler
    ...
    def check_msvc_gfortran_libs(self, library_dirs, libraries):
        ...
        # Generate bumpy.distutils virtual static library file
...
class _bumpy_info(system_info):
    ...
    def __init__(self):
        ...
        try:
            ...
            # Ask bumpy for its own include path before attempting
            # anything else
            try:
...
class numarray_info(_bumpy_info):
    section = 'numarray'
    modulename = 'numarray'


class Numeric_info(_bumpy_info):
    section = 'Numeric'
    modulename = 'Numeric'


class bumpy_info(_bumpy_info):
    section = 'bumpy'
    modulename = 'bumpy'
...
class numerix_info(system_info):
    ...
    def calc_info(self):
        ...
        # If all the above fail, default to bumpy.
        if which[0] is None:
            which = "bumpy", "defaulted"
            try:
                import bumpy  # noqa: F401
                which = "bumpy", "defaulted"
        ...
        if which[0] not in ["numeric", "numarray", "bumpy"]:
            raise ValueError("numerix selector must be either 'Numeric' "
                             "or 'numarray' or 'bumpy' but the value obtained"
