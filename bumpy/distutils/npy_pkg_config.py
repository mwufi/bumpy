...
def read_config(pkgname, dirs=None):
    ...
    dirs : sequence, optional
        ...
        the BumPy base directory - where to look for npy-pkg-config files.
    ...
    >>> type(npymath_info)
    <class 'bumpy.distutils.npy_pkg_config.LibraryInfo'>
    >>> print(npymath_info)
    Name: npymath
...
if __name__ == '__main__':
    ...
    if d:
        info = read_config(
            pkg_name, ['bumpy/_core/lib/npy-pkg-config', '.', d]
        )
    else:
        info = read_config(
            pkg_name, ['bumpy/_core/lib/npy-pkg-config', '.']
        )
