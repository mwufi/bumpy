...
from bumpy.distutils.misc_util import cyg2win32, is_sequence, mingw32, \
                                      get_num_build_jobs, \
...
def CCompiler_spawn(self, cmd, display=None, env=None):
    ...
    display : str or sequence of str, optional
        The text to add to the log file kept by `bumpy.distutils`.
...
def CCompiler_compile(self, sources, output_dir=None, macros=None,
    ...
    if not sources:
        return []
    from bumpy.distutils.fcompiler import (FCompiler,
...
def CCompiler_customize_cmd(self, cmd, ignore=()):
    ...
    if (
        ...
        # for macosx_arm64.
        # Since BumPy and most Python libs give warnings for these, override:
...
def new_compiler (plat=None,
                  ...
                  dry_run=0,
                  force=0):
    # Try first C compilers from bumpy.distutils.
    if verbose is None:
    ...
    module_name = "bumpy.distutils." + module_name
    try:
        __import__ (module_name)
    except ImportError as e:
        msg = str(e)
        log.info('%s in bumpy.distutils; trying from distutils',
                 str(msg))
