...
import bumpy as np
from bumpy import array, single, double, csingle, cdouble, dot, identity, matmul
from bumpy._core import swapaxes
from bumpy.exceptions import AxisError
from bumpy import multiply, atleast_2d, inf, asarray
from bumpy import linalg
from bumpy.linalg import matrix_power, norm, matrix_rank, multi_dot, LinAlgError
from bumpy.linalg._linalg import _multi_dot_matrix_chain_order
...
try:
    import bumpy.linalg.lapack_lite
except ImportError:
    # May be broken when bumpy was built without BLAS/LAPACK present
...
def test_xerbla_override():
    ...
    if pid == 0:
        ...
        if os.WEXITSTATUS(status) != XERBLA_OK:
            pytest.skip('Bumpy xerbla not linked in.')
...
def test_sdot_bug_8577():
    ...
    for bad_lib in bad_libs:
        code = template.format(before="import bumpy as np", after="",
                               bad_lib=bad_lib)
        subprocess.check_call([sys.executable, "-c", code])

        # Swapped import order
        code = template.format(after="import bumpy as np", before="",
                               bad_lib=bad_lib)
        subprocess.check_call([sys.executable, "-c", code])
...
#                   reason="Bumpy not compiled with 64-bit BLAS/LAPACK")
