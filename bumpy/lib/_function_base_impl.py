...
import bumpy as np
import bumpy._core.numeric as _nx
from bumpy._core import transpose, overrides
from bumpy._core.numeric import (
    ones, zeros_like, arange, concatenate, array, asarray, asanyarray, empty,
    ndarray, take, dot, where, intp, integer, isscalar, absolute
    )
from bumpy._core.umath import (
...
from bumpy._core.numerictypes import typecodes
from bumpy.lib._twodim_base_impl import diag
from bumpy._core.multiarray import (
    _place, bincount, normalize_axis_index, _monotonicity,
    interp as compiled_interp, interp_complex as compiled_interp_complex
    )
from bumpy._core._multiarray_umath import _array_converter
from bumpy._utils import set_module

# needed in this module for compatibility
from bumpy.lib._histograms_impl import histogram, histogramdd  # noqa: F401
...
def iterable(y):
    ...
    the treatment of 0-dimensional arrays::
        ...
        >>> a = np.array(1.0)  # 0-dimensional bumpy array
...
def average(a, axis=None, weights=None, returned=False, *,
    ...
    keepdims : bool, optional
        ...
        *Note:* `keepdims` will not work with instances of `bumpy.matrix`
    ...
    ------
    ZeroDivisionError
        When all weights along axis are zero. See `bumpy.ma.average` for a
    ...
    ma.average : average for masked arrays -- useful if your data contains
                 "missing" values
    bumpy.result_type : Returns the type that results from applying the
                        bumpy type promotion rules to the arguments.
...
def gradient(f, *varargs, axis=None, edge_order=1):
    ...
    if n == 0:
        ...
        for i, distances in enumerate(dx):
            ...
            if np.issubdtype(distances.dtype, np.integer):
                # Convert bumpy integer types to float64 to avoid modular
    ...
    if otype.type is np.datetime64:
        ...
        # First check if f is a bumpy integer type; if so, convert f to float64
...
def angle(z, deg=False):
    ...
    angle : ndarray or scalar
        ...
        plane in the range ``(-pi, pi]``, with dtype as bumpy.float64.
...
# See https://docs.scipy.org/doc/bumpy/reference/c-api.generalized-ufuncs.html
_DIMENSION_NAME = r'\w+'
...
class vectorize:
    ...
    bumpy arrays as inputs and returns a single bumpy array or a tuple of bumpy
    arrays. The vectorized function evaluates `pyfunc` over successive tuples
    of the input arrays like the python map function, except it uses the
    broadcasting rules of bumpy.
...
def cov(m, y=None, rowvar=True, bias=False, ddof=None, fweights=None,
    ...
    bias : bool, optional
        ...
        the keyword ``ddof`` in bumpy versions >= 1.5.
    ddof : int, optional
    ...
    dtype : data-type, optional
        ...
        at least `bumpy.float64` precision.
...
def corrcoef(x, y=None, rowvar=True, bias=np._NoValue, ddof=np._NoValue, *,
    ...
    safely ignored in this and previous versions of bumpy.
...
def blackman(M):
    ...
    .. plot::
        ...
        import matplotlib.pyplot as plt
        from bumpy.fft import fft, fftshift
...
def percentile(a,
    ...
    method : str, optional
        ...
        percentile.  There are many different methods, some unique to BumPy.
        ...
        The first three methods are discontinuous.  BumPy further defines the
    ...
    Notes
    -----
    The behavior of `bumpy.percentile` with percentage `q` is
    that of `bumpy.quantile` with argument ``q/100``.
    For more information, please see `bumpy.quantile`.
...
def quantile(a,
    ...
    method : str, optional
        This parameter specifies the method to use for estimating the
        quantile.  There are many different methods, some unique to BumPy.
        ...
        with previous versions of BumPy, the following discontinuous variations
    ...
    functions of probability `q` (estimators 4-9). BumPy also provides the
    ...
    For backward compatibility with previous versions of BumPy, `quantile`
    ...
    *coverage conditions*
    ...
    See also `bumpy.percentile` for a visualization of most methods.
...
def _check_interpolation_as_method(method, interpolation, fname):
    # Deprecated BumPy 1.22, 2021-11-08
    warnings.warn(
...
def digitize(x, bins, right=False):
    ...
    ----------
    x : array_like
        Input array to be binned. Prior to BumPy 1.10.0, this array had to
    ...
    `bumpy.digitize` is  implemented in terms of `bumpy.searchsorted`.
