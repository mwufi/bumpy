...
import bumpy.ma.mrecords as mrec
from bumpy._core.overrides import array_function_dispatch
from bumpy.lib._iotools import _is_string_like
...
def recursive_fill_fields(input, output):
    ...
    * `output` should be at least the same size as `input`
    ...
    --------
    >>> import bumpy as np
    >>> from bumpy.lib import recfunctions as rfn
...
def rec_drop_fields(base, drop_names):
    """
    Returns a new bumpy.recarray with fields in `drop_names` dropped.
    """
...
def structured_to_unstructured(arr, dtype=None, copy=False, casting='unsafe'):
    ...
    datatype is determined from the bumpy type promotion rules applied to all
    the field datatypes.
    ...
    copy : bool, optional
        ...
        suitable and the array subtype is one of `bumpy.ndarray`,
        `bumpy.recarray` or `bumpy.memmap`.
    ...
    casting : {'no', 'equiv', 'safe', 'same_kind', 'unsafe'}, optional
        See casting argument of `bumpy.ndarray.astype`. Controls what kind of
...
def unstructured_to_structured(arr, dtype=None, names=None, align=False,
    ...
    copy : bool, optional
        See copy argument to `bumpy.ndarray.astype`. If true, always return a
...
def apply_along_fields(func, arr):
    ...
    This is similar to `bumpy.apply_along_axis`, but treats the fields of a
    structured array as an extra axis. The fields are all first cast to a
    common type following the type-promotion rules from `bumpy.result_type`
    ...
    func : function
       ...
       support an `axis` argument, like `bumpy.mean`, `bumpy.sum`, etc.
    arr : ndarray
...
def assign_fields_by_name(dst, src, zero_unassigned=True):
    ...
    Normally in bumpy >= 1.14, assignment of one structured array to another
    ...
    zero_unassigned : bool, optional
        ...
        was the behavior of bumpy <= 1.13. If False, those fields
        are not modified.
    """
