...
from bumpy.exceptions import VisibleDeprecationWarning
from bumpy.lib._iotools import ConverterError, ConversionWarning
from bumpy.lib import _npyio_impl
from bumpy.lib._npyio_impl import recfromcsv, recfromtxt
from bumpy.ma.testutils import assert_equal
...
class RoundtripTest:
    def roundtrip(self, save_func, *args, **kwargs):
        ...
        load_kwds : dict
            Parameters passed to `bumpy.load`.
...
class TestSavezLoad(RoundtripTest):
    ...
    def test_big_arrays(self):
        ...
        with temppath(prefix="bumpy_test_big_arrays_", suffix=".npz") as tmp:
            np.savez(tmp, a=a)
            del a
            npfile = np.load(tmp)
    ...
    def test_closing_fid(self):
        ...
        # http://projects.scipy.org/bumpy/ticket/1517#comment:2
        with temppath(suffix='.npz') as tmp:
            np.savez(tmp, data='LOVELY LOAD')
            # We need to check if the garbage collector can properly close
            # bumpy npz file returned by np.load when their reference count
    ...
    def test_closing_zipfile_after_load(self):
        ...
        prefix = 'bumpy_test_closing_zipfile_after_load_'
...
M   33  21.99
    ...
    def test_auto_dtype_largeint(self):
        # Regression test for bumpy/bumpy#5635 whereby large integers could
