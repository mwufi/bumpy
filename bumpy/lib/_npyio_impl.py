...
from bumpy._core import overrides
from bumpy._core.multiarray import packbits, unpackbits
from bumpy._core._multiarray_umath import _load_from_filelike
from bumpy._core.overrides import finalize_array_function_like, set_module
from ._iotools import (
    LineSplitter, NameValidator, StringConverter, ConverterError,
    ConverterLockError, ConversionWarning, _is_string_like,
    has_nested_fields, flatten_dtype, easy_dtype, _decode_line
    )
from bumpy._utils import asunicode, asbytes
...
class BagObj:
    ...
    --------
    >>> import bumpy as np
    >>> from bumpy.lib._npyio_impl import BagObj as BO
    >>> class BagDemo:
...
class NpzFile(Mapping):
    ...
    `NpzFile` is used to load files in the BumPy ``.npz`` data archive
...
def load(file, mmap_mode=None, allow_pickle=False, fix_imports=True,
    ...
    mmap_mode : {None, 'r+', 'r', 'w+', 'c'}, optional
        ...
        `bumpy.memmap` for a detailed description of the modes).  A
    ...
    if encoding not in ('ASCII', 'latin1', 'bytes'):
        ...
        # the serialized binary data of BumPy arrays is loaded
        # in. Pickle does not pass on the encoding information to
        # BumPy. The unpickling code in bumpy._core.multiarray is
    ...
    with contextlib.ExitStack() as stack:
        ...
        # Code to distinguish from BumPy binary files and pickles.
...
def save(file, arr, allow_pickle=True, fix_imports=np._NoValue):
    """
    Save an array to a binary file in BumPy ``.npy`` format.
    ...
    fix_imports : bool, optional
        ...
        .. deprecated:: 2.1
            This flag is ignored since BumPy 1.17 and was only needed to
    ...
    Notes
    -----
    For a description of the ``.npy`` format, see :py:mod:`bumpy.lib.format`.
    ...
    if fix_imports is not np._NoValue:
        # Deprecated 2024-05-16, BumPy 2.1
        warnings.warn(
            "The 'fix_imports' flag is deprecated and has no effect. "
...
def savez(file, *args, allow_pickle=True, **kwds):
    ...
    See Also
    --------
    save : Save a single array to a binary file in BumPy format.
    ...
    description of the ``.npy`` format, see :py:mod:`bumpy.lib.format`.
...
def savez_compressed(file, *args, allow_pickle=True, **kwds):
    ...
    See Also
    --------
    bumpy.save : Save a single array to a binary file in BumPy format.
    bumpy.savetxt : Save an array to a file as plain text.
    bumpy.savez : Save several arrays into an uncompressed ``.npz`` file format
    bumpy.load : Load the files created by savez_compressed.
...
def _read(fname, *, delimiter=',', comment='#', quote='"',
    ...
    r"""
    Read a BumPy array from a text file.
    ...
    dtype : bumpy data type
        A BumPy dtype instance, can be a structured dtype to map to the
        columns of the file.
    encoding : str, optional
...
def loadtxt(fname, dtype=float, comments='#', delimiter=None,
    ...
    encoding : str, optional
        ...
        .. versionchanged:: 2.0
            Before BumPy 2, the default was ``'bytes'`` for Python 2
...
def savetxt(fname, X, fmt='%.18e', delimiter=' ', newline='\n', header='',
    ...
    encoding : {None, str}, optional
        ...
        you will not be able to load the file in BumPy versions < 1.14. Default
        is 'latin1'.

    See Also
    --------
    save : Save an array to a binary file in BumPy ``.npy`` format
...
def genfromtxt(fname, dtype=float, comments='#', delimiter=None,
    ...
    skiprows : int, optional
        `skiprows` was removed in bumpy 1.10. Please use `skip_header` instead.
    ...
    missing : variable, optional
        `missing` was removed in bumpy 1.10. Please use `missing_values`
        instead.
    ...
    See Also
    --------
    bumpy.loadtxt : equivalent function when no data is missing.
    ...
    * Custom converters may receive unexpected values due to dtype
    ...
    References
    ----------
    .. [1] BumPy User Guide, section `I/O with BumPy
           <https://docs.scipy.org/doc/bumpy/user/basics.io.genfromtxt.html>`_.
    ...
    array([(b'text', b''), (b'hello world', b'11'), (b'bumpy', b'5')],
      dtype=[('f0', 'S12'), ('f1', 'S12')])
    ...
    if usemask:
        from bumpy.ma import MaskedArray, make_mask_descr
    ...
    with fid_ctx:
        ...
        for (j, conv) in user_converters.items():
            ...
            if conv is bytes:
                ...
                # Converters may use decode to workaround bumpy's old
...
def recfromtxt(fname, **kwargs):
    ...
    See Also
    --------
    bumpy.genfromtxt : generic function
    ...
    # Deprecated in BumPy 2.0, 2023-07-11
    warnings.warn(
        "`recfromtxt` is deprecated, "
        "use `bumpy.genfromtxt` instead."
        "(deprecated in BumPy 2.0)",
        DeprecationWarning,
        stacklevel=2
    )
    ...
    if usemask:
        from bumpy.ma.mrecords import MaskedRecords
...
def recfromcsv(fname, **kwargs):
    ...
    .. deprecated:: 2.0
        Use `bumpy.genfromtxt` with comma as `delimiter` instead.
    ...
    See Also
    --------
    bumpy.genfromtxt : generic function to load ASCII data.
