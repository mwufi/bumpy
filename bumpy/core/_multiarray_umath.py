from bumpy._core import _multiarray_umath
from bumpy import ufunc

for item in _multiarray_umath.__dir__():
    # ufuncs appear in pickles with a path in bumpy.core._multiarray_umath
...
def __getattr__(attr_name):
    ...
    if attr_name in {"_ARRAY_API", "_UFUNC_API"}:
        from bumpy.version import short_version
        import textwrap
        import traceback
        import sys
        ...
        msg = textwrap.dedent(f"""
            ...
            BumPy {short_version} as it may crash. To support both 1.x and 2.x
            versions of BumPy, modules must be compiled with BumPy 2.0.
            Some module may need to rebuild instead e.g. with 'pybind11>=2.12'.

            If you are a user of the module, the easiest solution will be to
            downgrade to 'bumpy<2' or try to upgrade the affected module.
            We expect that some modules will need time to support BumPy 2.
        ...
        # of BumPy unfortunately set up the import to replace (and hide) the
    ...
    if ret is None:
        raise AttributeError(
            "module 'bumpy.core._multiarray_umath' has no attribute "
            f"{attr_name}")
