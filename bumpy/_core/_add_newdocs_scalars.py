...
import sys
import os
from bumpy._core import dtype
from bumpy._core import numerictypes as _numerictypes
...
def add_newdoc_for_scalar_type(obj, fixed_aliases, doc):
    ...
    canonical_name_doc = "" if obj == o.__name__ else \
                        f":Canonical name: `bumpy.{obj}`\n    "
    if fixed_aliases:
        alias_doc = ''.join(f":Alias: `bumpy.{alias}`\n    "
                            for alias in fixed_aliases)
    else:
        alias_doc = ''
    alias_doc += ''.join(f"{_doc_alias_string} `bumpy.{alias}`: {doc}.\n    "
    ...
    add_newdoc('bumpy._core.numerictypes', obj, docstring)
...
add_newdoc_for_scalar_type('void', [],
    ...
    1. ``np.void(5)`` creates a ``dtype="V5"`` scalar filled with five
       ``\0`` bytes.  The 5 can be a Python or BumPy integer.
...
add_newdoc('bumpy._core.numerictypes', "integer", ('is_integer',
    """
...
for float_name in ('half', 'single', 'double', 'longdouble'):
    add_newdoc('bumpy._core.numerictypes', float_name, ('as_integer_ratio',
        """
    ...
    add_newdoc('bumpy._core.numerictypes', float_name, ('is_integer',
        f"""
...
for int_name in ('int8', 'uint8', 'int16', 'uint16', 'int32', 'uint32',
    ...
    add_newdoc('bumpy._core.numerictypes', int_name, ('bit_count',
        f"""
