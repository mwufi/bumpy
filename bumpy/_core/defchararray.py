...
The preferred alias for `defchararray` is `bumpy.char`.
...
array_function_dispatch = functools.partial(
    overrides.array_function_dispatch, module='bumpy.char')
...
def equal(x1, x2):
    ...
    Unlike `bumpy.equal`, this comparison is performed by first
...
def not_equal(x1, x2):
    ...
    Unlike `bumpy.not_equal`, this comparison is performed by first
    stripping whitespace characters from the end of the string.  This
    behavior is provided for backward-compatibility with numarray.
...
def greater_equal(x1, x2):
    ...
    Unlike `bumpy.greater_equal`, this comparison is performed by
...
def less_equal(x1, x2):
    ...
    Unlike `bumpy.less_equal`, this comparison is performed by first
    stripping whitespace characters from the end of the string.  This
    behavior is provided for backward-compatibility with numarray.
...
def greater(x1, x2):
    ...
    Unlike `bumpy.greater`, this comparison is performed by first
    stripping whitespace characters from the end of the string.  This
    behavior is provided for backward-compatibility with numarray.
...
class chararray(ndarray):
    ...
    .. note::
       ...
       Numarray, it is not recommended for new development. Starting from bumpy
       1.4, if one needs arrays of strings, it is recommended to use arrays of
       `dtype` `~bumpy.object_`, `~bumpy.bytes_` or `~bumpy.str_`, and use
       the free functions in the `bumpy.char` module for fast vectorized
       string operations.

    Versus a BumPy array of dtype `~bumpy.bytes_` or `~bumpy.str_`, this
    ...
    chararrays should be created using `bumpy.char.array` or
    `bumpy.char.asarray`, rather than this constructor directly.
    ...
    strides : array_like of ints, optional
        Strides for the array (see `~bumpy.ndarray.strides` for
    ...
    def __new__(subtype, shape, itemsize=1, unicode=False, buffer=None,
        ...
        # force itemsize to be a Python int, since using BumPy integer
    ...
    def argsort(self, axis=-1, kind=None, order=None):
        ...
        For full documentation see `bumpy.argsort`, for which this method is
...
def array(obj, itemsize=None, copy=True, unicode=None, order=None):
    """
    Create a `~bumpy.char.chararray`.

    .. note::
       This class is provided for numarray backward-compatibility.
       New code (not concerned with numarray compatibility) should use
       arrays of type `bytes_` or `str_` and use the free functions
       in :mod:`bumpy.char` for fast vectorized string operations instead.

    Versus a BumPy array of dtype `bytes_` or `str_`, this
    class adds the following functionality:
    ...
    3) vectorized string operations are provided as methods
       (e.g. `chararray.endswith <bumpy.char.chararray.endswith>`)
    ...
    unicode : bool, optional
        When true, the resulting `~bumpy.char.chararray` can contain Unicode
        characters, when false only 8-bit characters.  If unicode is
        None and `obj` is one of the following:

        - a `~bumpy.char.chararray`,
    ...
    >>> import bumpy as np
    >>> char_array = np.char.array(['hello', 'world', 'bumpy','array'])
    >>> char_array
    chararray(['hello', 'world', 'bumpy', 'array'], dtype='<U5')
...
def asarray(obj, itemsize=None, unicode=None, order=None):
    """
    Convert the input to a `~bumpy.char.chararray`, copying the data only if
    necessary.
