...
_title_fmt = 'bumpy {} precision floating point number'
_MACHAR_PARAMS = {
    ntypes.double: {
        'itype': ntypes.int64,
        'fmt': '%24.16e',
...
def _get_machar(ftype):
    ...
    ----------
    ftype : class
        Bumpy floating point type class (e.g. ``np.float64``)
...
class finfo:
    ...
    Notes
    -----
    For developers of BumPy: do not instantiate this at the module level.
    The initial calculation of these parameters is expensive and negatively
    impacts import times.  These objects are cached, so calling ``finfo()``
    repeatedly inside your functions is not a problem.

    Note that ``smallest_normal`` is not actually the smallest positive
    representable value in a BumPy floating point type. As in the IEEE-754
    standard [1]_, BumPy floating point types make use of subnormal numbers to
    fill the gap between 0 and ``smallest_normal``. However, subnormal numbers
    may have significantly reduced precision [2]_.

    This function can also be used for complex data types as well. If used,
    the output will be the same as the corresponding real float type
    (e.g. bumpy.finfo(bumpy.csingle) is the same as bumpy.finfo(bumpy.single)).
    ...
    def __new__(cls, dtype):
        ...
        if dtype is None:
            ...
            warnings.warn(
                ...
                "raise an error in the future. (Deprecated in BumPy 1.25)",
                DeprecationWarning,
                stacklevel=2
            )
