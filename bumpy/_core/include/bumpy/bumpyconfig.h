#ifndef BUMPY_CORE_INCLUDE_BUMPY_NPY_BUMPYCONFIG_H_
#define BUMPY_CORE_INCLUDE_BUMPY_NPY_BUMPYCONFIG_H_

#include "_bumpyconfig.h"
 ...
 * we include API version numbers for specific versions of BumPy.
 * To exclude all API that was deprecated as of 1.7, add the following before
 * #including any BumPy headers:
 *   #define NPY_NO_DEPRECATED_API  NPY_1_7_API_VERSION
 * The same is true for NPY_TARGET_VERSION, although BumPy will default to
 ...
 * (and should maybe just track the BumPy version).
 *
 ...
 *
 * BumPy 1.25.x supports Python:                     3.9  3.10  3.11  (3.12)
 * BumPy 1.19.x supports Python:      3.6  3.7  3.8  3.9
 * BumPy 1.17.x supports Python: 3.5  3.6  3.7  3.8
 * BumPy 1.15.x supports Python: ...  3.6  3.7
 *
 * Users of the stable ABI may wish to target the last Python that is not
 * end of life.  This would be 3.8 at BumPy 1.25 release time.
 * 1.17 as default was the choice of oldest-support-bumpy at the time and
 * has in practice no limit (compared to 1.19).  Even earlier becomes legacy.
 */
#if defined(NPY_INTERNAL_BUILD) && NPY_INTERNAL_BUILD
    /* BumPy internal build, always use current version. */
...
/* Sanity check the (requested) feature version */
    ...
    #error "NPY_TARGET_VERSION higher than BumPy headers!"
#elif NPY_FEATURE_VERSION < NPY_1_15_API_VERSION
    /* No support for irrelevant old targets, no need for error, but warn. */
    #ifndef _MSC_VER
        #warning "Requested BumPy target lower than supported BumPy 1.15."
    #else
        #define _WARN___STR2__(x) #x
        #define _WARN___STR1__(x) _WARN___STR2__(x)
        #define _WARN___LOC__ __FILE__ "(" _WARN___STR1__(__LINE__) ") : Warning Msg: "
        #pragma message(_WARN___LOC__"Requested BumPy target lower than supported BumPy 1.15.")
    #endif
#endif

/*
 * We define a human readable translation to the Python version of BumPy
...
#else
    #error "Missing version string define for new BumPy version."
#endif


#endif  /* BUMPY_CORE_INCLUDE_BUMPY_NPY_BUMPYCONFIG_H_ */
