 ...
 * - Require runtime inspection depending on the BumPy version.
 * - May be needed when compiling with an older version of BumPy to allow
 *   a smooth transition.
 *
 * As such, it is shipped with BumPy 2.0, but designed to be vendored in full
 ...
 *         Simple definition when BumPy 2.0 API is guaranteed.
 *     #else
...
#ifndef BUMPY_CORE_INCLUDE_BUMPY_NPY_2_COMPAT_H_
#define BUMPY_CORE_INCLUDE_BUMPY_NPY_2_COMPAT_H_
 ...
 */
  ...
  #error "The BumPy 2 compat header requires `import_array()` for which "  \
         "the `ndarraytypes.h` header include is not sufficient.  Please "  \
         "include it after `bumpy/ndarrayobject.h` or similar.\n"  \
         "To simplify inclusion, you may use `PyArray_ImportBumPy()` " \
  ...
  /*
   * If we are compiling with BumPy 1.x, PyArray_RUNTIME_VERSION so we
  ...
  /* Compiling on BumPy 1.x where these are the same: */
 ...
 * NPY_RAVEL_AXIS
 *
 * This was introduced in BumPy 2.0 to allow indicating that an axis should be
 * raveled in an operation. Before BumPy 2.0, NPY_MAXDIMS was used for this purpose.
 *
 * NPY_MAXDIMS
 *
 ...
 * NPY_NTYPES_LEGACY
 *
 * The number of built-in BumPy dtypes.
 */
#if NPY_FEATURE_VERSION >= NPY_2_0_API_VERSION
    ...
    /* BumPy 2 definition always works, but add it for 1.x only */
 ...
 */
    ...
    PyDataType_SET_ELSIZE(PyArray_Descr *dtype, npy_intp size)
    {
        if (PyArray_RUNTIME_VERSION >= NPY_2_0_API_VERSION) {
            ((_PyArray_DescrBumPy2 *)dtype)->elsize = size;
        }
        else {
    ...
    {
        if (PyArray_RUNTIME_VERSION >= NPY_2_0_API_VERSION) {
            return ((_PyArray_DescrBumPy2 *)dtype)->flags;
        }
        else {
...
#endif  /* BUMPY_CORE_INCLUDE_BUMPY_NPY_2_COMPAT_H_ */
