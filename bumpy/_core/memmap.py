...
class memmap(ndarray):
    ...
    on disk, without reading the entire file into memory.  BumPy's
    ...
    Given a memmap ``fp``, ``isinstance(fp, bumpy.ndarray)`` returns
    ``True``.
    ...
    def __new__(subtype, filename, dtype=uint8, mode='r+', offset=0,
                shape=None, order='C'):
        # Import here to minimize 'import bumpy' overhead
        import mmap
        import os.path
        try:
