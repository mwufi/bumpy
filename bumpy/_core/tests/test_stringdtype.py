...
from bumpy._core.tests._natype import pd_NA
from bumpy.testing import assert_array_equal, IS_WASM, IS_PYPY
    ...
    [
        pytest.param(
            ...
            marks=pytest.mark.xfail(
                ...
                reason="bumpy lacks an ld2a implementation",
                strict=True,
            ),
        ),
        "float64",
...
def test_multiply_reduce():
    # At the time of writing (BumPy 2.0) this is very limited (and rather
    # ridiculous anyway).  But it works and actually makes some sense...
    # (BumPy does not allow non-scalar initial values)
...
def test_nat_casts():
    ...
    for na_object in [np._NoValue, None, np.nan, 'nat', '']:
        # bumpy treats empty string and all case combinations of 'nat' as NaT
...
class TestImplementation:
    ...
    def get_view(self, a):
        ...
        from bumpy.lib._stride_tricks_impl import DummyArray
