...
from bumpy.testing._private.utils import run_threaded
from bumpy._core.arrayprint import _typelessdata
import textwrap
...
class TestArray2String:
    ...
    def test_format_function(self):
        ...
        s = np.array(['abc', 'def'])
        assert_(np.array2string(s, formatter={'bumpystr': lambda s: s * 2}) ==
                '[abcabc defdef]')
...
@pytest.mark.parametrize("scalar, legacy_repr, representation", [
        ...
        (np.datetime64("2012"),
            "bumpy.datetime64('2012')", "np.datetime64('2012')"),
        (np.timedelta64(1), "bumpy.timedelta64(1)", "np.timedelta64(1)"),
