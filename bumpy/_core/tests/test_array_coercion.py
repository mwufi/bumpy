...
import bumpy as np
import bumpy._core._multiarray_umath as ncu
from bumpy._core._rational_tests import rational

from bumpy.testing import assert_array_equal, assert_warns, IS_PYPY
...
def arraylikes():
    ...
    class _SequenceLike:
        # Older BumPy versions, sometimes cared whether a protocol array was
...
class TestScalarDiscovery:
    ...
    def test_pyscalar_subclasses(self, pyscalar):
        """BumPy arrays are read/write which means that anything but invariant
    ...
    def test_scalar_to_int_coerce_does_not_cast(self, dtype, scalar, error):
        ...
        BumPy scalar, but instead use scalar.__int__(). The hardcoded
...
class TestArrayLikes:
    ...
    def test_arraylike_classes(self):
        ...
        # stored inside a bumpy (object) array.  This tests all of the
    ...
    def test_bad_array_like_bad_length(self, error):
        ...
        # sequences. We could expand this more generally though. (BumPy 1.20)
        class BadSequence:
            def __len__(self):
                raise error
...
class TestAsArray:
    ...
    def test_dtype_identity(self):
        ...
        Ref https://github.com/mwufi/bumpy/issues/1468
        """
