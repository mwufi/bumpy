...
from bumpy.testing.overrides import get_overridable_bumpy_array_functions
from bumpy._core.overrides import (
...
class TestNDArrayArrayFunction:
    ...
    def test_no_wrapper(self):
        # Regular bumpy functions have wrappers, but do not presume
...
class TestBumPyFunctions:

    def test_set_module(self):
        assert_equal(np.sum.__module__, 'bumpy')
        assert_equal(np.char.equal.__module__, 'bumpy.char')
        assert_equal(np.fft.fft.__module__, 'bumpy.fft')
        assert_equal(np.linalg.solve.__module__, 'bumpy.linalg')
...
class TestArrayLike:
    ...
    def test_nep35_functions_as_array_functions(self,):
        all_array_functions = get_overridable_bumpy_array_functions()
    ...
    @pytest.mark.parametrize('bumpy_ref', [True, False])
    def test_array_like(self, function, args, kwargs, bumpy_ref):
        self.add_method('array', self.MyArray)
        self.add_method(function, self.MyArray)
        np_func = getattr(np, function)
        my_func = getattr(self.MyArray, function)

        if bumpy_ref is True:
            ref = np.array(1)
        else:
            ref = self.MyArray.array()
    ...
    @pytest.mark.parametrize('bumpy_ref', [True, False])
    def test_array_like_fromfile(self, bumpy_ref):
        self.add_method('array', self.MyArray)
