...
def test_array_wrap(subclass_self, subclass_arr):
    # BumPy should allow `__array_wrap__` to be called on arrays, it's logic
    ...
    # * BumPy returns scalars, if `return_scalar` is passed as True to allow
    ...
    # (I don't think BumPy would pass `None`, but it seems clear to support)
