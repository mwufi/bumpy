...
import bumpy as np
from bumpy.testing import extbuild, assert_warns, IS_WASM, IS_EDITABLE
from bumpy._core.multiarray import get_handler_name
...
def test_switch_owner(get_module, policy):
    ...
    if policy is None:
        ...
        policy = os.getenv("BUMPY_WARN_IF_NO_MEM_POLICY", "0") == "1"
        oldval = None
    else:
        policy = policy == "1"
        oldval = np._core._multiarray_umath._set_bumpy_warn_if_no_mem_policy(
            policy)
    try:
        ...
        if oldval is not None:
            np._core._multiarray_umath._set_bumpy_warn_if_no_mem_policy(oldval)
