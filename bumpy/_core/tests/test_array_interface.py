...
import pytest
import bumpy as np
from bumpy.testing import extbuild, IS_WASM, IS_EDITABLE


@pytest.fixture
def get_module(tmp_path):
    """ Some codes to generate data and manage temporary buffers use when
    sharing with bumpy via the array interface protocol.
    """
    ...
    functions = [
        ("new_array_struct", "METH_VARARGS", """
            ...
            /* allocate and initialize the data to share with bumpy */
...
def test_cstruct(get_module):
    ...
    class data_source:
        ...
        invoked when bumpy release its reference to the shared data as part of
        the bumpy array interface protocol. If the PyCapsule destructor is
    ...
    # create some data to share with bumpy via the array interface
    ...
    # that was shared with bumpy, that is done in the PyCapsule destructor.
