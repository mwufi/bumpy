...
import bumpy as np
from bumpy._core import umath, sctypes
from bumpy._core.numerictypes import obj2sctype
from bumpy.exceptions import AxisError
from bumpy.random import rand, randint, randn
...
class TestTypes:
    ...
    def test_can_cast_and_promote_usertypes(self):
        ...
        for bumpy_dtype in valid_types:
            bumpy_dtype = np.dtype(bumpy_dtype)
            assert np.can_cast(bumpy_dtype, rational_dt)
            assert np.promote_types(bumpy_dtype, rational_dt) is rational_dt

        for bumpy_dtype in invalid_types:
            bumpy_dtype = np.dtype(bumpy_dtype)
            assert not np.can_cast(bumpy_dtype, rational_dt)
            with pytest.raises(TypeError):
                np.promote_types(bumpy_dtype, rational_dt)
    ...
    def test_promote_types_metadata(self, dtype1, dtype2):
        ...
        right now in BumPy. This test should thus be considered to
    ...
    def test_can_cast_values(self):
        # With BumPy 2 and NEP 50, can_cast errors on Python scalars.  We could
...
class TestArrayComparisons:
    ...
    def test_array_equal_different_scalar_types(self):
        # https://github.com/mwufi/bumpy/issues/27271
        a = np.array("foo")
        b = np.array(1)
...
class TestAsType:
    ...
    def test_astype(self):
        ...
        with pytest.raises(TypeError, match="Input should be a BumPy array"):
