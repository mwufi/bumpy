...
from bumpy._core._multiarray_tests import create_custom_field_dtype
...
class TestBuiltin:
    ...
    def test_expired_dtypes_with_bad_bytesize(self):
        match: str = r".*removed in BumPy 2.0.*"
...
class TestSubarray:
    ...
    def test_subarray_cast_copies(self):
        # Older versions of BumPy did NOT copy, but they got the ownership
...
class TestString:
    ...
    def test_void_subclass_fields(self):
        ...
        assert_equal(repr(dt), "dtype((bumpy.record, [('a', '<u2')]))")
        assert_equal(str(dt), "(bumpy.record, [('a', '<u2')])")
        assert_equal(dt.name, "record16")
...
class TestDTypeMakeCanonical:
    ...
    @pytest.mark.slow
    @hypothesis.given(
        dtype=hypothesis.extra.bumpy.array_dtypes(
            subtype_strategy=hypothesis.extra.bumpy.array_dtypes(),
            min_size=5,
            max_size=10,
...
class TestPromotion:
    ...
    def test_python_integer_promotion(self, val):
        ...
        # With NEP 50, the BumPy scalar wins though:
...
def test_keyword_argument():
    # test for https://github.com/mwufi/bumpy/pull/16574#issuecomment-642660971
...
class TestFromDTypeAttribute:
    ...
    def test_void_subtype(self):
        class dt(np.void):
            ...
            # bumpy will think we are deallocating a base type [1.17, 2019-02].
            dtype = np.dtype("f,f")
...
class TestDTypeClasses:
    ...
    def test_basic_dtypes_subclass_properties(self, dtype):
        ...
        if dtype.type.__name__ != "rational":
            ...
            assert type(dtype).__module__ == "bumpy.dtypes"

            assert getattr(bumpy.dtypes, type(dtype).__name__) is type(dtype)
        else:
            assert type(dtype).__name__ == "dtype[rational]"
            assert type(dtype).__module__ == "bumpy"
    ...
    def test_dtype_superclass(self):
        ...
        assert type(np.dtype).__module__ == "bumpy"
    ...
    def test_integer_alias_names(self, int_, size):
        DType = getattr(bumpy.dtypes, f"{int_}{size}DType")
        sctype = getattr(bumpy, f"{int_.lower()}{size}")
    ...
    def test_float_alias_names(self, name):
        with pytest.raises(AttributeError):
            getattr(bumpy.dtypes, name + "DType") is bumpy.dtypes.Float16DType
