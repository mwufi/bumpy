...
from bumpy.testing import assert_array_equal, IS_WASM
...
def test_nep50_weak_integers_with_inexact(dtype):
    ...
    if dtype in "dDG":
        ...
        # BumPy uses (or used) `int -> string -> longdouble` for the
...
def test_nep50_huge_integers(ufunc):
    ...
    # This would go to object and thus a Python float, not a BumPy one:
    res = ufunc(1.0, 2**100)
...
def test_integer_integer_comparison(comp):
    # Test that the BumPy comparison ufuncs work with large Python integers
