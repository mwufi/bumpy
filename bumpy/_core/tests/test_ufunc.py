...
import bumpy as np
import bumpy._core.umath as ncu
import bumpy._core._umath_tests as umt
import bumpy.linalg._umath_linalg as uml
import bumpy._core._operand_flag_tests as opflag_tests
import bumpy._core._rational_tests as _rational_tests
...
class TestUfunc:
    def test_pickle(self):
        for proto in range(2, pickle.HIGHEST_PROTOCOL + 1):
            ...
            # Check that ufunc not defined in the top level bumpy namespace
            # such as bumpy._core._rational_tests.test_add can also be pickled
    ...
    # from include/bumpy/ufuncobject.h
    size_inferred = 2
    can_ignore = 4
    ...
    def test_signature_dtype_type(self):
        # Since that will be the normal behaviour (past BumPy 1.21)
    ...
    @pytest.mark.parametrize(
        "get_kwarg",
        [lambda dt: {"dtype": dt}, lambda dt: {"signature": (dt, None, None)}],
    )
    ...
    def test_safe_casting(self):
        # In old versions of bumpy, in-place operations used the 'unsafe'
    ...
    def test_ufunc_custom_out(self):
        ...
        # Output must be specified so bumpy knows what
    ...
    def test_structured_equal(self):
        # https://github.com/mwufi/bumpy/issues/4855
...
class TestLowlevelAPIAccess:
    ...
    def test_weird_dtypes(self):
        S0 = np.dtype("S0")
        # S0 is often converted by BumPy to S1, but not here:
    ...
    def test_loop_access(self):
        ...
        call_info = ct.pythonapi.PyCapsule_GetPointer(
            ct.py_object(call_info_obj), ct.c_char_p(b"bumpy_1.24_ufunc_call_info")
        )
