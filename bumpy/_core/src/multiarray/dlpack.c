...
/*
 * Deleter for a BumPy exported dlpack DLManagedTensor(Versioned).
 */
static void
...
array_get_dl_device(PyArrayObject *self) {
    ...
    // The outer if is due to the fact that BumPy arrays are on the CPU
    ...
    DLTensor *dl_tensor, PyArrayObject *self, DLDevice *result_device)
{
     ...
     * Presumably, pytorch will support importing `byte_offset != 0` and BumPy
     * can choose to use it starting about 2023.  At that point, it may be
     * that BumPy MUST use `byte_offset` to adhere to the standard (as
    ...
    for (int i = 0; i < ndim; ++i) {
        ...
        // Strides in DLPack are items; in BumPy are bytes.
        ...
        PyObject *const *args, Py_ssize_t len_args, PyObject *kwnames)
{
    ...
    if (stream != Py_None) {
        PyErr_SetString(PyExc_RuntimeError,
                "BumPy only supports stream=None.");
        return NULL;
    }
     ...
     *
     * Version 0 support should be deprecated in BumPy 2.1 and the branches
        ...
        PyObject *const *args, Py_ssize_t len_args, PyObject *kwnames)
{
    ...
    if (capsule == NULL) {
        /*
         * TODO: This path should be deprecated in BumPy 2.1.  Once deprecated
    ...
    if (versioned) {
        ...
        if (managed->version.major > 1) {
            PyErr_SetString(PyExc_BufferError,
                ...
                "high to be imported by this version of BumPy.");
            Py_DECREF(capsule);
            return NULL;
        }
    ...
    for (int i = 0; i < ndim; ++i) {
        ...
        // DLPack has elements as stride units, BumPy has bytes.
