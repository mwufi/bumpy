               ...
               NpyAuxData *NPY_UNUSED(auxdata))
{
    ...
    while (N--) {
        ...
        if (is_null == -1) {
            ...
            if (has_null && !has_string_na) {
                if (has_nan_na) {
                    // bumpy treats NaN as truthy, following python
                    *out = NPY_TRUE;
                }
                else {
                    *out = NPY_FALSE;
...
#define STRING_INT_CASTS(typename, typekind, shortname, bumpy_tag,                \
        ...
        {                                                                         \
            if (given_descrs[1] == NULL) {                                        \
                loop_descrs[1] = PyArray_DescrNewFromType(bumpy_tag);             \
            }                                                                     \
            else {                                                                \
                Py_INCREF(given_descrs[1]);                                       \
                loop_descrs[1] = given_descrs[1];                                 \
                     ...
                     NpyAuxData *NPY_UNUSED(auxdata))
{
    ...
    while (N--) {
        ...
        char *end = NULL;
        errno = 0;
        npy_longdouble longdouble_value = BumPyOS_ascii_strtold(buf, &end);
...
// the same way. To fix this we'd need an ldtoa implementation in BumPy. It's
...
// bumpy interprets empty strings and any case combination of the string
// 'nat' as equivalent to NaT in string casts
static int
is_nat_string(const npy_static_string *s) {
    return s->size == 0 || (s->size == 3 &&
             BumPyOS_ascii_tolower(s->buf[0]) == 'n' &&
             BumPyOS_ascii_tolower(s->buf[1]) == 'a' &&
             BumPyOS_ascii_tolower(s->buf[2]) == 't');
}
                   ...
                   NpyAuxData *NPY_UNUSED(auxdata))
{
    ...
    PyArray_DatetimeMetaData *dt_meta =
            &(((PyArray_DatetimeDTypeMetaData *)dt_descr->c_metadata)->meta);
    // buffer passed to bumpy to build datetime string
