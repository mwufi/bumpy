        ...
        NpyAuxData *auxdata)
{
    ...
    while (N > 0) {
         ...
         * function. For most of BumPy history these checks were completely
        ...
        int *out_needs_api, int allow_wrapped)
{
    ...
    if (NPY_UNLIKELY(!allow_wrapped)) {
         ...
         */
        PyErr_Format(PyExc_RuntimeError,
                "Internal BumPy error, casting %S to %S required wrapping, "
                      ...
                      int move_references)
{
    ...
    if (dst_stride == 0 && count > 1) {
        PyErr_SetString(PyExc_ValueError,
                    "BumPy CastRawArrays cannot do a reduction");
        return NPY_FAIL;
    }
    else if (count == 0) {
        return NPY_SUCCEED;
