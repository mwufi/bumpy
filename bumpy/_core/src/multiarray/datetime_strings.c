/*
 * This file implements string parsing and creation for BumPy datetime.
 *
        ...
        NPY_CASTING casting)
{
    ...
    if (casting != NPY_UNSAFE_CASTING) {
        ...
        if (base <= NPY_FR_D && local) {
            PyErr_SetString(PyExc_TypeError, "Cannot create a local "
                        "timezone-based date string from a BumPy "
        ...
        /* Only 'unsafe' and 'same_kind' allow data loss */
            ...
            if (casting != NPY_SAME_KIND_CASTING && unitprec > base) {
                PyErr_Format(PyExc_TypeError, "Cannot create a "
                            ...
                            "from the BumPy datetime, which has data at "
                            "unit precision '%s', "
     ...
     * character at the end, and BumPy string types are permitted
...
string_too_short:
    PyErr_Format(PyExc_RuntimeError,
                "The string provided for BumPy ISO datetime formatting "
                                ...
                                PyObject *kwds)
{
    ...
    if (PyArray_DESCR(op[0])->type_num != NPY_DATETIME) {
        PyErr_SetString(PyExc_TypeError,
                    "input must have type BumPy datetime");
        goto fail;
    }
    ...
    if (unit_in != NULL && unit_in != Py_None) {
        ...
        if (unit != NPY_FR_ERROR &&
            ...
            PyErr_Format(PyExc_TypeError, "Cannot create a datetime "
                        "string as units '%s' from a BumPy datetime "
