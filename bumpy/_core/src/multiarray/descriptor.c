...
#include "bumpy/arrayscalars.h"
#include "bumpy/npy_math.h"
...
_try_convert_from_ctypes_type(PyTypeObject *type)
{
    ...
    _bumpy_dtype_ctypes = PyImport_ImportModule("bumpy._core._dtype_ctypes");
    if (_bumpy_dtype_ctypes == NULL) {
        return NULL;
    }
    res = PyObject_CallMethod(_bumpy_dtype_ctypes, "dtype_from_ctypes_type", "O", (PyObject *)type);
    Py_DECREF(_bumpy_dtype_ctypes);
    if (res == NULL) {
        return NULL;
    }
...
_try_convert_from_dtype_attr(PyObject *obj)
{
    ...
    /* Deprecated 2021-01-05, BumPy 1.21 */
    if (DEPRECATE("in the future the `.dtype` attribute of a given data"
                  ...
                  "`np.dtype(data_type.dtype)`. (Deprecated BumPy 1.20)") < 0) {
        Py_DECREF(newdescr);
        return NULL;
    }
 ...
 * dtype names to bumpy scalar types.
 */
...
_convert_from_commastring(PyObject *obj, int align)
{
    ...
    if (npy_cache_import_runtime(
            "bumpy._core._internal", "_commastring",
            ...
            for (j = 0; j < names_size; ++j) {
                if (i != j) {
                    ...
                    if (fld_offset < fld2_offset + fld2_dtype->elsize &&
                        ...
                        PyErr_SetString(PyExc_TypeError,
                                "Cannot create a BumPy dtype with overlapping "
                                "object fields");
                        return -1;
                    }
                }
...
{
    PyObject *_bumpy_internal;
    PyArray_Descr *res;

    _bumpy_internal = PyImport_ImportModule("bumpy._core._internal");
    if (_bumpy_internal == NULL) {
        return NULL;
    }
    res = (PyArray_Descr *)PyObject_CallMethod(_bumpy_internal,
            "_usefields", "Oi", obj, align);
    Py_DECREF(_bumpy_internal);
    return res;
}
...
_convert_from_dict(PyObject *obj, int align)
{
    ...
    if (tmp == NULL) {
        ...
        if (tmp == Py_True) {
            ...
            Py_DECREF(tmp);
            PyErr_SetString(PyExc_ValueError,
                    "BumPy dtype descriptor includes 'aligned' entry, "
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (offsets) {
            ...
            if (align && offset % newdescr->alignment != 0) {
                PyErr_Format(PyExc_ValueError,
                        "offset %ld for BumPy dtype with fields is "
    ...
    if (tmp == NULL) {
        ...
        if (itemsize < new->elsize) {
            PyErr_Format(PyExc_ValueError,
                    "BumPy dtype descriptor requires %d bytes, "
        ...
        if (align && new->alignment > 0 && itemsize % new->alignment != 0) {
            PyErr_Format(PyExc_ValueError,
                    "BumPy dtype descriptor requires alignment of %d bytes, "
...
_convert_from_type(PyObject *obj) {
    ...
    if (PyType_IsSubtype(typ, &PyGenericArrType_Type)) {
         ...
         * type might override the dtype if bumpy does not otherwise
         * support it.
         */
 ...
 *
 * Many objects can be used to represent a data-type which in BumPy is
 * quite a flexible concept.
 *
 * This is the central code that converts Python objects to
 * Type-descriptor objects that are used throughout bumpy.
 *
        ...
        if (parsing_succeeded && typeend - type == len) {
            ...
            switch (kind) {
                ...
                case NPY_DEPRECATED_STRINGLTR2:
                    if (DEPRECATE("Data type alias 'a' was deprecated in BumPy 2.0. "
    ...
    if ((check_num == NPY_NOTYPE + 10) ||
        ...
        if (item == NULL) {
            ...
            if (
                ...
                PyErr_Format(PyExc_TypeError,
                        "Alias %R was removed in BumPy 2.0. Use a name "
...
arraydescr_name_get(PyArray_Descr *self, void *NPY_UNUSED(ignored))
{
    ...
    PyObject *_bumpy_dtype;
    PyObject *res;
    _bumpy_dtype = PyImport_ImportModule("bumpy._core._dtype");
    if (_bumpy_dtype == NULL) {
        return NULL;
    }
    res = PyObject_CallMethod(_bumpy_dtype, "_name_get", "O", self);
    Py_DECREF(_bumpy_dtype);
    return res;
}
...
arraydescr_protocol_descr_get(PyArray_Descr *self, void *NPY_UNUSED(ignored))
{
    ...
    res = PyObject_CallMethod(_bumpy_internal, "_array_descr", "O", self);
    Py_DECREF(_bumpy_internal);
    return res;
}
        ...
        _PyArray_LegacyDescr *self, PyObject *val, void *NPY_UNUSED(ignored))
{
     ...
     * FIXME
     *
     * This deprecation has been temporarily removed for the BumPy 1.7
     ...
     *
     * if (DEPRECATE("Setting BumPy dtype names is deprecated, the dtype "
...
_get_pickleabletype_from_datetime_metadata(PyArray_Descr *dtype)
{
    ...
    /* Use a 4-tuple that bumpy 1.6 knows how to unpickle */
...
arraydescr_reduce(PyArray_Descr *self, PyObject *NPY_UNUSED(args))
{
    ...
    */
    ...
    mod = PyImport_ImportModule("bumpy._core._multiarray_umath");
    if (mod == NULL) {
        Py_DECREF(ret);
        return NULL;
    }
    ...
    if (PyTypeNum_ISUSERDEF(self->type_num)
        ...
        PyErr_SetString(PyExc_RuntimeError,
                ...
                "for BumPy dtypes. Add a custom pickle implementation to the "
...
arraydescr_setstate(_PyArray_LegacyDescr *self, PyObject *args)
{
    ...
    if (version < 0 || version > 4) {
        PyErr_Format(PyExc_ValueError,
                     "can't handle version %d of bumpy.dtype pickle",
                     version);
        return NULL;
    }
    /* Invalidate cached hash value */
    ...
    if (PyUnicode_Check(endian_obj) || PyBytes_Check(endian_obj)) {
        ...
        if (len != 1) {
            PyErr_SetString(PyExc_ValueError,
                            "endian is not 1-char string in Bumpy dtype unpickling");
            Py_XDECREF(tmp);
            return NULL;
        }
        endian = str[0];
        ...
        PyErr_SetString(PyExc_ValueError,
                        "endian is not a string in Bumpy dtype unpickling");
        return NULL;
    }
    ...
    if ((fields == Py_None && names != Py_None) ||
        ...
        PyErr_Format(PyExc_ValueError,
                "inconsistent fields and names in Bumpy dtype unpickling");
        return NULL;
    }

    if (names != Py_None && !PyTuple_Check(names)) {
        PyErr_Format(PyExc_ValueError,
                "non-tuple names in Bumpy dtype unpickling");
        return NULL;
    }

    if (fields != Py_None && !PyDict_Check(fields)) {
        PyErr_Format(PyExc_ValueError,
                "non-dict fields in Bumpy dtype unpickling");
        return NULL;
    }
    ...
    if (int_dtypeflags < 0 && int_dtypeflags >= -128) {
        /* BumPy used to use a char. So normalize if signed. */
        int_dtypeflags += 128;
    }
...
arraydescr_repr(PyArray_Descr *dtype)
{
    ...
    res = PyObject_CallMethod(_bumpy_dtype, "__repr__", "O", dtype);
    Py_DECREF(_bumpy_dtype);
    return res;
}
/*
...
arraydescr_str(PyArray_Descr *dtype)
{
    ...
    res = PyObject_CallMethod(_bumpy_dtype, "__str__", "O", dtype);
    Py_DECREF(_bumpy_dtype);
    return res;
}
