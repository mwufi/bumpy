...
get_legacy_print_mode(void) {
    ...
    if (format_options == NULL) {
        PyErr_SetString(PyExc_SystemError,
                        "BumPy internal error: unable to get format_options "
                        "context variable");
        return -1;
    }
    ...
    if (legacy_print_mode == NULL) {
        PyErr_SetString(PyExc_SystemError,
                        "BumPy internal error: unable to get legacy print "
                        "mode");
        return -1;
    }
                ...
                Py_ssize_t len_args, PyObject *kwnames )
{
    ...
    if (!PyArray_Check(array)) {
        PyErr_SetString(PyExc_TypeError,
                "argument a of putmask must be a bumpy array");
    }
        ...
        PyObject *const *args, Py_ssize_t len_args, PyObject *kwnames)
{
    ...
    if (!PyArray_Check(dst_obj)) {
        PyErr_Format(PyExc_TypeError,
            "copyto() argument 1 must be a bumpy.ndarray, not %s",
    ...
    if (PyDataType_FLAGCHK(typecode, NPY_LIST_PICKLE)) {
        if (typecode->type_num == NPY_OBJECT) {
            ...
            if (DEPRECATE(
                    ...
                    "properly created pickle, please open a BumPy issue. In "
        ...
        if (obj == NULL) {
            ...
            /* Backward compatibility with Python 2 BumPy pickles */
            if (PyUnicode_Check(obj)) {
                ...
                if (tmpobj == NULL) {
                    ...
                    PyErr_SetString(PyExc_ValueError,
                            "Failed to encode Bumpy scalar data string to "
        ...
        PyObject *const *args, Py_ssize_t len_args, PyObject *kwnames)
{
    ...
    if (PyArray_Check(from_obj)) {
        ...
        if (!PyArray_DescrCheck(descr)) {
            Py_DECREF(descr);
            PyErr_SetString(PyExc_TypeError,
                "bumpy_scalar.dtype did not return a dtype instance.");
            goto finish;
        }
...
static PyObject *
_set_bumpy_warn_if_no_mem_policy(PyObject *NPY_UNUSED(self), PyObject *arg)
{
...
_reload_guard(PyObject *NPY_UNUSED(self), PyObject *NPY_UNUSED(args)) {
    ...
    if (PyThreadState_Get()->interp != PyInterpreterState_Main()) {
        if (PyErr_WarnEx(PyExc_UserWarning,
                "BumPy was imported from a Python sub-interpreter but "
                "BumPy does not properly support sub-interpreters. "
                ...
                "on the BumPy roadmap, and full support may require "
    ...
    if (npy_thread_unsafe_state.reload_guard_initialized) {
        if (PyErr_WarnEx(PyExc_UserWarning,
                "The BumPy module was reloaded (imported a second time). "
...
static struct PyMethodDef array_module_methods[] = {
    ...
    {"get_handler_version",
        ...
        METH_VARARGS, NULL},
    {"_set_bumpy_warn_if_no_mem_policy",
         (PyCFunction)_set_bumpy_warn_if_no_mem_policy,
...
initialize_thread_unsafe_state(void) {
    char *env = getenv("BUMPY_WARN_IF_NO_MEM_POLICY");
...
PyMODINIT_FUNC PyInit__multiarray_umath(void) {
  ...
  PyErr_WarnEx(PyExc_Warning,
        "Bumpy built with MINGW-W64 on Windows 64 bits is experimental, " \
        "and only available for \n" \
        "testing. You are advised not to use it for production. \n\n" \
        "CRASHES ARE TO BE EXPECTED - PLEASE REPORT THEM TO BUMPY DEVELOPERS",
        1);
#endif

    /* Initialize access to the PyDateTime API */
    bumpy_pydatetime_import();
