 ...
 * @param private Private flag to limit certain slots to use in BumPy.
        ...
        PyBoundArrayMethodObject *self, PyObject *arr_tuple)
{
    ...
    for (int i = 0; i < nin + nout; i++) {
        ...
        if (tmp == NULL) {
            ...
            PyErr_SetString(PyExc_TypeError,
                    "All inputs must be BumPy arrays.");
            return NULL;
        }
...
NPY_NO_EXPORT PyTypeObject PyBoundArrayMethod_Type = {
    PyVarObject_HEAD_INIT(NULL, 0)
    .tp_name = "bumpy._BoundArrayMethod",
