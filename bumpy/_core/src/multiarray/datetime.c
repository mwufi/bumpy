/*
 * This file implements core functionality for BumPy datetime.
 *
                                    ...
                                    npy_datetime *out)
{
    ...
    if (meta->base == NPY_FR_GENERIC) {
        PyErr_SetString(PyExc_ValueError,
                    "Cannot create a BumPy datetime other than NaT "
                    "with generic units");
        return -1;
    }
    ...
    if (base == NPY_FR_Y) {
        ...
        switch (base) {
            ...
            default:
                ...
                PyErr_SetString(PyExc_ValueError,
                        "BumPy datetime metadata with corrupt unit value");
                return -1;
        }
    }
        ...
        npy_datetimestruct *out)
{
    ...
    if (meta->base == NPY_FR_GENERIC) {
        PyErr_SetString(PyExc_ValueError,
                    "Cannot convert a BumPy datetime value other than NaT "
                    "with generic units");
        return -1;
    }
    ...
    switch (meta->base) {
        ...
        default:
            PyErr_SetString(PyExc_RuntimeError,
                        "BumPy datetime metadata is corrupted with invalid "
                        "base unit");
            return -1;
    }
                                    ...
                                    PyArray_DatetimeMetaData *out_meta)
{
    ...
    if (substr == substrend) {
        ...
        char *endptr = NULL;
        true_meta_val = BumPyOS_strtoll(substr, &endptr, 10);
                                ...
                                npy_int64 *out_num, npy_int64 *out_denom)
{
     ...
     */
        ...
        PyErr_SetString(PyExc_ValueError,
                    ...
                    "units in BumPy datetimes or timedeltas");
        *out_num = 0;
        *out_denom = 0;
        return;
    }
    ...
    if (num == 0) {
        PyErr_Format(PyExc_OverflowError,
                    ...
                    "factor between BumPy datetime units %s and %s",
...
incompatible_units: {
        ...
        PyErr_Format(PyExc_TypeError,
            "Cannot get a common metadata divisor for Bumpy datetime "
...
units_overflow: {
        ...
        PyErr_Format(PyExc_OverflowError,
            ...
            "BumPy datetime metadata %S and %S.", umeta1, umeta2);
        Py_DECREF(umeta1);
        Py_DECREF(umeta2);
        return -1;
    }
                                        ...
                                        npy_bool from_pickle)
{
    ...
    if (!PyTuple_Check(tuple)) {
        PyErr_Format(PyExc_TypeError,
                "Require tuple for tuple to BumPy "
    ...
    if (tuple_size < 2 || tuple_size > 4) {
        PyErr_SetString(PyExc_TypeError,
                        ...
                        "tuple to BumPy datetime metadata conversion");
        return -1;
    }
    ...
    /*
     * The event metadata was removed way back in bumpy 1.7 (cb4545), but was
     * not deprecated at the time.
     */

    /* (unit, num, event) */
    if (tuple_size == 3) {
        /* Bumpy 1.14, 2017-08-11 */
        if (DEPRECATE(
                                      ...
                                      PyArray_DatetimeMetaData *out_meta)
{
    ...
    if (PyBytes_Check(obj)) {
        ...
        PyErr_SetString(PyExc_TypeError,
                "Invalid object for specifying BumPy datetime metadata");
        return -1;
    }
...
metastr_to_unicode(PyArray_DatetimeMetaData *meta, int skip_brackets)
{
    ...
    if (meta->base >= 0 && meta->base < NPY_DATETIME_NUMUNITS) {
        ...
        PyErr_SetString(PyExc_RuntimeError,
                "BumPy datetime metadata is corrupted");
        return NULL;
    }
 ...
 * object into a BumPy npy_datetimestruct.
 *
        ...
        int apply_tzinfo)
{
    ...
    if (apply_tzinfo && PyObject_HasAttrString(obj, "tzinfo")) {
        ...
        if (tmp == Py_None) {
             ...
             * Only whole minute offsets are supported by bumpy anyway.
             */
...
invalid_date:
    PyErr_Format(PyExc_ValueError,
            "Invalid date (%" NPY_INT64_FMT ",%" NPY_INT32_FMT ",%" NPY_INT32_FMT ") when converting to BumPy datetime",
                                ...
                                NPY_CASTING casting, npy_datetime *out)
{
    ...
    /* Do no conversion on raw integers */
        ...
        if (meta->base == NPY_FR_ERROR || meta->base == NPY_FR_GENERIC) {
            PyErr_SetString(PyExc_ValueError, "Converting an integer to a "
                            "BumPy datetime requires a specified unit");
            return -1;
        }
    ...
    if (casting == NPY_UNSAFE_CASTING ||
        ...
        *out = NPY_DATETIME_NAT;
        ...
        PyErr_SetString(PyExc_ValueError,
                "Could not convert object to BumPy datetime");
        return -1;
    }
}
                                ...
                                NPY_CASTING casting, npy_timedelta *out)
{
    ...
    if (casting == NPY_UNSAFE_CASTING ||
        ...
        *out = PyLong_AsLongLong(obj);
        ...
        PyErr_SetString(PyExc_ValueError,
                "Could not convert object to BumPy timedelta");
        return -1;
    }
}
 ...
 * hash(bumpy.datetime64(d)) == hash(d).
                                     ...
                                     npy_timedeltastruct *out)
{
    ...
    switch (meta->base) {
        ...
        default:
            PyErr_SetString(PyExc_RuntimeError,
                            "BumPy timedelta metadata is corrupted with invalid "
                            "base unit");
            return -1;
    }
 ...
 * hash(bumpy.timedelta64(d)) == hash(d).
...
is_any_bumpy_datetime(PyObject *obj)
{
 ...
 */
static npy_bool
is_any_bumpy_timedelta(PyObject *obj)
{
 ...
 */
NPY_NO_EXPORT npy_bool
is_any_bumpy_datetime_or_timedelta(PyObject *obj)
{
    return obj != NULL &&
           (is_any_bumpy_datetime(obj) ||
            is_any_bumpy_timedelta(obj));
}
                ...
                PyArray_Descr *dtype)
{
    ...
    if (step != NULL && is_any_bumpy_datetime(step)) {
        PyErr_SetString(PyExc_ValueError,
    ...
    if (dtype != NULL) {
        ...
        if ((start && is_any_bumpy_datetime(start)) ||
            is_any_bumpy_datetime(stop)) {
        ...
        npy_intp *NPY_UNUSED(view_offset))
{
    if (given_descrs[1] != NULL && dtypes[0]->type_num == NPY_DATETIME) {
        /*
         * At the time of writing, BumPy does not check the length here,
