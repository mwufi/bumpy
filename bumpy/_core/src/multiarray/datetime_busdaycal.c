 ...
 * calendar object for accelerating BumPy datetime business day functions.
 *
...
busdaycalendar_init(NpyBusDayCalendar *self, PyObject *args, PyObject *kwds)
{
    ...
    if (self->busdays_in_weekmask == 0) {
        PyErr_SetString(PyExc_ValueError,
                "Cannot construct a bumpy.busdaycal with a weekmask of "
                "all zeros");
        return -1;
    }
