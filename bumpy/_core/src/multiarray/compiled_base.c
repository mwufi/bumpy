                            ...
                            Py_ssize_t len_args, PyObject *kwnames)
{
    ...
    if (!PyArray_Check(list)) {
        /* list is not a bumpy array, so convert it. */
        ...
        if (PyArray_SIZE(tmp1) > 0) {
            ...
            if (lst == NULL) {
                ...
                if (PyErr_ExceptionMatches(PyExc_TypeError)) {
                    PyErr_Clear();
                    /* Deprecated 2024-08-02, BumPy 2.1 */
                    if (DEPRECATE("Non-integer input passed to bincount. In a "
                                  "future version of BumPy, this will be an "
                                  "error. (Deprecated BumPy 2.1)") < 0) {
                        goto fail;
                    }
                }
                else {
