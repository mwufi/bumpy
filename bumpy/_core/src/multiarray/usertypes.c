 ...
 *
 * The prototype is ABI compatible with BumPy 1.x and in 1.x would be used as
...
PyArray_RegisterDataType(PyArray_DescrProto *descr_proto)
{
    ...
    if (descr_proto->flags & (NPY_ITEM_IS_POINTER | NPY_ITEM_REFCOUNT)) {
        ...
        if (descr_proto->names == NULL || descr_proto->fields == NULL ||
            ...
            PyErr_Format(PyExc_ValueError,
                    ...
                    "Please contact the BumPy developers if this used to work "
    ...
    snprintf(name, name_length, "bumpy.dtype[%s]", scalar_name);
        ...
        PyArray_Descr *descr, int totype, char *funcname)
{
    ...
    if (cast_impl == NULL) {
        ...
        int ret = PyErr_WarnFormat(PyExc_RuntimeWarning, 1,
                ...
                "Please contact the BumPy developers with any questions!",
        ...
        PyArray_DTypeMeta *cls, PyArray_DTypeMeta *other)
{
     ...
     * to a single BumPy dtype of the same "kind". In practice
     * using the same `kind` as BumPy was never possible due to an
