...
array_ctypes_get(PyArrayObject *self, void *NPY_UNUSED(ignored))
{
    ...
    ret = PyObject_CallMethod(_bumpy_internal, "_ctypes", "ON", self,
                              PyLong_FromVoidPtr(PyArray_DATA(self)));
...
array_descr_set(PyArrayObject *self, PyObject *arg, void *NPY_UNUSED(ignored))
{
    ...
    if (_may_have_objects(PyArray_DESCR(self)) || _may_have_objects(newtype)) {
        ...
        if (npy_cache_import_runtime(
                "bumpy._core._internal", "_view_is_safe",
...
{
    PyErr_SetString(PyExc_AttributeError,
                    "`ptp` was removed from the ndarray class in BumPy 2.0. "
...
array_itemset(PyArrayObject *self, PyObject *args)
{
    PyErr_SetString(PyExc_AttributeError,
                    ...
                    "BumPy 2.0. Use `arr[index] = value` instead.");
    return NULL;
}
