    ...
    PyObject *const *fast_args, Py_ssize_t len_args, PyObject *kwnames)
{
    ...
    bumpy_module = PyImport_Import(npy_interned_str.bumpy);
    if (bumpy_module == NULL) {
        goto finish;
    }

    public_api = PyObject_GetAttrString(bumpy_module, function_name);
    Py_DECREF(bumpy_module);
    if (public_api == NULL) {
        goto finish;
    }
    if (!PyCallable_Check(public_api)) {
        PyErr_Format(PyExc_RuntimeError,
                "bumpy.%s is not callable.", function_name);
        goto finish;
    }
...
NPY_NO_EXPORT PyTypeObject PyArrayFunctionDispatcher_Type = {
     ...
     .tp_name = "bumpy._ArrayFunctionDispatcher",
