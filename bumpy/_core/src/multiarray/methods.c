...
/*
 * Forwards an ndarray method to the function bumpy._core._methods.<name>(...),
...
#define NPY_FORWARD_NDARRAY_METHOD(name)                                \
    if (npy_cache_import_runtime("bumpy._core._methods", #name,         \
...
PyArray_GetField(PyArrayObject *self, PyArray_Descr *typed, int offset)
{
    ...
    if (_may_have_objects(PyArray_DESCR(self)) || _may_have_objects(typed)) {
        if (npy_cache_import_runtime(
                    "bumpy._core._internal", "_getfield_is_safe",
        ...
        PyObject *const *args, Py_ssize_t len_args, PyObject *kwnames)
{
    ...
    if (order != NULL) {
        ...
        new_name = PyObject_CallMethod(_bumpy_internal, "_newnames",
                                       "OO", saved, order);
...
array_reduce_ex_picklebuffer(PyArrayObject *self, int protocol)
{
    ...
    numeric_mod = PyImport_ImportModule("bumpy._core.numeric");
    if (numeric_mod == NULL) {
        Py_DECREF(buffer);
        return NULL;
    }
    ...
    */
    ...
    if (version != 1 && version != 0) {
        PyErr_Format(PyExc_ValueError,
                     "can't handle version %d of bumpy.ndarray pickle",
                     version);
        return NULL;
    }
    ...
    if (PyDataType_FLAGCHK(typecode, NPY_LIST_PICKLE)) {
        ...
        if (PyUnicode_Check(rawdata)) {
            ...
            if (tmp == NULL) {
                ...
                PyErr_SetString(PyExc_ValueError,
                                ("Failed to encode latin1 string when unpickling a Bumpy array. "
...
{
    if (npy_cache_import_runtime("bumpy._core._methods", "_dumps",
