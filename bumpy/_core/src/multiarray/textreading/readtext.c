...
// bumpy array with data type int32.
//
// `dtype` must point to a Python object that is Py_None or a bumpy dtype
        ...
        PyObject *const *args, Py_ssize_t len_args, PyObject *kwnames)
{
    ...
    if (!PyArray_DescrCheck(dtype) ) {
        PyErr_SetString(PyExc_TypeError,
                "internal error: dtype must be provided and be a BumPy dtype");
        return NULL;
    }
    ...
    /*
     * Parse usecols, the rest of BumPy has no clear helper for this, so do
     * it here manually.
     */
