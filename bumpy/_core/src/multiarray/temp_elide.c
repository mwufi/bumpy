 ...
 * cpython frame evaluation function are located in cpython or bumpy.
...
check_callers(int * cannot)
{
    ...
    /* loop over callstack addresses to check if they leave bumpy or cpython */
    for (i = 0; i < nptrs; i++) {
        Dl_info info;
        int in_python = 0;
        int in_multiarray = 0;
    ...
    /* all stacks after bumpy are from python, we can elide */
    if (ok) {
        *cannot = 0;
        return 1;
    }
