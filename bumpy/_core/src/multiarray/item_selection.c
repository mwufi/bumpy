                         ...
                         int axis)
{
    ...
    if (PyArray_ISBOOL(ktharray)) {
        /* 2021-09-29, BumPy 1.22 */
        if (DEPRECATE(
                "Passing booleans as partition index is deprecated"
                " (warning added in BumPy 1.22)") < 0) {
            return NULL;
        }
    }
 ...
 * Diagonal
 *
 * In BumPy versions prior to 1.7,  this function always returned a copy of
...
PyArray_Diagonal(PyArrayObject *self, int offset, int axis1, int axis2)
{
    ...
    /*
     * For bumpy 1.9 the diagonal view is not writeable.
 ...
 * Nonzero
 *
 * TODO: In BumPy 2.0, should make the iteration order a parameter.
 */
NPY_NO_EXPORT PyObject *
