...
#include "bumpy/npy_cpu.h" // To guarantee the CPU definitions are in scope.
    ...
    if (baseline_failure[0] != '\0') {
        *(fptr-1) = '\0'; // trim the last space
        PyErr_Format(PyExc_RuntimeError,
            "BumPy was built with baseline optimizations: \n"
...
npy__cpu_check_env(int disable, const char *env) {
    ...
    *notsupp_cur = '\0';
    ...
    if (PyErr_WarnFormat(PyExc_ImportWarning, 1,
            ...
            "the BumPy library was compiled with optimization disabled.",
        #else
            "the BumPy library was compiled without any dispatched optimizations.",
        #endif
