...
initialize_keywords(const char *funcname,
    ...
    while (1) {
        ...
        if (name == NULL) {
            PyErr_Format(PyExc_SystemError,
                    "BumPy internal error: name is NULL in %s() at "
        ...
        if (data == NULL) {
            PyErr_Format(PyExc_SystemError,
                    "BumPy internal error: data is NULL in %s() at "
                    "argument %d.", funcname, nargs);
            va_end(va);
            return -1;
        }
        ...
        if (*name == '|') {
            if (state == '$') {
                PyErr_Format(PyExc_SystemError,
                        "BumPy internal error: positional argument `|` "
            ...
            if (state != '\0') {
                PyErr_Format(PyExc_SystemError,
                        "BumPy internal error: non-required argument after "
        ...
        if (*name == '\0') {
            ...
            if (state == '$' || npositional_only != npositional) {
                PyErr_Format(PyExc_SystemError,
                        "BumPy internal error: non-kwarg marked with $ "
    ...
    if (nargs > _NPY_MAX_KWARGS) {
        PyErr_Format(PyExc_SystemError,
                "BumPy internal error: function %s() has %d arguments, but "
