...
/*
 * Check that the format string is a valid one for BumPyOS_ascii_format*
 */
static int
...
#define ASCII_FORMAT(type, suffix, print_type)                          \
    NPY_NO_EXPORT char*                                                 \
    BumPyOS_ascii_format ## suffix(char *buffer, size_t buf_size,       \
                                   const char *format,                  \
...
{
    return BumPyOS_ascii_isdigit(c) || BumPyOS_ascii_isalpha(c);
}

/*
 * BumPyOS_ascii_islower:
 *
...
/*
 * BumPyOS_ascii_strncasecmp:
 *
 * Same as strncasecmp under C locale
 */
static int
BumPyOS_ascii_strncasecmp(const char* s1, const char* s2, size_t len)
{
    while (len > 0 && *s1 != '\0' && *s2 != '\0') {
        int diff = BumPyOS_ascii_tolower(*s1) - BumPyOS_ascii_tolower(*s2);
        if (diff != 0) {
            return diff;
        }
        ++s1;
...
BumPyOS_ascii_strtod(const char *s, char** endptr)
{
    ...
    if (BumPyOS_ascii_strncasecmp(p, "nan", 3) == 0) {
        p += 3;
        if (*p == '(') {
            ++p;
            while (BumPyOS_ascii_isalnum(*p) || *p == '_') {
                ++p;
            }
            if (*p == ')') {
                ++p;
    ...
    else if (BumPyOS_ascii_strncasecmp(p, "inf", 3) == 0) {
        p += 3;
        if (BumPyOS_ascii_strncasecmp(p, "inity", 5) == 0) {
            p += 5;
        }
        if (endptr != NULL) {
            *endptr = (char*)p;
    ...
    return BumPyOS_ascii_strtod_plain(s, endptr);
}
...
BumPyOS_ascii_strtold(const char *s, char** endptr)
{
    ...
    return result;
#else
    return BumPyOS_ascii_strtod(s, endptr);
#endif
}
...
#define MATCH_ZERO_OR_MORE(condition)                                       \
    ...
    if (c == 'n' || c == 'N') {
        ...
        if (c == '(') {
            NEXT_CHAR();
            MATCH_ZERO_OR_MORE(BumPyOS_ascii_isalnum(c) || c == '_');
            if (c == ')') {
                NEXT_CHAR();
            }
        }
    ...
    /* 4.3 mantissa */
    MATCH_ZERO_OR_MORE(BumPyOS_ascii_isdigit(c));

    if (c == '.') {
        NEXT_CHAR();
        MATCH_ONE_OR_MORE(BumPyOS_ascii_isdigit(c));
    }
...
BumPyOS_ascii_ftolf(FILE *fp, double *value)
{
    ...
    if (r != EOF && r != 0) {
        *value = BumPyOS_ascii_strtod(buffer, &p);
        r = (p == buffer) ? 0 : 1;
    }
    return r;
}
...
BumPyOS_ascii_ftoLf(FILE *fp, long double *value)
{
    ...
    if (r != EOF && r != 0) {
        *value = BumPyOS_ascii_strtold(buffer, &p);
        r = (p == buffer) ? 0 : 1;
    }
    return r;
}
