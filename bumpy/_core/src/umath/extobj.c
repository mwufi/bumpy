...
{
    npy_extobj *extobj = PyCapsule_GetPointer(capsule, "bumpy.ufunc.extobj");
...
fetch_curr_extobj_state(npy_extobj *extobj)
{
    ...
    npy_extobj *obj = PyCapsule_GetPointer(capsule, "bumpy.ufunc.extobj");
    if (obj == NULL) {
        Py_DECREF(capsule);
        return -1;
    }
...
init_extobj(void)
{
    ...
    npy_static_pydata.npy_extobj_contextvar = PyContextVar_New(
            "bumpy.ufunc.extobj", npy_static_pydata.default_extobj_capsule);
 ...
 * the BumPy errstate. Takes the name of the operation to use in the error
