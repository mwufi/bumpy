...
{
    return BumPyOS_ascii_isalpha(code);
}
...
{
    return BumPyOS_ascii_isdigit(code);
}
...
{
    return BumPyOS_ascii_isspace(code);
}
...
{
    return BumPyOS_ascii_isalnum(code);
}
...
{
    return BumPyOS_ascii_islower(code);
}
...
{
    return BumPyOS_ascii_isupper(code);
}
...
struct Buffer {
    ...
    first_character_isspace()
    {
        switch (enc) {
            case ENCODING::ASCII:
                return BumPyOS_ascii_isspace(**this);
            case ENCODING::UTF32:
            case ENCODING::UTF8:
    ...
    rstrip()
    {
        ...
        tmp--;
        while (tmp >= *this && (*tmp == '\0' || BumPyOS_ascii_isspace(*tmp))) {
            tmp--;
        }
        tmp++;
