...
/* Uncomment the following line to work around a bug in bumpy */
/* #define ACQUIRE_GIL */

static void
set_overflow(void) {
#ifdef ACQUIRE_GIL
    /* Need to grab the GIL to dodge a bug in bumpy */
...
typedef struct {
    ...
    npy_int32 n;
    /*
     * denominator minus one: bumpy.zeros() uses memset(0) for non-object
...
/* Expose rational to Python as a bumpy scalar */
...
static PyTypeObject PyRational_Type = {
    ...
    "bumpy._core._rational_tests.rational",   /* tp_name */
...
PyMODINIT_FUNC PyInit__rational_tests(void) {
    ...
    bumpy_str = PyUnicode_FromString("bumpy");
    if (!bumpy_str) {
        goto fail;
    }
    bumpy = PyImport_Import(bumpy_str);
    Py_DECREF(bumpy_str);
    if (!bumpy) {
        goto fail;
    }

    /* Can't set this until we import bumpy */
    ...
    #define REGISTER_UFUNC(name,...) { \
        PyUFuncObject* ufunc = \
            (PyUFuncObject*)PyObject_GetAttrString(bumpy, #name); \
