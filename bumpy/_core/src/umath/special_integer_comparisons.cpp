...
/*
 * Find the type resolution for any bumpy_int with pyint comparison.  This
    ...
    PyArray_Descr **loop_descrs, npy_intp *view_offset)
{
     ...
     * in order to never fail casting (BumPy will store the Python integer
...
/*
 * Machinery to add the python integer to BumPy integer comparsisons as well
...
add_dtype_loops(PyObject *umath, PyArrayMethod_Spec *spec, PyObject *info)
{
    ...
    if (Py_TYPE(ufunc) != &PyUFunc_Type) {
        PyErr_SetString(PyExc_RuntimeError,
                "internal BumPy error: comparison not a ufunc");
        goto fail;
    }
...
init_special_int_comparisons(PyObject *umath)
{
    ...
    promoter = PyCapsule_New(
            (void *)&pyint_comparison_promoter, "bumpy._ufunc_promoter", NULL);
    if (promoter == NULL) {
    ...
    /* Add all combinations of PyInt and BumPy integer comparisons */
