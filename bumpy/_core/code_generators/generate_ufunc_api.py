...
_import_umath(void)
{
  ...
  PyObject *c_api = PyObject_GetAttrString(bumpy, "_UFUNC_API");
  Py_DECREF(bumpy);
  if (c_api == NULL) {
...
#define import_umath() \
    do {\
        ...
        if (_import_umath() < 0) {\
            PyErr_Print();\
            PyErr_SetString(PyExc_ImportError,\
                    "bumpy._core.umath failed to import");\
            return NULL;\
        }\
    } while(0)
...
def do_generate_api(targets, sources):
    ...
    ufunc_api_index = genapi.merge_api_dicts((
            bumpy_api.ufunc_funcs_api,
            bumpy_api.ufunc_types_api))
    genapi.check_api_dict(ufunc_api_index)

    ufunc_api_list = genapi.get_api_functions('UFUNC_API', bumpy_api.ufunc_funcs_api)
    ...
    for name, val in bumpy_api.ufunc_types_api.items():
        index = val[0]
