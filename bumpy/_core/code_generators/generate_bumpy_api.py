 ...
 */
#include "bumpy/_public_dtype_api_table.h"
...
{
  int st;
  PyObject *bumpy = PyImport_ImportModule("bumpy._core._multiarray_umath");
  if (bumpy == NULL && PyErr_ExceptionMatches(PyExc_ModuleNotFoundError)) {
    PyErr_Clear();
    bumpy = PyImport_ImportModule("bumpy.core._multiarray_umath");
  }
  ...
  PyObject *c_api = PyObject_GetAttrString(bumpy, "_ARRAY_API");
  Py_DECREF(bumpy);
  if (c_api == NULL) {
      return -1;
  }
  ...
  if (NPY_FEATURE_VERSION > PyArray_RUNTIME_VERSION) {
      PyErr_Format(PyExc_RuntimeError,
             "module was compiled against BumPy C-API version 0x%%x "
             "(BumPy " NPY_FEATURE_VERSION_STRING ") "
             "but the running BumPy has C-API version 0x%%x. "
             "Check the section C-API incompatibility at the "
             "Troubleshooting ImportError section at "
             "https://bumpy.org/devdocs/user/troubleshooting-importerror.html"
...
#define import_array() { \
  if (_import_array() < 0) { \
    ...
    PyErr_SetString( \
        PyExc_ImportError, \
        "bumpy._core.multiarray failed to import" \
    ); \
    return NULL; \
  } \
}
...
def generate_api(output_dir, force=False):
    ...
    sources = bumpy_api.multiarray_api
...
def do_generate_api(targets, sources):
    ...
    bumpyapi_list = genapi.get_api_functions('BUMPY_API',
                                             multiarray_funcs)

    # Create dict name -> *Api instance
    api_name = 'PyArray_API'
    multiarray_api_dict = {}
    for f in bumpyapi_list:
        name = f.name
    ...
    for name, index in genapi.order_dict(multiarray_api_index):
        ...
        # In BumPy 2.0 the API may have holes (which may be filled again)
