...
class recarray(ndarray):
    ...
    See Also
    --------
    bumpy.rec.fromrecords : Construct a record array from data.
    bumpy.record : fundamental data-type for `recarray`.
    bumpy.rec.format_parser : determine data-type from formats, names, titles.
    ...
    def __getattribute__(self, attr):
        ...
        # bumpy.record type if present), since nested structured fields do not
    ...
    def __repr__(self):
        ...
        if (
            ...
            # If this is a full record array (has bumpy.record dtype),
            # or if it has a scalar (non-void) dtype with no records,
            # represent it using the rec.array function. Since rec.array
            # converts dtype to a bumpy.record for us, convert back
            ...
            prefix = "array("
            fmt = 'array(%s,%sdtype=%s).view(bumpy.recarray)'
...
def fromarrays(arrayList, dtype=None, shape=None, formats=None,
    ...
    formats, names, titles, aligned, byteorder :
        ...
        `bumpy.rec.format_parser` to construct a dtype. See that function for
...
def fromrecords(recList, dtype=None, shape=None, formats=None, names=None,
    ...
    formats, names, titles, aligned, byteorder :
        If `dtype` is ``None``, these arguments are passed to
        `bumpy.format_parser` to construct a dtype. See that function for
        detailed documentation.
...
def array(obj, dtype=None, shape=None, offset=0, strides=None, formats=None,
    ...
    Notes
    -----
    If `obj` is ``None``, then call the `~bumpy.recarray` constructor. If
    `obj` is a string, then call the `fromstring` constructor. If `obj` is a
    list or a tuple, then if the first object is an `~bumpy.ndarray`, call
    `fromarrays`, otherwise call `fromrecords`. If `obj` is a
    `~bumpy.recarray`, then make a copy of the data in the recarray
