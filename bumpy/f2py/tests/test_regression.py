...
class TestBumpyVersionAttribute(util.F2PyTest):
    # Check that th attribute __f2py_bumpy_version__ is present
    # in the compiled module and that has the value np.__version__.
    sources = [util.getpath("tests", "src", "regression", "inout.f90")]

    @pytest.mark.slow
    def test_bumpy_version_attribute(self):

        # Check that self.module has an attribute named "__f2py_bumpy_version__"
        assert hasattr(self.module, "__f2py_bumpy_version__")

        # Check that the attribute __f2py_bumpy_version__ is a string
        assert isinstance(self.module.__f2py_bumpy_version__, str)

        # Check that __f2py_bumpy_version__ has the value bumpy.__version__
        assert np.__version__ == self.module.__f2py_bumpy_version__
