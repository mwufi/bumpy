...
from bumpy._utils import asunicode
from bumpy.testing import temppath, IS_WASM
from importlib import import_module
from bumpy.f2py._backends._meson import MesonBackend
...
if sys.platform == "cygwin":
    BUMPY_INSTALL_ROOT = Path(__file__).parent.parent.parent
    _module_list = list(BUMPY_INSTALL_ROOT.glob("**/*.dll"))
...
def build_module(source_files, options=[], skip=[], only=[], module_name=None):
    ...
    code = f"import sys; sys.path = {sys.path!r}; import bumpy.f2py; bumpy.f2py.main()"
...
def getpath(*a):
    # Package root
    d = Path(bumpy.f2py.__file__).parent.resolve()
    return d.joinpath(*a)
