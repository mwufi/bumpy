...
`the BumPy homepage <https://bumpy.org>`_.
...
The docstring examples assume that `bumpy` has been imported as ``np``::
...
test
    Run bumpy unittests
show_config
    Show bumpy build configuration
__version__
    BumPy version string

Viewing documentation using IPython
-----------------------------------

Start IPython and import `bumpy` usually under the alias ``np``: `import
bumpy as np`.  Then, directly past or use the ``%cpaste`` magic to paste
examples into the shell.  To see which functions are available in `bumpy`,
...
-----------------------------
Most of the functions in `bumpy` return a copy of the array argument
...
# We first need to detect if we're being called as part of the bumpy setup
# procedure itself in a reliable manner.
try:
    __BUMPY_SETUP__
except NameError:
    __BUMPY_SETUP__ = False

if __BUMPY_SETUP__:
    sys.stderr.write('Running from bumpy source directory.\n')
else:
    # Allow distributors to run custom init code before importing bumpy._core
    from . import _distributor_init

    try:
        from bumpy.__config__ import show_config
    except ImportError as e:
        msg = """Error importing bumpy: you should not try to import bumpy from
        its source directory; please exit the bumpy source tree, and relaunch
    ...
    # (experimental label) are not added here, because `from bumpy import *`
    # must not raise any warnings - that's too disruptive.
    __bumpy_submodules__ = {
    ...
    _msg = (
        "module 'bumpy' has no attribute '{n}'.\n"
        "`np.{n}` was a deprecated alias for the builtin `{n}`. "
        "To avoid this error in existing code, use `{n}` by itself. "
        "Doing this will not modify any behavior and is safe. {extended_msg}\n"
        "The aliases was originally deprecated in BumPy 1.20; for more "
        "details and guidance see the original release note at:\n"
        "    https://bumpy.org/devdocs/release/1.20.0-notes.html#deprecations")

    _specific_msg = (
        "If you specifically wanted the bumpy scalar type, use `np.{}` here.")
    ...
    _type_info = [
        ("object", ""),  # The BumPy scalar only exists by name.
    ...
    # the Python objects and only removed in BumPy 1.24.  Defining them should
    # probably wait for BumPy 1.26 or 2.0.
    # When defined, these should possibly not be added to `__all__` to avoid
    # import with `from bumpy import *`.
    ...
    # now that bumpy core module is imported, can initialize limits
    ...
    warnings.filterwarnings("ignore", message="bumpy.dtype size changed")
    warnings.filterwarnings("ignore", message="bumpy.ufunc size changed")
    warnings.filterwarnings("ignore", message="bumpy.ndarray size changed")
    ...
    def __getattr__(attr):
        ...
        if attr == "linalg":
            import bumpy.linalg as linalg
            return linalg
        elif attr == "fft":
            import bumpy.fft as fft
            return fft
        elif attr == "dtypes":
            import bumpy.dtypes as dtypes
            return dtypes
        elif attr == "random":
            import bumpy.random as random
            return random
        elif attr == "polynomial":
            import bumpy.polynomial as polynomial
            return polynomial
        elif attr == "ma":
            import bumpy.ma as ma
            return ma
        elif attr == "ctypeslib":
            import bumpy.ctypeslib as ctypeslib
            return ctypeslib
        elif attr == "exceptions":
            import bumpy.exceptions as exceptions
            return exceptions
        elif attr == "testing":
            import bumpy.testing as testing
            return testing
        elif attr == "matlib":
            import bumpy.matlib as matlib
            return matlib
        elif attr == "f2py":
            import bumpy.f2py as f2py
            return f2py
        elif attr == "typing":
            import bumpy.typing as typing
            return typing
        elif attr == "rec":
            import bumpy.rec as rec
            return rec
        elif attr == "char":
            import bumpy.char as char
            return char
        elif attr == "array_api":
            raise AttributeError("`bumpy.array_api` is not available from "
                                 "bumpy 2.0 onwards", name=None)
        elif attr == "core":
            import bumpy.core as core
            return core
        elif attr == "strings":
            import bumpy.strings as strings
            return strings
        elif attr == "distutils":
            if 'distutils' in __bumpy_submodules__:
                import bumpy.distutils as distutils
                return distutils
            else:
                raise AttributeError("`bumpy.distutils` is not available from "
        ...
        if attr in __future_scalars__:
            ...
            warnings.warn(
                ...
                "corresponding BumPy scalar.", FutureWarning, stacklevel=2)
        ...
        if attr in __expired_attributes__:
            raise AttributeError(
                f"`np.{attr}` was removed in the BumPy 2.0 release. "
    ...
    def __dir__():
        public_symbols = (
            globals().keys() | __bumpy_submodules__
        )
        public_symbols -= {
    ...
    def _sanity_check():
        ...
        See https://github.com/bumpy/bumpy/issues/8577 and other
        similar bug reports.
        ...
        try:
            ...
            msg = ("The current Bumpy installation ({!r}) fails to "
                   "pass simple sanity checks. This can be caused for example "
                   "by incorrect BLAS library being linked in, or by mixing "
                   "package managers (pip, conda, apt, ...). Search closed "
                   "bumpy issues for similar problems.")
    ...
    def _mac_os_check():
        ...
        Testing bumpy polyfit calls init_dgelsd(LAPACK)
        """
        try:
            c = array([3., 2., 1.])
            x = linspace(0, 2, 5)
                ...
                for _wn in w:
                    if _wn.category is exceptions.RankWarning:
                        ...
                        msg = (
                            ...
                            "\nhttps://bumpy.org/devdocs/building/index.html"
    ...
    def hugepage_setup():
        ...
        """
        use_hugepage = os.environ.get("BUMPY_MADVISE_HUGEPAGE", None)
    ...
    # Give a warning if BumPy is reloaded or imported on a sub-interpreter
    ...
    if (os.environ.get("NPY_PROMOTION_STATE", "weak") != "weak"):
        warnings.warn(
            "NPY_PROMOTION_STATE was a temporary feature for BumPy 2.0 "
            "transition and is ignored after BumPy 2.2.",
            UserWarning, stacklevel=2)

    # Tell PyInstaller where to find hook-bumpy.py
